<script type="text/javascript">
	...
	function ChMenuChild(JSONMenuApplication) {
		...
		const details = MenuApplication.MenuDiagramDetail;
		const detail = details?.find(d => d.menu_diagram_detail_urut == 1);

		if (detail) {
			$("#diagramdetail")
				.html(detail.menu_diagram_detail_keterangan ?? '')
				.show();
		} else {
			$("#diagramdetail").hide();
		}

		$("#diagramdetail2").html(details.find(d => d.menu_diagram_detail_urut == 2)?.menu_diagram_detail_keterangan ?? '');

		if (MenuApplication.MenuNode != 0) {
			let last_menu_parent = null;

			for (let i = 0; i < MenuApplication.MenuNode.length; i++) {

				let item = MenuApplication.MenuNode[i];

				// 👉 jika menu_parent berubah (dan bukan item pertama)
				if (last_menu_parent !== item.menu_parent) {
					$("#addicons").append(`
						<div class="grid-separator">
							<span class="menu-parent-title">${item.menu_name_parent}</span>
							<hr>
						</div>
					`);
				}

				last_menu_parent = item.menu_parent;

				$("#addicons").append(`
						<div class="icon-item">
							<a href="<?= base_url() ?>${item.menu_link}"
								class="icon-circle"
								style="border: 5px solid ${item.menu_node_background_color}">
								<img src="<?= Get_Assets_Url(); ?>Global/icomoon/PNG/16px/${item.menu_node_icon}" alt="icon">
							</a>
							<span class="icon-label">${item.menu_node_menu_name}</span>
						</div>
					`);
			}

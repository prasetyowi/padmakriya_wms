...
$("#btn-history-pallet").on("click", function() {
    ...
    $.ajax({
        url: "<?= base_url('WMS/Laporan/LaporanInformasiPallet/get_history_pallet_by_id'); ?>",
        type: "POST",
        data: {
...
$(document).on("click", "#start_scan", function() {
    ...
    const scan = (decodedText) => {
        ...
        $.ajax({
            url: "<?= base_url('WMS/Laporan/LaporanInformasiPallet/get_pallet_by_kode'); ?>",
            type: "POST",
            data: {
...
$("#search_kode_pallet").on('click', function() {
    if ($("#kode_barcode").val() != "") {
        fetch('<?= base_url('WMS/Laporan/LaporanInformasiPallet/getKodeAutoComplete?params='); ?>' + $(
                "#kode_barcode").val())
...
//     if (this.value != "") {
//         fetch('<?= base_url('WMS/Laporan/LaporanInformasiPallet/getKodeAutoComplete?params='); ?>' + this.value)
//             .then(response => response.json())
//             .then((results) => {
//                 if (!results[0]) {
//                     document.getElementById('table-fixed').style.display = 'none';
...
$('#release_pallet').on('click', function() {
    ...
    $.ajax({
        type: "POST",
        url: "<?= base_url('WMS/Laporan/LaporanInformasiPallet/releasePallet'); ?>",
        data: {
            pallet_id: pallet_id
        },
        dataType: "JSON",
        success: function(response) {
            if (response != 0) {
                ...
                setTimeout(() => {
                    location.href =
                        "<?= base_url("WMS/Laporan/LaporanInformasiPallet/LaporanInformasiPalletMenu") ?>";
                }, 2000);
            } else {
...
$("#deletePalletDetail").on('click', function() {
    ...
    confirmAlert('Apakah anda yakin ingin menghapus?', 'Tekan ya untuk hapus dan tidak untuk kembali',
        ...
        if (result.value == true) {
            $.ajax({
                type: "POST",
                url: "<?= base_url('WMS/Laporan/LaporanInformasiPallet/deletePalletDetail') ?>",
                data: {

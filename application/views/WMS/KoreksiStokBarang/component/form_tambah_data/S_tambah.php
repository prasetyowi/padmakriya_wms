<script>
    ...
    $("#no_koreksi_draft").on("change", function() {
        let id = $(this).val();
        requestAjax("<?= base_url('WMS/KoreksiStokBarang/getDataKoreksiDraftById'); ?>", {
            id: id
            ...
            if (header.attachment != null) {
                ...
                <label name="CAPTION-ATTACHMENT">File Attachment</label> <br>
				<a class="text-primary" href="<?= base_url('assets/images/uploads/KoreksiStokBarang/') ?>${header.attachment}" target="_blank">${header.attachment}</a>
                <input type="hidden" class="form-control" name="file" value="${header.attachment}" id="file" required readonly />
                </div>
                `)
            }
        ...
        // $.ajax({
        // 	url: "<?= base_url('WMS/KoreksiStokBarang/getDataKoreksiDraftById'); ?>",
        // 	type: "POST",
        // 	data: {
        // 		id: id
        // 	},
        ...
        if (data.length != 0) {
            ...
            <tr>
                ...
                <td class="text-center">
                    <button type="button" data-toggle="tooltip" data-placement="top" title="Pilih Pallet" data-id="${v.id}" data-gudang="${gudang}" data-gudang-id="${gudang_id}" data-qty-plan="${v.qty_plan}" data-ed="${v.ed}" data-sku-kode="${v.sku_kode}" data-sku-nama="${v.sku_nama}" data-qty-aktual="${qyt_aktual}" class="btn btn-info pilihpallet" ><i class="fas fa-hand-point-up"></i></button> 
                </td>
            </tr>
        `);
            });
    ...
    function get_data(id, gudang_id, ed, qty_plan, sku_kode, gudang) {
        ...
        $.ajax({
            url: "<?= base_url('WMS/KoreksiStokBarang/getDataPalletBySkuStockId'); ?>",
            type: "POST",
            data: {
                id: id,
                gudang_id: gudang_id,
                ...
                if (response.length > 0) {
                    ...
                    $.each(response, function(i, v) {
                        ...
                        if (v.scan == null) {
                            ...
                            action +=
                                `<button type='button' class='form-control btn btn-danger batalkan_pilih_pallet' name='batalkan_pilih_pallet' id='batalkan_pilih_pallet' title="batalkan pilih pallet" data-id="${id}" data-qty-plan="${qty_plan}" data-gudang-id="${gudang_id}" data-ed="${ed}" data-sku-kode="${sku_kode}" data-id-temp="${v.id_temp}" data-pallet-id="${v.pallet_id}" data-sku-stock-id="${v.sku_stock_id}"><i class="fas fa-xmark"></i></button>`;
						...
						<tr class="text-center" style="${style}">
							...
							<td>${v.qty}</td>
							<td>
								<input type='text' class='form-control numeric  UpdateQtyAmbil' ${is_scan} name='qty_ambil[]' id='qty_ambil[]' data-id-temp="${v.id_temp}" data-pallet-id="${v.pallet_id}" data-qty-plan="${qty_plan}" data-qty="${v.qty}" data-id="${id}" data-sku-stock-id="${v.sku_stock_id}" data-ed="${ed}" data-sku-kode="${sku_kode}" data-index="${i}" data-tipe="${tipe_transaksi}" value='${is_value}'/>
							</td>
							<td>${is_lock}</td>
							<td>${action}</td>
						</tr>
    ...
    $(document).on("change", ".UpdateQtyAmbil", function() {
        ...
        var tipe_transaksi = $(this).attr('data-tipe');
        ...
        if (qty_ambil <= qty_plan) {
            if (tipe_transaksi != 'Koreksi Masuk') {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('#table_data_tambah_koreksi_stok').fadeOut("slow", function() {
                                ...
                                $(`#qtyAktualKoreksi_${gudang_id}_${ed}_${sku_kode}`).text(
                                    new_qty_)
                            });
                        ...
                        success: function(data) {
                            ...
                            if (data.type == 200) {
                                ...
                                $('#table_list_data_pilih_pallet').fadeOut("slow",
                                    ...
                                    get_data(id, gudang_id, ed, qty_plan, sku_kode,
                                        gudang);
                                });
                            }
                            if (data.type == 201) {
    ...
    $(document).on("click", "#simpan_koreksi", function() {
        ...
        let no_referensi_dokumen = $("#no_referensi_dokumen").val();
        let file = $("#file").val();
        ...
        if (is_zero == count) {
            ...
            messageBoxBeforeRequest('Pastikan data yang sudah anda input benar!', 'Iya, Simpan', 'Tidak, Tutup')
                .then((result) => {
                    if (result.value == true) {
                        requestAjax("<?= base_url('WMS/KoreksiStokBarang/save_data'); ?>", {
                            koreksi_draft_id: koreksi_draft_id,
                            ...
                            no_referensi_dokumen: no_referensi_dokumen,
                            file: file,
                            lastUpdated
                        }, "POST", "JSON", function(response) {
                            if (response == 1) {
                    ...
                    if (result.value == true) {
                        requestAjax("<?= base_url('WMS/KoreksiStokBarang/confirm_data'); ?>", {
                            ...
                            if (response == 1) {
                                ...
                                setTimeout(() => {
                                    location.href =
                                        "<?= base_url('WMS/KoreksiStokBarang/KoreksiStokBarangMenu') ?>";
                                }, 500);
                            }

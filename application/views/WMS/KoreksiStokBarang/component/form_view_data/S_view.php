<script>
	...
	$(document).ready(function() {
		...
		$.ajax({
			url: "<?= base_url('WMS/KoreksiStokBarang/getDataKoreksiViewById'); ?>",
			type: "POST",
			data: {
				id: global_id
			},
			...
			success: function(response) {
				...
				$("#koreksi_stok_id_view").val(header.id);
	...
	function get_data(id, gudang_id, ed) {
		...
		$.ajax({
			url: "<?= base_url('WMS/KoreksiStokBarang/getDataPalletBySkuStockId_view'); ?>",
			type: "POST",
			data: {
				id: id,
				gudang_id: gudang_id,
          ...
          `);
					...
					$("#table_list_data_pilih_pallet_view > tbody").html(`<tr><td colspan="5" class="text-center text-danger">Data Kosong</td></tr>`);
				}

						...
						for (var index = 0; index < arr_sku.length; index++) {
							if (arr_sku[index] != "") {
								arr_detail.push({
									...
									'sku_qty_kirim': $("#item-" + index + "-DeliveryOrderDetail-sku_qty_kirim").val(),
								...
								$.ajax({
									...
									success: function(data) {
										if (data == 1) {
											...
											setTimeout(() => {
												location.href = "<?= base_url(); ?>WMS/SuratTugasPengiriman/SettlementMenu/?delivery_order_batch_id=" + $('#DeliveryOrder-delivery_order_batch_id').val();
												// location.reload();
											}, 500);
										} else {

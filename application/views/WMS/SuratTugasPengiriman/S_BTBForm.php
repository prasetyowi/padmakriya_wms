<script type="text/javascript">
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response == 0) {
					...
					setTimeout(() => {
						window.location.href = "<?php echo base_url() ?>WMS/SuratTugasPengiriman/ClosingPengirimanMenu/?delivery_order_batch_id=" + delivery_order_batch_id;
					}, 1000);
				} else {
					$.ajax({
						type: 'POST',
	...
	function ChBTBFormMenu(JSONChannel) {
		...
		if (Channel.GudangPenerima != 0) {
			for (i = 0; i < Channel.GudangPenerima.length; i++) {
				...
				$("#filter_gudang_penerima_do_retur").append('<option value="' + depo_detail_id + '">' + depo_detail_nama + '</option>');
				$("#filter_gudang_penerima_terkirim_sebagian").append('<option value="' + depo_detail_id + '">' + depo_detail_nama + '</option>');
				$("#filter_gudang_penerima_gagal_kirim").append('<option value="' + depo_detail_id + '">' + depo_detail_nama + '</option>');
				$("#filter_gudang_penerima_barang_titipan").append('<option value="' + depo_detail_id + '">' + depo_detail_nama + '</option>');
					...
					if (response) {
						...
						if (response.Reason != 0) {
							for (x = 0; x < response.Reason.length; x++) {
								...
								$("#slc_reason" + i).append('<option value="' + reason_id + '">' + reason_keterangan + '</option>');
							}
						}
				...
				if (response) {
					...
					if (response.Reason != 0) {
						for (x = 0; x < response.Reason.length; x++) {
							...
							$('[name="slcreason"]').append('<option value="' + reason_id + '">' + reason_keterangan + '</option>');
						}
					}
	...
	function ChTerkirimSebagianMenu(JSONChannel) {
		...
		if (Channel.DeliveryOrder != 0) {
			...
			for (i = 0; i < Channel.DeliveryOrder.length; i++) {
				...
				strmenu = strmenu + '	<td><input type="hidden" id="delivery_order_detail_id' + i + '" name="delivery_order_detail_id' + i + '" value="' + delivery_order_detail_id + '"></td>';
				strmenu = strmenu + '	<td style="width:20%;">' + sku_nama_produk + '</td>';
				strmenu = strmenu + '	<td>' + sku_kemasan + '</td>';
				strmenu = strmenu + '	<td>' + sku_satuan + '</td>';
				strmenu = strmenu + '	<td>' + Math.round(sku_qty) + '</td>';
				strmenu = strmenu + '	<td><input type="text" style="width:70%;" class="form-control" id="txt_qty_kirim' + i + '" name="txt_qty_kirim' + i + '" value="0"></td>';
				strmenu = strmenu + '	<td style="width:20%;"><select id="slc_reason_detail' + i + '" class="form-control" style="width:100%"></select></td>';
				strmenu = strmenu + '</tr>';
			...
			for (i = 0; i < Channel.DeliveryOrder.length; i++) {
				...
				if (Channel.Reason != 0) {
					for (x = 0; x < Channel.Reason.length; x++) {
						...
						$("#slc_reason_detail" + i).append('<option value="' + reason_id + '">' + reason_keterangan + '</option>');
					}
				}
		...
		if (Channel.Pallet != 0) {
			...
			for (i = 0; i < Channel.PalletDetail.length; i++) {
				...
				if (penerimaan_tipe_nama == "titipan") {
					strmenu = strmenu + '	<td style="width:20%;"><input type="date" class="form-control" id="slcskuexpireddate' + i + '" value="' + sku_stock_expired_date + '" onchange="UpdateSkuExpDatePallet(\'' + pallet_detail_id + '\',\'' + sku_id + '\',this.value,\'' + depo_detail_id + '\')"></td>';
				} else {
					strmenu = strmenu + '	<td style="width:20%;"><select class="form-control" id="slcskuexpireddate' + i + '" onchange="UpdateSkuExpDatePallet(\'' + pallet_detail_id + '\',\'' + sku_id + '\',this.value,\'' + depo_detail_id + '\')"></select></td>';
				}
				strmenu = strmenu + '	<td style="width:15%;"><input type="text" style="width:100%;" class="form-control" id="jumlah_barang' + i + '" value="' + sku_stock_qty + '" onchange="UpdateQtySKUPallet(\'' + delivery_order_batch_id + '\',\'' + pallet_detail_id + '\',\'' + sku_id + '\',this.value,\'' + penerimaan_tipe_nama + '\', ' + i + ')"></td>';
				strmenu = strmenu + '	<td>' + penerimaan_tipe_nama + '</td>';
				strmenu = strmenu + '	<td><button type="button" class="btn btn-sm btn-danger" onclick="DeletePalletDetail(\'' + pallet_id + '\',\'' + pallet_detail_id + '\')"><i class="fa fa-trash"></i></button></td>';
				strmenu = strmenu + '</tr>';
							...
							success: function(response) {
								if (response) {
									if (response.ExpiredDate != 0) {
										$("#slcskuexpireddate" + j).append('<option value="">** Pilih SKU Exp **</option>');
										for (x = 0; x < response.ExpiredDate.length; x++) {
											...
											sku_stock_expired_date_slc = response.ExpiredDate[x].sku_stock_expired_date;
											...
											if (sku_stock_id_slc == sku_stock_id_j) {
												$("#slcskuexpireddate" + j).append('<option value="' + sku_stock_expired_date_slc + '" selected>' + sku_stock_expired_date_slc + '</option>');
												UpdateSkuExpDatePallet(pallet_detail_id_j, sku_id_j, sku_stock_expired_date_slc, depo_detail_id);
											} else {
												$("#slcskuexpireddate" + j).append('<option value="' + sku_stock_expired_date_slc + '">' + sku_stock_expired_date_slc + '</option>');
											}
										}
									}
								}
	...
	function ChDataTableSKU(JSONChannel) {
		...
		if (Channel.sku != 0) {
			...
			for (i = 0; i < Channel.sku.length; i++) {
				...
				strmenu = strmenu + '<tr>';
				strmenu = strmenu + '	<td><input type="checkbox" name="CheckboxSKU" id="chk_sku_' + i + '" value="' + sku_id + ' || ' + sku_nama_produk + '"></td>';
				strmenu = strmenu + '	<td><input type="hidden" id="principle' + i + '" value="' + principle + '">' + principle + '</td>';
				strmenu = strmenu + '	<td><input type="hidden" id="brand' + i + '" value="' + brand + '">' + brand + '</td>';
				strmenu = strmenu + '	<td><input type="hidden" id="sku_induk_nama' + i + '" value="' + sku_induk_nama + '">' + sku_induk_nama + '</td>';
				strmenu = strmenu + '	<td><input type="hidden" id="sku_kode' + i + '" value="' + sku_kode + '">' + sku_kode + '</td>';
				strmenu = strmenu + '	<td style="width:20%;"><input type="hidden" id="sku_nama_produk' + i + '" value="' + sku_nama_produk + '">' + sku_nama_produk + '</td>';
				strmenu = strmenu + '	<td><input type="hidden" id="sku_kemasan' + i + '" value="' + sku_kemasan + '">' + sku_kemasan + '</td>';
				strmenu = strmenu + '	<td><input type="hidden" id="sku_satuan' + i + '" value="' + sku_satuan + '">' + sku_satuan + '</td>';
				strmenu = strmenu + '</tr>';
	...
	function GetPrincipleByPerusahaan(perusahaan) {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$("#filter_principle").append('<option value=""><span name="CAPTION-PILIH">** Pilih **</span></option>');
	...
	function GetCheckerByPrinciple(principle) {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$("#filter_checker").append('<option value=""><span name="CAPTION-PILIH">** Pilih **</span></option>');

				$.each(response, function(i, v) {
					$("#filter_checker").append(`<option value="${v.karyawan_id}">${v.karyawan_nama}</option>`);
				});
			}
		});
	}
	...
	document.getElementById('kode_barcode').addEventListener('keyup', function() {
		...
		if (this.value != "") {
			fetch('<?= base_url('WMS/SuratTugasPengiriman/getKodeAutoComplete?params='); ?>' + this.value)

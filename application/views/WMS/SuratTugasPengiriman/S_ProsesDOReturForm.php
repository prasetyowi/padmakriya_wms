<script type="text/javascript">
                    ...
                    $.each(response, function(i, v) {
                        $("#table-sku-delivery-only > tbody").append(`
							<tr id="row-${i}">
								...
								<td class="text-center">
                                    <button class="btn btn-primary btn-small" onclick="ViewSKUStock('${v.sku_id}','${v.sku_kode}','${v.sku_nama_produk}','${i}','${v.sku_konversi_faktor}')"><i class="fa fa-plus"></i></button>
            ...
            if (cek_customer > 0) {
                ...
                if (arr_sku.length > 0) {
                    $("#table-sku-delivery-only > tbody tr").each(function() {
                        var is_Qty = $(this).find("td:eq(9) input[type='number']");
    ...
    function ViewSKUStock(sku_id, sku_kode, sku_nama_produk, idx, sku_konversi_faktor) {
        $("#modal-do-detail2").modal('show');

        var sku_qty = $("#item-" + idx + "-DeliveryOrderDetail-sku_qty").val();
                            ...
                            $.each(response, function(i, v) {
                                $.ajax({
                                    ...
                                    success: function(response2) {
                                        ...
                                        $("#item-" + i + "-sku_stock_expired_date-do_detail2").html('');
                                        $("#item-" + i + "-sku_stock_expired_date-do_detail2").append(`<option value="">** <span name="CAPTION-PILIH">Pilih</span> **</option>`);

                                        if (response2.length > 0) {
                                            $.each(response2, function(i2, v2) {
                                                $("#item-" + i + "-sku_stock_expired_date-do_detail2").append(`<option value="${v2.sku_stock_id}" ${v2.sku_stock_id == v.sku_stock_id ? 'selected' : ''}>${v2.sku_stock_expired_date + " | " + v2.depo_detail_nama + " | " + v2.sku_stock_batch_no}</option>`);
                                            });
                                        }

<script type="text/javascript">
	...
	function GetDeliveryOrderBatchByID() {
		...
		requestAjax("<?= base_url('WMS/KonfirmasiPenerimaanTunai/GetDeliveryOrderBatchByID') ?>", {
			id: id
	...
	function GetListPembayaranByID() {
		var id = $('#id_do_batch').val();

		requestAjax("<?= base_url('WMS/KonfirmasiPenerimaanTunai/GetListPembayaranByID') ?>", {
			id: id
		}, "POST", "JSON", function(response) {
			if (response.list_pembayaran != 0) {
				$(response.list_pembayaran).each(function(i, v) {
					$("#table_list_pembayaran > tbody").append(`
							<tr>
								<td class="text-center">${i+1}</td>
								<td class="text-center">
									<select id="DeliveryOrderPayment-tipe_pembayaran_id${i}" name="DeliveryOrderPayment[tipe_pembayaran_id${i}]" disabled class="form-control" style="width:100%" onChange="handleOnChange('${i}', this)">
										<option value="${v.tipe_pembayaran_id}" selected>${v.tipe_pembayaran_nama}</option>
									</select>
								</td>
								<td class="text-center">
									<input ${v.tipe_pembayaran_id == '0' || v.tipe_pembayaran_id == '' ? `disabled`: ``} type="date" class="form-control" id="DeliveryOrderPayment-delivery_order_payment_tgl_jatuh_tempo${i}" name="DeliveryOrderPayment[delivery_order_payment_tgl_jatuh_tempo${i}]" autocomplete="off" value="" onChange="UpdateListPembayaran('${i}')" />
								</td>
								<td class="text-center">
									<input disabled type="text" class="form-control" onkeyup="formatNominal(this)" autocomplete="off" value="${formatNumber(parseFloat(v.delivery_order_jumlah_bayar))}"/>
								</td>
								<td class="text-center">
									<input ${v.tipe_pembayaran_id == '1' ? `disabled`: ``} type="text" class="form-control" onkeyup="formatNominal(this)" id="DeliveryOrderPayment-delivery_order_payment_value${i}" name="DeliveryOrderPayment[delivery_order_payment_value${i}]" autocomplete="off" value="${v.payment_value != null ? formatNumber(parseFloat(v.payment_value)) : ''}" onChange="UpdateListPembayaran('${i}')" />
								</td>
								<td class="text-center">
									<button type="button" class="btn btn-sm btn-primary" onClick="ViewModalPembayaranDetail2('${i}')"><i class="fa fa-eye"></i></button>
								</td>
							</tr>
					`);
				})
			...
			if (response.List_Keterangan != 0) {
				$.each(response.List_Keterangan, function(i, v) {
					arr_keterangan.push({
						do_id: v.delivery_order_id,
						tipe_pembayaran_id: v.tipe_pembayaran_id,
						keterangan: v.delivery_order_payment_driver_keterangan
					})
				});
			}
		})
	...
	function GetListDOByID() {
		...
		requestAjax("<?= base_url('WMS/KonfirmasiPenerimaanTunai/GetListDOByID') ?>", {
			id: id
		}, "POST", "JSON", function(response) {
			if (response.List_DO != 0) {
				...
				$.each(response.List_DO, function(i, v) {
					...
					<td class="text-center" style="vertical-align: middle;">${v.delivery_order_tgl_buat_do}</td>
					<td class="text-center" style="vertical-align: middle;">${v.delivery_order_tgl_rencana_kirim}</td>
					<td class="text-center" style="vertical-align: middle;">${v.delivery_order_kode}</td>
					<td class="text-center" style="vertical-align: middle;">${v.no_so}</td>
					<td class="text-center" style="vertical-align: middle;">${v.no_so_eksternal}</td>
					<td class="text-center" style="vertical-align: middle;">${v.delivery_order_kirim_nama}</td>
					<td class="text-center" style="vertical-align: middle;">${v.delivery_order_kirim_alamat}</td>
					<td class="text-center" style="vertical-align: middle;">${v.tipe_delivery_order_alias}</td>
					<td class="text-center" style="vertical-align: middle;">${v.delivery_order_tipe_pembayaran}</td>
					<td class="text-center" style="vertical-align: middle;">Rp. ${formatNumber(parseFloat(v.delivery_order_jumlah_bayar))}</td>
					</tr>
					`);
				...
				$('#table_list_do > tbody').append(`
					<tr style="vertical-align: middle; background-color: #4942E4">
                        <td colspan="10" class="text-center" style="color: white;">GRAND TOTAL</td>
                        <td class="text-center" style="color: white">Rp. ${formatNumber(parseFloat(grandTotal))}</td>
					</tr>
					`);
	...
	//             $.ajax({
	//                 type: 'POST',
	//                 url: "<?= base_url() ?>WMS/KonfirmasiPenerimaanTunai/GetTipeBiaya",
	//                 dataType: "JSON",
	...
	//             $.ajax({
	//                 type: 'POST',
	//                 url: "<?= base_url() ?>WMS/KonfirmasiPenerimaanTunai/GetDODetail3",
	//                 data: {
	//                     do_id: do_id
	//                 },
	//                 dataType: "JSON",
	...
	//             $.ajax({
	//                 type: 'post',
	//                 url: "<?= base_url() ?>WMS/KonfirmasiPenerimaanTunai/deleteDODetail3ByID",
	//                 data: {
	//                     id: arr_del_biaya[0]
	//                 },
	//                 dataType: 'JSON',
	...
	// 				requestAjax("<?= base_url() ?>WMS/KonfirmasiPenerimaanTunai/saveAllDO", {
	...
	// 						setTimeout(() => {
	// 							location.href =
	// 								"<?= base_url() ?>WMS/KonfirmasiPenerimaanTunai/KonfirmasiPenerimaanTunaiMenu";
	// 						}, 500);
	...
	$('#btn_simpan_all').click(function() {
		...
		$('#table_list_pembayaran > tbody tr').each(function() {
			var tipe_pembayaran = $(this).find("td:eq(1) select").val();
			var tgl_jatuh_tempo = $(this).find("td:eq(2) input").val();
			var nilai_bayar = $(this).find("td:eq(4) input").val().replaceAll(",", '');

			if (tipe_pembayaran == "0") {
				if (nilai_bayar == "" || nilai_bayar == "0") {
					message("Error!",
						"Jumlah Terima Tunai tidak boleh 0, silahkan isi jumlah terima terlebih dahulu",
						"error");
					boolean = false;
				}
			}
	...
	function GetSuratTugasPengirimanMenu() {
		requestAjax("<?= base_url('WMS/KonfirmasiPenerimaanTunai/GetSuratTugasPengirimanMenu') ?>", {}, "POST", "JSON",
			function(response) {
				if (response) {
	...
	$('#btnviewfdjr').click(function() {
		...
		requestAjax("<?= base_url('WMS/KonfirmasiPenerimaanTunai/GetSuratTugasPengirimanTable') ?>", {
			Tgl_FDJR: Tgl_FDJR,
			No_FDJR: No_FDJR,
			...
			for (let i = 0; i < Channel.SuratTugasPengiriman.length; i++) {
				...
				if (arr[delivery_order_batch_kode]) {
					if (cek_do_batch_kode == delivery_order_batch_kode) {
						...
						// } else {
						// strmenu +=
						//     '	<td><a href="<?php echo base_url(); ?>WMS/KonfirmasiPenerimaanTunai/ClosingPenerimaan/?id=' +
						...
						if (delivery_order_batch_status == 'in transit' || delivery_order_batch_status ==
							...
							strmenu +=
								'	<td style="vertical-align: middle; text-align: center;" rowspan="' + arr[
									delivery_order_batch_kode] +
								'"><a href="<?php echo base_url(); ?>WMS/KonfirmasiPenerimaanTunai/ClosingPenerimaan/?id=' +
								delivery_order_batch_id + '" class="btn btn-warning">Closing Penerimaan</a></td>';
						} else if (delivery_order_batch_status == 'completed') {
							strmenu +=
								'	<td style="vertical-align: middle; text-align: center;" rowspan="' + arr[
									delivery_order_batch_kode] +
								'"><a href="<?php echo base_url(); ?>WMS/KonfirmasiPenerimaanTunai/ViewClosingPenerimaan/?id=' +
								delivery_order_batch_id + '" class="btn btn-info">View Penerimaan</a></td>';
						} else {
							strmenu +=
								'	<td style="vertical-align: middle; text-align: center;" rowspan="' + arr[
									delivery_order_batch_kode] +
							...
							//     '	<td style="vertical-align: middle; text-align: center;" rowspan="' + arr[
							//         delivery_order_batch_kode] +
							//     '" hidden><a href="<?php echo base_url(); ?>WMS/KonfirmasiPenerimaanTunai/ClosingPenerimaan/?id=' +
							//     delivery_order_batch_id + '" class="btn btn-success">View Closing Penerimaan</a></td>';
						}
						// }
						strmenu += '</tr>';
					}
	...
	function GetListPembayaran() {
			...
			$.each(arr_pembayaran, function(i, v) {
				$("#table_list_pembayaran > tbody").append(`
					<tr>
						...
						<td class="text-center">
							<input type="text" class="form-control" id="DeliveryOrderPayment-delivery_order_payment_value${i}" name="DeliveryOrderPayment[delivery_order_payment_value${i}]" autocomplete="off" value="${v.delivery_order_payment_value}" onChange="UpdateListPembayaran('${i}')" />
						</td>
						<td class="text-center">
							<button type="button" class="btn btn-sm btn-primary" onClick="ViewModalPembayaranDetail('${i}')"><i class="fa fa-add"></i></button>
							<button type="button" class="btn btn-sm btn-danger btnDeleteView"  onClick="DeletePembayaran('${i}')"><i class="fa fa-trash"></i></button>
						</td>
					</tr>
				`);
		...
		if ('<?= $act ?>' == 'view') {
			$("#btn_simpan_all").css('display', 'none')
			$("#table_list_pembayaran input").prop('disabled', true);
			$("#table_list_pembayaran select").prop('disabled', true);
			$('.btnDeleteView').css('display', 'none');
		}
	...
	function ViewModalPembayaranDetail(idx) {
		...
		requestAjax("<?= base_url('WMS/KonfirmasiPenerimaanTunai/Get_list_delivery_order') ?>", {
			id: id,
			...
			if ($.fn.dataTable.isDataTable('#table_list_pembayaran_detail')) {
				$('#table_list_pembayaran_detail').DataTable().destroy();
			}
			...
			if (response != 0) {
				...
				$.each(response, function(i, v) {
                    ...
                    <tr>
                        <td class="text-center" style="vertical-align: middle;">
                            <input type="checkbox" class="cbView" name="CheckboxDO" id="check-do-${i}" value="${v.delivery_order_id}" ${v.checked != '0' && $("#DeliveryOrderPayment-delivery_order_reff_no" + idx).val() == v.delivery_order_reff_no  ? 'checked' : ''}>
                        </td>
                        <td class="text-center" style="vertical-align: middle;">${v.delivery_order_kode}</td>
                        <td class="text-center" style="vertical-align: middle;">${v.sales_order_kode}</td>
                        <td class="text-center" style="vertical-align: middle;">${v.sales_order_no_po}</td>
                        <td class="text-center" style="vertical-align: middle;">${v.client_pt_nama}</td>
                        <td class="text-center" style="vertical-align: middle;">${v.tipe_pembayaran_nama}</td>
                        <td class="text-center" style="vertical-align: middle;">${v.tipe_delivery_order_alias}</td>
                        <td class="text-center" style="vertical-align: middle;">Rp. ${formatNumber(parseFloat(v.delivery_order_jumlah_bayar))}</td>
                    </tr>
                `);
				...
				$("#table_list_pembayaran_detail").DataTable({
					'lengthMenu': [
						[-1],
						["All"]
					]
				...
				if ('<?= $act ?>' == 'view') {
					$("#btn_update_list_pembyaran_detail").css('display', 'none')
					$('.btnDeleteView').css('display', 'none');
					$('#table_list_pembayaran_detail .cbView').prop('disabled', true);
				}
			}
		})
	}

	function ViewModalPembayaranDetail2(idx) {
		console.log(arr_keterangan);

		var id = $('#id_do_batch').val();
		var tipe_pembayaran_id = $("#DeliveryOrderPayment-tipe_pembayaran_id" + idx + " option:selected").val();

		$('#btn_update_list_pembyaran_detail').attr('tipe_pembayaran_id', tipe_pembayaran_id);
		...
		requestAjax("<?= base_url('WMS/KonfirmasiPenerimaanTunai/GetListDODetail') ?>", {
			id: id,
			tipe_pembayaran: tipe_pembayaran_id
		}, "POST", "JSON", function(response) {
			if ($.fn.dataTable.isDataTable('#table_list_pembayaran_detail')) {
				$('#table_list_pembayaran_detail').DataTable().destroy();
			}
			...
			if (response != 0) {
				$.each(response, function(i, v) {
					var foundItem = arr_keterangan.find(function(item) {
						if (item.do_id == v.delivery_order_id && item.tipe_pembayaran_id == parseFloat(v.delivery_order_tipe_pembayaran)) {
					...
					$('#table_list_pembayaran_detail > tbody').append(`
						<tr>
							...
							<td class="text-center" style="vertical-align: middle;">${v.tipe_delivery_order_alias}</td>
							<td class="text-center" style="vertical-align: middle;">Rp. ${formatNumber(parseFloat(v.delivery_order_jumlah_bayar))}</td>
							<td class="text-center" style="vertical-align: middle;"><input type="text" class="form-control" id="keterangan" data-do-id="${v.delivery_order_id}" name="keterangan" autocomplete="off" value="${foundItem != undefined ? foundItem['keterangan'] : ''}" /></td>
						</tr>
					`);
				});
	...
	function simpanKeterangan(event) {
		var tipe_pembayaran_id = $(event).attr('tipe_pembayaran_id');

		// if (arr_keterangan.length != 0) {
		// 	for (let i = 0; i < arr_keterangan.length; i++) {
		// 		if (arr_keterangan[i].tipe_pembayaran_id == tipe_pembayaran_id) {
		// 			arr_keterangan.splice(i, 1);
		// 			i--;
		// 		}
		// 	}
		// }

		$('#table_list_pembayaran_detail tbody tr').each(function() {
			var keterangan = $(this).find("td:eq(6) input[id='keterangan']").val();
			var do_id = $(this).find("td:eq(6) input[id='keterangan']").attr('data-do-id');

			if (keterangan != '') {
				// if (arr_keterangan.length != 0) {
				// 	for (let i = 0; i < arr_keterangan.length; i++) {
				// 		if (arr_keterangan[i].do_id == do_id) {
				// 			arr_keterangan.splice(i, 1);
				// 			i--;
				// 		}
				// 	}
				...
				var index = arr_keterangan.findIndex(function(item) {
					return item.do_id == do_id && item.tipe_pembayaran_id == tipe_pembayaran_id
				});

				if (index != -1) {
					arr_keterangan[index].keterangan = keterangan
				}
			}
		});

			...
			<div class="panel-body form-horizontal form-label-left">
				<div class="row">
					<table id="tableskumenu" width="100%" class="table table-striped">
						...
						<tbody>
							<?php foreach ($SettlementDetail as $i => $row) : ?>
								<tr>
									<td class="text-center"><?= $row['idx']; ?></td>
									<td class="text-center"><?= $row['tgl']; ?></td>
									<td class="text-center">
										<?php if ($row['documentjenis'] == "BKB") { ?>
											<button type="button" id="btn_document_no<?= $i ?>" class="btn btn-link" onclick="ViewDetailBKB('<?= $row['documentno']; ?>','<?= $row['skuid']; ?>')"><?= $row['documentno']; ?></button>
										<?php } else if ($row['documentjenis'] == "BTB") { ?>
											<button type="button" id="btn_document_no<?= $i ?>" class="btn btn-link" onclick="ViewDetailBTB('<?= $row['documentno']; ?>','<?= $row['skuid']; ?>')"><?= $row['documentno']; ?></button>
										<?php } else if ($row['documentjenis'] == "DO") { ?>
											<button type="button" id="btn_document_no<?= $i ?>" class="btn btn-link" onclick="ViewDetailDO('<?= $row['documentno']; ?>','<?= $row['skuid']; ?>')"><?= $row['documentno']; ?></button>
										<?php } else if ($row['documentjenis'] == "DOR") { ?>
											<button type="button" id="btn_document_no<?= $i ?>" class="btn btn-link" onclick="ViewDetailDOR('<?= $row['documentno']; ?>','<?= $row['skuid']; ?>')"><?= $row['documentno']; ?></button>
										<?php } ?>
									</td>
									<td class="text-center"><?= $row['documentjenis']; ?></td>
									<td class="text-center"><?= $row['documentstatus']; ?></td>
									<td class="text-center"><?= $row['qty']; ?></td>
									<td class="text-center"><?= $row['qtykumulatif']; ?></td>
				...
				<div class="row pull-right">
					...
					<!-- <a href="<?php echo base_url() ?>WMS/SettlementPengiriman/DeliveryOrderReturForm/?delivery_order_batch_id=<?= $delivery_order_batch_id; ?>" class="btn btn-primary"><span name="CAPTION-PROSESDORETUR">Proses DO Retur</span></a> -->
					<a href="<?php echo base_url() ?>WMS/SettlementPengiriman/SettlementMenu/?delivery_order_batch_id=<?php echo $delivery_order_batch_id; ?>" type="button" id="btn_back" class="btn btn-primary" style="display: ;"><span name="CAPTION-BACK">Kembali</span></a>
				</div>
			</div>
		</div>
	</div>
							...
							<div class="row">
								<div class="col-xs-3">
									<div class="form-group field-deliveryorderdraft-delivery_order_kode">
										<label class="control-label" for="deliveryorderdraft-delivery_order_kode" name="CAPTION-DONO">DO No</label>
							...
							<div class="row">
								...
								<div class=" col-xs-3">
									<div class="form-group field-deliveryorderdraft-delivery_order_tgl_surat_jalan">
										<label class="control-label" for="deliveryorderdraft-delivery_order_tgl_surat_jalan" name="Tanggal Surat Jalan">Tanggal Surat Jalan</label>

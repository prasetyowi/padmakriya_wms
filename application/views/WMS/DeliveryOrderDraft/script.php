<script type="text/javascript">
				...
				'columns': [{
						render: function(v) {
							if (v.delivery_order_draft_status == "Approved") {
								...
								<input type="hidden" id="item-${v.idx}-ListDODraft-delivery_order_draft_update_tgl" value="${v.delivery_order_draft_update_tgl}">
					...
					{
						// data: 'umur_so'
						data: 'delivery_order_draft_tipe_layanan'
					},
					{
						render: function(v) {
					...
					},
					{
						render: function(v) {
							...
							if (v.delivery_order_draft_status == "Approved") {
								...
								if (v.tipe_delivery_order_id == 'C42B0674-75B3-468F-B377-E4B23D9B8E12') {
									a += `<a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderDraft/dosplit/?id=${v.delivery_order_draft_id}" class="btn btn-warning btn-small"><i class="fa-solid fa-code-branch"></i><br>`;
								}
	...
	$(document).on("click", ".btn-choose-sku-multi", function() {
		...
		var no = 1;
		var act = '<?= $this->uri->segment(4) ?>';
		var do_reff_id = $("#deliveryorderdraft-delivery_order_draft_reff_id").val();
		...
		if (arr_checkbox_sku.length > 0) {
			...
			// arr_sku.push(...arr_sku_do_split.map(item => "'" + item + "'"));

			arr_sku = [];
			arr_sku = arr_checkbox_sku
			...
			$.ajax({
				...
				data: {
					// sku_id: arr_sku,
					sku_id: arr_checkbox_sku,
					do_draft_id: $("#deliveryorderdraft-delivery_order_draft_id").val(),
					act: act,
					do_reff_id: do_reff_id
				},
				dataType: "JSON",
				success: function(response) {
					$.each(response, function(i, v) {
						...
						var options2 = `<option value="">**Pilih**</option>`;

						<?php if (isset($Lokasi)) { ?>
							<?php for ($x = 0; $x < count($Lokasi); $x++) { ?>
								var selected = (v.tipe_stock == "<?= $Lokasi[$x]->nama ?>") ? 'selected' : '';
								options += `<option value="<?= $Lokasi[$x]->nama ?>" ${selected}><?= $Lokasi[$x]->nama ?></option>`;
							<?php } ?>
						<?php } ?>

						<?php if (isset($Bulan)) { ?>
							<?php for ($x = 0; $x < count($Bulan); $x++) { ?>
								var selected = (v.sku_filter_expdatebulan == "<?= $Bulan[$x] ?>") ? 'selected' : '';
								options2 += `<option value="<?= $Bulan[$x] ?>" ${selected}><?= $Bulan[$x] ?></option>`;
							<?php } ?>
						<?php } ?>

						if (act == 'dosplit' || do_reff_id != "") {
							var str = `<td class="text-center"><input onchange="changeQtySisa(this.value, ${i}, '${v.sku_qty}')" min="0" type="number" id="item-${i}-DeliveryOrderDetailDraft-sku_qty_ambil" class="form-control sku-qty-ambil" value="0" /></td>`
						} else {
							var str = ''
						}
							...
							<tr id="row-${i}">
								...
								<td class="text-center" style="width:10%">
									<select id="item-${i}-DeliveryOrderDetailDraft-sku_request_expdate" class="form-control sku-request-expdate" onchange="reqFilter(this.value,'${i}')">
										<option value="0" ${v.sku_request_expdate == '0' ? 'selected' : ''}>Tidak</option>
										<option value="1" ${v.sku_request_expdate == '1' ? 'selected' : ''}>Ya</option>
									</select>
								</td>
								<td class="text-center" style="width:10%">
									<select id="item-${i}-DeliveryOrderDetailDraft-sku_filter_expdate" class="form-control sku-filter-expdate" disabled>
										<option value="">**Pilih**</option>
										<option value=">=" ${v.sku_filter_expdate == '>=' ? 'selected' : ''}>=></option>
									</select>
								</td>
								<td class="text-center" style="width:10%">
								...
								<td class="text-center"><input type="text" id="item-${i}-DeliveryOrderDetailDraft-sku_keterangan" class="form-control sku-keterangan" value="${v.sku_keterangan ? v.sku_keterangan : ''}" /></td>
								<td class="text-center"><input ${act == 'dosplit' || do_reff_id != "" ? 'readonly' : ''} min="0" type="number" id="item-${i}-DeliveryOrderDetailDraft-sku_qty" class="form-control sku-qty" value="${v.sku_qty}" onchange="UpdateTotalHarga('${i}')"/></td>
								${str}
								<td class="text-center">
	...
	function changeQtySisa(value, index, qty_sisa) {
		if (value == "") {
			message("Error!", "Jumlah barang tidak boleh kosong!", "error");
			$(`#item-${index}-DeliveryOrderDetailDraft-sku_qty`).val(parseInt(qty_sisa));
			$(`#item-${index}-DeliveryOrderDetailDraft-sku_qty_ambil`).val(0);
			return false;
		} else if (parseInt(value) > parseInt(qty_sisa)) {
			message("Error!", "Jumlah barang tidak boleh melebihi Qty Sisa!", "error");
			$(`#item-${index}-DeliveryOrderDetailDraft-sku_qty`).val(parseInt(qty_sisa));
			$(`#item-${index}-DeliveryOrderDetailDraft-sku_qty_ambil`).val(0);
			return false;
		} else {
			$(`#item-${index}-DeliveryOrderDetailDraft-sku_qty`).val(parseInt(qty_sisa) - parseInt(value));
		}
			...
			$('#table-customer').DataTable({
				...
				initComplete: function() {
					...
					var input = parent_dt.find('.dataTables_filter input').unbind(),
						self = this.api(),
						$searchButton = $('<button class="btn btn-flat btn-success btn-sm mb-0 mr-0 ml-5 btn-search-dt">').html('<i class="fa fa-fw fa-search">').click(function() {
							self.search(input.val()).draw();
						}),
						$clearButton = $('<button class="btn btn-flat btn-warning btn-sm mb-0 mr-0 ml-5 btn-reset-dt">').html('<i class="fa fa-fw fa-recycle">').click(function() {
							input.val('');
							$searchButton.click();
						})
	...
	// 			$("#loadingsku").show();
	// 			// if (act == 'dosplit') {
	// 			// 	var table_sku_delivery_only = $("#table-sku-delivery-only tbody tr").length;
	// 			// 	if (table_sku_delivery_only > 0) {
	// 			// 		for (let i = 0; i < table_sku_delivery_only; i++) {
	// 			// 			var sku_id = $("#item-" + i + "-DeliveryOrderDetailDraft-sku_id").val()
	// 			// 			arr_sku_do_split.push(sku_id);
	// 			// 		}
	// 			// 	}
	// 			// }
	// 			$.ajax({
	...
	// 					sku_kemasan: sku_kemasan,
	// 					sku_satuan: sku_satuan,
	// 					arr_sku_do_split: arr_sku_do_split,
	// 					dod_id: $("#deliveryorderdraft-delivery_order_draft_id").val(),
	// 					dod_reff_id: $("#deliveryorderdraft-delivery_order_draft_reff_id").val(),
	// 					act: act
	// 				},
	// 				dataType: "JSON",
	// 				async: false,
	...
	function initDataSKU() {
		...
		if (client_pt != "") {
			...
			$("#table-sku-delivery-only > tbody tr").each(function() {
				var sku_id = $(this).find("td:eq(0) input[type='hidden'].sku-id").val();
			...
			$('#table-sku').DataTable({
				...
				'ajax': {
					...
					dataType: "json",
					data: function(data) {
						data.client_pt = client_pt,
							data.perusahaan = perusahaan,
							data.tipe_pembayaran = tipe_pembayaran,
							data.brand = brand,
							data.principle = principle,
							data.sku_induk = sku_induk,
							data.sku_nama_produk = sku_nama_produk
						data.sku_kemasan = sku_kemasan
						data.sku_satuan = sku_satuan
						data.arr_sku_do_split = arr_sku_do_split
						data.dod_id = $("#deliveryorderdraft-delivery_order_draft_id").val()
						data.dod_reff_id = $("#deliveryorderdraft-delivery_order_draft_reff_id").val()
						data.act = act
					},
					beforeSend: function() {
						Swal.fire({
				...
				'columns': [{
						render: function(data, type, row, meta) {
							var str = '';
							str += `<input type="checkbox" onchange="addToArraySKU(this)" name="CheckboxSKU" id="check-sku-${row.idx}" value="${row.sku_id}">`
				...
				initComplete: function() {
					parent_dt = $('#table-sku').closest('.dataTables_wrapper')
					parent_dt.find('.dataTables_filter').css('width', 'auto')
					var input = parent_dt.find('.dataTables_filter input').unbind(),
						self = this.api(),
	...
	function addToArraySKU(checkbox) {

		if (checkbox.checked) {
			arr_checkbox_sku.push(checkbox.value);
		} else {
			arr_checkbox_sku = arr_checkbox_sku.filter(sku_id => sku_id != checkbox.value)
		}
							...
							if (cek_qty == 0) {
								...
								for (var index = 0; index < arr_sku.length; index++) {
									if (arr_sku[index] != "") {
										arr_detail.push({
											...
											'sku_filter_expdate': $("#item-" + index + "-DeliveryOrderDetailDraft-sku_filter_expdate").val(),
											'sku_tipe_stock': $("#item-" + index + "-DeliveryOrderDetailDraft-sku_tipe_stock").val(),
											'sku_filter_expdatebulan': $("#item-" + index + "-DeliveryOrderDetailDraft-sku_filter_expdatebulan").val(),
								...
								Swal.fire({
									...
									if (result.value == true) {
										...
										form_data.append('delivery_order_draft_input_pembayaran_tunai', $("#input_tunai").val());
										form_data.append('sales_order_id', $("#deliveryorderdraft-sales_order_id").val());
										form_data.append('delivery_order_draft_kode', "");
										form_data.append('delivery_order_draft_yourref', "");
										form_data.append('client_wms_id', $("#deliveryorderdraft-client_wms_id").val());
										form_data.append('delivery_order_draft_tgl_buat_do', $("#deliveryorderdraft-delivery_order_draft_tgl_buat_do").val());
										form_data.append('delivery_order_draft_tgl_expired_do', $("#deliveryorderdraft-delivery_order_draft_tgl_expired_do").val());
										form_data.append('delivery_order_draft_tgl_surat_jalan', $("#deliveryorderdraft-delivery_order_draft_tgl_surat_jalan").val());
										form_data.append('delivery_order_draft_tgl_rencana_kirim', $("#deliveryorderdraft-delivery_order_draft_tgl_rencana_kirim").val());
										form_data.append('delivery_order_draft_tgl_aktual_kirim', "");
										form_data.append('delivery_order_draft_keterangan', $("#deliveryorderdraft-delivery_order_draft_keterangan").val());
										...
										form_data.append('delivery_order_draft_tipe_layanan', document.querySelector('input[id="deliveryorderdraft-delivery_order_draft_tipe_layanan"]:checked').value);
										form_data.append('delivery_order_draft_tipe_pembayaran', document.querySelector('input[id="deliveryorderdraft-delivery_order_draft_tipe_pembayaran"]:checked').value);
										...
										form_data.append('delivery_order_draft_reff_id', $("#deliveryorderdraft-delivery_order_draft_reff_id").val());
										form_data.append('delivery_order_draft_reff_no', $("#deliveryorderdraft-delivery_order_draft_reff_no").val());
										form_data.append('delivery_order_draft_total', "");
										form_data.append('unit_mandiri_id', "<?= $this->session->userdata('unit_mandiri_id') ?>");
										form_data.append('depo_id', "<?= $this->session->userdata('depo_id') ?>");
										form_data.append('client_pt_id', $("#deliveryorderdraft-client_pt_id").val());
										form_data.append('delivery_order_draft_kirim_nama', $("#deliveryorderdraft-delivery_order_draft_kirim_nama").val());
										form_data.append('delivery_order_draft_kirim_alamat', $("#deliveryorderdraft-delivery_order_draft_kirim_alamat").val());
										form_data.append('delivery_order_draft_kirim_telp', $("#deliveryorderdraft-delivery_order_draft_kirim_telepon").val());
										form_data.append('delivery_order_draft_kirim_provinsi', $("#deliveryorderdraft-delivery_order_draft_kirim_provinsi").val());
										form_data.append('delivery_order_draft_kirim_kota', $("#deliveryorderdraft-delivery_order_draft_kirim_kota").val());
										form_data.append('delivery_order_draft_kirim_kecamatan', $("#deliveryorderdraft-delivery_order_draft_kirim_kecamatan").val());
										form_data.append('delivery_order_draft_kirim_kelurahan', $("#deliveryorderdraft-delivery_order_draft_kirim_kelurahan").val());
										form_data.append('delivery_order_draft_kirim_kodepos', $("#deliveryorderdraft-delivery_order_draft_kirim_kodepos").val());
										form_data.append('delivery_order_draft_kirim_area', $("#deliveryorderdraft-delivery_order_draft_kirim_area").val());
										form_data.append('delivery_order_draft_kirim_invoice_pdf', "");
										form_data.append('delivery_order_draft_kirim_invoice_dir', "");
										form_data.append('pabrik_id', $("#deliveryorderdraft-pabrik_id").val());
										form_data.append('delivery_order_draft_ambil_nama', $("#deliveryorderdraft-delivery_order_draft_ambil_nama").val());
										form_data.append('delivery_order_draft_ambil_alamat', $("#deliveryorderdraft-delivery_order_draft_ambil_alamat").val());
										form_data.append('delivery_order_draft_ambil_telp', $("#deliveryorderdraft-delivery_order_draft_ambil_telepon").val());
										form_data.append('delivery_order_draft_ambil_provinsi', $("#deliveryorderdraft-delivery_order_draft_ambil_provinsi").val());
										form_data.append('delivery_order_draft_ambil_kota', $("#deliveryorderdraft-delivery_order_draft_ambil_kota").val());
										form_data.append('delivery_order_draft_ambil_kecamatan', $("#deliveryorderdraft-delivery_order_draft_ambil_kecamatan").val());
										form_data.append('delivery_order_draft_ambil_kelurahan', $("#deliveryorderdraft-delivery_order_draft_ambil_kelurahan").val());
										form_data.append('delivery_order_draft_ambil_kodepos', $("#deliveryorderdraft-delivery_order_draft_ambil_kodepos").val());
										form_data.append('delivery_order_draft_ambil_area', $("#deliveryorderdraft-delivery_order_draft_ambil_area").val());
										...
										form_data.append('tipe_delivery_order_id', $("#deliveryorderdraft-tipe_delivery_order_id").val());
										form_data.append('tipe_gudang', $("#deliveryorderdraft-tipe_gudang").val());
										form_data.append('detail', json_arr);
										form_data.append('so_eksternal', $("#input_so_eks").val());
										form_data.append('do_eksternal', $("#input_do_eks").val());
										//ajax save data
										$.ajax({
											async: false,
	...
	$("#btnupdatedodraft").on("click", function() {
		...
		var act = '<?= $this->uri->segment(4) ?>';
		var reff_id = $("#deliveryorderdraft-delivery_order_draft_reff_id").val()
		var area = $("#deliveryorderdraft-delivery_order_draft_kirim_area").val();
		...
		if (area != "") {
			if (tipe_do != "") {
				if (cek_customer > 0) {
					if (arr_sku.length > 0) {
						$("#table-sku-delivery-only > tbody tr").each(function() {
							if (act == 'dosplit' || reff_id != "") {
								var is_Qty = $(this).find("td:eq(12) input[type='number']");
							} else {
								var is_Qty = $(this).find("td:eq(11) input[type='number']");
							}
							var is_tipe_stock = $(this).find("td:eq(6) select");
						...
						if (cek_tipe_stock == 0) {
							if (cek_qty == 0) {
								...
								for (var index = 0; index < arr_sku.length; index++) {
									if (arr_sku[index] != "") {
										if (act == 'dosplit' || reff_id != "") {
											var str = $("#item-" + index + "-DeliveryOrderDetailDraft-sku_qty_ambil").val()
										} else {
											var str = $("#item-" + index + "-DeliveryOrderDetailDraft-sku_qty").val()
										}
										arr_detail.push({
											'sku_id': $("#item-" + index + "-DeliveryOrderDetailDraft-sku_id").val(),
											'sku_kode': $("#item-" + index + "-DeliveryOrderDetailDraft-sku_kode").val(),
								...
								Swal.fire({
									...
									if (result.value == true) {
										...
										form_data.append('sales_order_id', $("#deliveryorderdraft-sales_order_id").val());
										form_data.append('delivery_order_draft_kode', $("#deliveryorderdraft-delivery_order_draft_kode").val());
										form_data.append('delivery_order_draft_yourref', "");
										form_data.append('client_wms_id', $("#deliveryorderdraft-client_wms_id").val());
										form_data.append('delivery_order_draft_tgl_buat_do', $("#deliveryorderdraft-delivery_order_draft_tgl_buat_do").val());
										form_data.append('delivery_order_draft_tgl_expired_do', $("#deliveryorderdraft-delivery_order_draft_tgl_expired_do").val());
										...
										form_data.append('confirm_rate', "");
										if (act == 'dosplit') {
											form_data.append('delivery_order_draft_reff_id', dod_id);
											form_data.append('delivery_order_draft_reff_no', $("#deliveryorderdraft-delivery_order_draft_kode").val());
										} else {
											form_data.append('delivery_order_draft_reff_id', $("#deliveryorderdraft-delivery_order_draft_reff_id").val());
											form_data.append('delivery_order_draft_reff_no', $("#deliveryorderdraft-delivery_order_draft_reff_no").val());
										}
										...
										form_data.append('delivery_order_draft_ambil_area', $("#deliveryorderdraft-delivery_order_draft_ambil_area").val());
										form_data.append('delivery_order_draft_update_who', "");
										form_data.append('delivery_order_draft_update_tgl', $("#deliveryorderdraft-delivery_order_draft_update_tgl").val());
										form_data.append('delivery_order_draft_approve_who', "");
										form_data.append('delivery_order_draft_approve_tgl', "");
										form_data.append('delivery_order_draft_reject_who', "");
										form_data.append('delivery_order_draft_reject_tgl', "");
										...
										messageBoxBeforeRequest('Pastikan data yang sudah anda input benar!', 'Iya, Simpan', 'Tidak, Tutup').then((result) => {
											if (result.value == true) {
												if (act == 'dosplit') {
													var link = 'WMS/Distribusi/DeliveryOrderDraft/insert_delivery_order_draft'
												} else {
													var link = 'WMS/Distribusi/DeliveryOrderDraft/update_delivery_order_draft'
												}
												requestAjax("<?= base_url(); ?>" + link, {
													dataForm: form_data
													...
													if (response.status == true) {
														if (response.message == "reload") {
															...
															setTimeout(() => {
																if (act == 'dosplit') {
																	var link = 'WMS/Distribusi/DeliveryOrderDraft'
																} else {
																	var link = 'WMS/Distribusi/DeliveryOrderDraft/edit/?id=' + dod_id
																}
																// location.href =
								...
								if (cek_qty == 0) {
									...
									Swal.fire({
										...
										if (result.value == true) {
											...
											messageBoxBeforeRequest('Pastikan data yang sudah anda input benar!', 'Iya, Simpan', 'Tidak, Tutup').then((result) => {
												if (result.value == true) {
													requestAjax("<?= base_url('WMS/Distribusi/DeliveryOrderDraft/update_delivery_order_draft'); ?>", {
														dataForm: form_data
													}, "POST", "JSON", function(response) {
														if (response.status == true) {
															if (response.message == "reload") {
																...
																message_topright("success", response.message);
																setTimeout(() => {
																	location.href = "<?= base_url(); ?>WMS/Distribusi/DeliveryOrderDraft/edit/?id=" + dod_id;
																}, 500);
															}
														} else {
															message_topright("error", response.message);
					...
					if (arr_sku.length > 0) {
						...
						if (cek_tipe_stock == 0) {
							if (cek_qty == 0) {
								...
								messageBoxBeforeRequest('Pastikan data yang sudah anda input benar!', 'Iya, Simpan', 'Tidak, Tutup').then((result) => {
									if (result.value == true) {
										requestAjax("<?= base_url('WMS/Distribusi/DeliveryOrderDraft/confirm_delivery_order_draft'); ?>", {
											...
											// console.log(data);
											if (data.type === 200) {
												message_topright("success", "Data berhasil diapprove");
												setTimeout(() => {
													location.href = "<?= base_url(); ?>WMS/Distribusi/DeliveryOrderDraft/detail/?id=" + dod_id;
												}, 500);
											}
											if (data.type === 400) {
												messageNotSameLastUpdated();
											...
											if (data.type === 204) {
												...
												$.each(data.data, (index, item) => {
													...
													arrayOfErrorsToDisplaySkuStokKurang.push({
														title: 'Data Gagal diapprove!',
											...
											if ($("#deliveryorderdraft-tipe_delivery_order_id").val() == "C5BE83E2-01E8-4E24-B766-26BB4158F2CD") {
												$.each(data, function(i, v) {
													...
													if (v.type == 211) {
														message_topright("success", "SKU Stock Cocok");
													}
												})
											}
										}, "#btnconfirmdodraft")
	...
	$("#btnapprovdodraft").on("click", function() {
		...
		if (count_do_list > 0) {
			...
			for (var i = 0; i < count_do_list; i++) {
				...
				var tgl_update = $("#item-" + i + "-ListDODraft-delivery_order_draft_update_tgl").val();
			...
			if (arr_do_draft.length > 0) {
				Swal.fire({
					...
					if (result.value == true) {
						...
						$.ajax({
							type: 'POST',
							url: "<?= base_url('WMS/Distribusi/DeliveryOrderDraft/proses_approval_delivery_order_draft') ?>",
							dataType: "JSON",
							data: {
							...
							success: function(response) {
								...
								setTimeout(() => {
									...
									Swal.fire({
										title: 'Success',
										text: "Delivery order success approved",
										icon: 'success',
										showCancelButton: false,
										confirmButtonColor: '#3085d6',
										cancelButtonColor: '#d33',
							...
							complete: function() {
								...
								// message("Success", "Delivery order success approved", "success");
	...
	function DeleteSKU(row, index) {
		...
		<?php if ($act == "dosplit") { ?>
			arr_sku_do_split = arr_sku_do_split.filter(function(item) {
				return item != $(`#item-${index}-DeliveryOrderDetailDraft-sku_id`).val();
			})
		<?php } ?>
		// console.log(arr_sku, $(`#item-${index}-DeliveryOrderDetailDraft-sku_id`).val());

		// arr_sku = arr_sku.filter(function(item) {
		// 	return item != "'" + $(`#item-${index}-DeliveryOrderDetailDraft-sku_id`).val() + "'";
		// })
	...
	const handlerDataSearchUbahKeDraft = () => {
		...
		const principle = $("#principleUbahKeDraft").val();
		...
		$.ajax({
			...
			beforeSend: function() {
				...
				$("#btn_ubah_do_draft").prop("disabled", true);
			...
			success: function(response) {
				...
				if (response.length > 0) {
					...
					$("#tableDataUbahKeDraft > tbody").append(`
						<tr>
							<td class="text-center" colspan="10"><strong class="text-danger">Data Kosong</strong></td>
						</tr>
					`);
				}
			},
			error: function(xhr, ajaxOptions, thrownError) {
				message("Error", "Error 500 Internal Server Connection Failure", "error");
				$("#btn_ubah_do_draft").prop("disabled", false);
			},
			complete: function() {
				$("#btn_ubah_do_draft").prop("disabled", false);
				Swal.close();
		...
		if (arrUbahDraft.length > 0) {
			Swal.fire({
				...
				if (result.value == true) {
					//ajax save data
					$.ajax({
						url: "<?= base_url('WMS/Distribusi/DeliveryOrderDraft/proses_delivery_order_ubah_ke_draft'); ?>",
						type: "POST",
						data: {
							arr_do_draft: arrUbahDraft
						},
						dataType: "JSON",
						beforeSend: function() {
							...
							$("#btn_save_ubah_draft").prop("disabled", true);
						},
						success: function(response) {

							ViewResponseMessageUbahDo(response);

						},
						error: function(xhr, ajaxOptions, thrownError) {
							message("Error", "Error 500 Internal Server Connection Failure", "error");
							$("#btn_save_ubah_draft").prop("disabled", false);
						},
						complete: function() {
							message("Success", "Generate Delivery Order Draft Success", "success");
							$("#btn_save_ubah_draft").prop("disabled", false);
						}
					});
				}
	...
	const handlerDataSearchEditDO = () => {
		...
		$.ajax({
			...
			beforeSend: function() {
				...
				$("#btn_search_data_edit_do").prop("disabled", true);
				$("#btn_save_edit_do").prop("disabled", true);
			},
			success: function(response) {
				if (response.data.length > 0) {
					...
					var options = '<option value=""><label name="CAPTION-PILIH">Pilih</label></option>';

					<?php foreach ($TipeDeliveryOrder as $type) : ?>
						options += `<option value="<?= $type['tipe_delivery_order_id'] ?>"><?= $type['tipe_delivery_order_alias'] ?></option>`
					<?php endforeach; ?>

					var options2 = '<option value=""><label name="CAPTION-PILIH">Pilih</label></option>';

					<?php foreach ($TipeGudang as $type) : ?>
						options2 += `<option value="<?= $type['tipegudang_kode'] ?>"><?= $type['tipegudang_nama'] ?></option>`
					<?php endforeach; ?>
					...
					response.data.map((item, index) => {
						$("#tableDataDOEdit > tbody").append(`
							<tr>
								...
								<td>${item.tipe_delivery_order_alias}</td>
								<td>${item.tipegudang_nama}</td>
								<td>
									<input type="date" class="form-control aktualRencanaKirim" disabled id="aktualRencanaKirim${item.delivery_order_draft_id}" />
								</td>
								<td>
									<select disabled id="TipeEditGlobal${item.delivery_order_draft_id}" name="TipeEditGlobal${item.delivery_order_draft_id}" class="form-control TipeEditGlobal">
										${options}
									</select>
								</td>
								<td>
									<select disabled id="TipeGudangEditGlobal${item.delivery_order_draft_id}" name="TipeGudangEditGlobal${item.delivery_order_draft_id}" class="form-control TipeGudangEditGlobal">
										${options2}
									</select>
								</td>
							</tr>
			...
			error: function(xhr, ajaxOptions, thrownError) {
				...
				$("#btn_search_data_edit_do").prop("disabled", false);
				$("#btn_save_edit_do").prop("disabled", false);
			},
			complete: function() {
				Swal.close();
		...
		if (e.checked) {
			for (var i = 0; i < checkboxes.length; i++) {
				if (checkboxes[i].type == 'checkbox') {
					...
					$(`.TipeEditGlobal`).val($("#allChangeTipe").val() == '' ? '' : $(
						"#allChangeTipe").val())
					$(`.TipeEditGlobal`).prop('disabled', false);
					$(`.TipeGudangEditGlobal`).val($("#allChangeTipeGudang").val() == '' ? '' : $(
						"#allChangeTipeGudang").val())
					$(`.TipeGudangEditGlobal`).prop('disabled', false);
				}
			}
		} else {
			for (var i = 0; i < checkboxes.length; i++) {
				if (checkboxes[i].type == 'checkbox') {
					...
					$(".TipeEditGlobal").val('')
					$(".TipeEditGlobal").prop('disabled', true)
					$(".TipeGudangEditGlobal").val('')
					$(".TipeGudangEditGlobal").prop('disabled', true)
				}
			}
		}
	}
	...
	const handlerRemoveAktualWhenUnchecked = (event) => {
		...
		var tipe_gudang_asli = $(`#TipeGudangAsli${event.currentTarget.value}`).val();
		var tipe_gudang = $("#allChangeTipeGudang").val() != '' ? $(`#allChangeTipeGudang option:selected`).text() : '';

		if (event.currentTarget.checked == false) {
			$(`#aktualRencanaKirim${event.currentTarget.value}`).val('');
			$(`#aktualRencanaKirim${event.currentTarget.value}`).prop('disabled', true);
			$(`#TipeEditGlobal${event.currentTarget.value}`).val('');
			$(`#TipeEditGlobal${event.currentTarget.value}`).prop('disabled', true);
			$(`#TipeGudangEditGlobal${event.currentTarget.value}`).val('');
			$(`#TipeGudangEditGlobal${event.currentTarget.value}`).prop('disabled', true);
		} else {
			...
			$(`#TipeEditGlobal${event.currentTarget.value}`).val($("#allChangeTipe").val() == '' ? '' : $(
				"#allChangeTipe").val())
			$(`#TipeEditGlobal${event.currentTarget.value}`).prop('disabled', false);
			$(`#TipeGudangEditGlobal${event.currentTarget.value}`).val($("#allChangeTipeGudang").val() == '' ? '' : $(
				"#allChangeTipeGudang").val())
			$(`#TipeGudangEditGlobal${event.currentTarget.value}`).prop('disabled', false);
		}
	}
	...
	const handlerAllChangeTipe = (value) => {
		const checkboxes = $("input[name='chk-data[]']");

		for (var i = 0; i < checkboxes.length; i++) {
			if (checkboxes[i].checked == true) {
				const valueId = checkboxes[i].value
				$(`#TipeEditGlobal${valueId}`).val(value);
			}
		}
	}

	const handlerAllChangeTipeGudang = (value) => {
		if (value == "") {
			message("Error!", "field tidak boleh kosong", 'error');
			return false;
		}
		...
		const checkboxes = $("input[name='chk-data[]']");
		var tipe_gudang = $("#allChangeTipeGudang").val() != '' ? $(`#allChangeTipeGudang option:selected`).val() : '';

		for (var i = 0; i < checkboxes.length; i++) {
			if (checkboxes[i].checked == true) {
				const valueId = checkboxes[i].value
				$(`#TipeGudangEditGlobal${valueId}`).val(tipe_gudang);
			}
		}
	}
	...
	const handlerSaveEditDO = () => {
		...
		// if ($(`#allChangeTipeGudang`).val() == "") {
		// 	message("Error!", "Tipe gudang tidak boleh kosong", 'error')
		// 	return false;
		// }
		...
		for (var i = 0; i < checkboxes.length; i++) {
			if (checkboxes[i].checked == true) {
				...
				const TipeEditGlobal = $(`#TipeEditGlobal${valueId}`).val();
				const TipeGudang = $(`#TipeGudangEditGlobal${valueId}`).val();

				if (aktualRencanaKirim == "" && TipeEditGlobal == "" && TipeGudang == "") {
					error++;
					message("Error!", "Wajib mengisi salah satu dari: Tanggal Rencana Kirim, Tipe, atau Tipe Gudang yang sudah dichecked.", 'error')
					return false;
				} else {
					arrData.push({
						valueId,
	...
	const handlerSavePembatalanDO = () => {
		...
		Swal.fire({
			...
			if (result.value == true) {
				...
				$.ajax({
					url: "<?= base_url('WMS/Distribusi/DeliveryOrderDraft/proses_pembatalan_delivery_order_draft'); ?>",
					type: "POST",
					data: {
						arr_do_draft: arrData
					},
					dataType: "JSON",
					beforeSend: function() {
						...
						$("#btn_save_pembatalan_do").prop("disabled", true);
					},
					success: function(response) {

						ViewResponseMessagePembatalanDO(response);

					},
					error: function(xhr, ajaxOptions, thrownError) {
						message("Error", "Error 500 Internal Server Connection Failure", "error");
						$("#btn_save_pembatalan_do").prop("disabled", false);
					},
					complete: function() {
						message("Success", "Generate Delivery Order Draft Success", "success");
						$("#btn_save_pembatalan_do").prop("disabled", false);
	...
	function ViewResponseMessageUbahDo(response) {
		$("#modal_response_message_ubah_do").modal('show');

		$("#table_response_message_ubah_do > tbody").empty();

		if ($.fn.DataTable.isDataTable('#table_response_message_ubah_do')) {
			$('#table_response_message_ubah_do').DataTable().clear();
			$('#table_response_message_ubah_do').DataTable().destroy();
		}

		if (response.length > 0) {
			$.each(response, function(i, v) {

				$("#table_response_message_ubah_do > tbody").append(`
					<tr>
						<td class="text-center">${i+1}</td>
						<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}">${v.StatusMsg}</td>
						<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}">${v.ErrMsg}</td>
					</tr>
				`);
			});

			$('#table_response_message_ubah_do').DataTable({
				'lengthMenu': [
					[100, 200, 250, -1],
					[100, 200, 250, 'All']
				]
	...
	function CloseResponseMessageUbahDo() {
		handlerDataSearchUbahKeDraft();
	}

	function ViewResponseMessagePembatalanDO(response) {
		$("#modal_response_message_pembatalan_do").modal('show');

		$("#table_response_message_pembatalan_do > tbody").empty();

		if ($.fn.DataTable.isDataTable('#table_response_message_pembatalan_do')) {
			$('#table_response_message_pembatalan_do').DataTable().clear();
			$('#table_response_message_pembatalan_do').DataTable().destroy();
		}

		if (response.length > 0) {
			$.each(response, function(i, v) {

				$("#table_response_message_pembatalan_do > tbody").append(`
					<tr>
						<td class="text-center">${i+1}</td>
						<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}">${v.StatusMsg}</td>
						<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}">${v.ErrMsg}</td>
			...
			$('#table_response_message_pembatalan_do').DataTable({
				'lengthMenu': [
					[100, 200, 250, -1],
					[100, 200, 250, 'All']
				]
	...
	function CloseResponseMessagePembatalanDO() {
		handlerDataSearchPembatalanDO();
	}
</script>
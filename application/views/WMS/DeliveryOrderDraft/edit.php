				...
				<div class="col-md-12 col-sm-12 col-xs-12">
					<div class="x_panel">
						...
						<div class="row">
							...
							<div class="col-xs-3">
								<div class="form-group field-deliveryorderdraft-delivery_order_draft_kode">
									<label class="control-label" for="deliveryorderdraft-tipe_gudang" name="CAPTION-TIPEGUDANG">Tipe Gudang</label>
									<select name="DeliveryOrderDraft[tipe_gudang]" class="input-sm form-control" id="deliveryorderdraft-tipe_gudang">
										<option value="">** <label name="CAPTION-TIPEGUDANG">Tipe Gudang</label> **</option>
										<?php foreach ($TipeGudang as $type) : ?>
											<option value="<?= $type['tipegudang_kode'] ?>" <?= $type['tipegudang_kode'] == $header['tipe_gudang'] ? 'selected' : '' ?>><?= $type['tipegudang_nama'] ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-xs-3">
								<div class="form-group field-deliveryorderdraft-delivery_order_draft_tipe_pembayaran">
									...
									<fieldset>
										<label>
											<input class="radio-payment-type" type="radio" name="DeliveryOrderDraft[delivery_order_draft_tipe_pembayaran]" id="deliveryorderdraft-delivery_order_draft_tipe_pembayaran" value="0" <?= (int)$header['delivery_order_draft_tipe_pembayaran'] == 0 ? 'checked' : '' ?>> <label name="CAPTION-TUNAI">Tunai</label>
										</label>
									</fieldset>
									<fieldset>
										<label>
											<input class="radio-payment-type" type="radio" name="DeliveryOrderDraft[delivery_order_draft_tipe_pembayaran]" id="deliveryorderdraft-delivery_order_draft_tipe_pembayaran" value="1" <?= $header['delivery_order_draft_tipe_pembayaran'] == 1 ? 'checked' : '' ?>> <label name="CAPTION-NONTUNAI">Non Tunai</label>
										</label>
									</fieldset>
								</div>
							</div>
							<div class="col-xs-3">
								<div class="form-group field-deliveryorderdraft-delivery_order_draft_tipe_layanan">
									...
									?>
										<fieldset>
											<label>
												<input class="radio-service-type" type="radio" id="deliveryorderdraft-delivery_order_draft_tipe_layanan" name="DeliveryOrderDraft[delivery_order_draft_tipe_layanan]" value="<?= $row['tipe_layanan_nama'] ?>" <?= $header['delivery_order_draft_tipe_layanan'] == $row['tipe_layanan_nama'] ? 'checked' : '' ?>>
							...
							<div class="col-xs-3" id="tunai">
								...
								<div class="form-group">
									<label class="control-label" for="" name="CAPTION-DOKUMENSOEKSTERNAl">Dokumen SO Eksternal</label>
									<input type="text" id="input_so_eks" class="form-control input-sm" name="input_so_eks" autocomplete="off" placeholder="input dokumen so eksternal" value="<?= $header['so_eksternal_no'] ?>">
								</div>
							</div>
							<div class="col-xs-3">
								<div class="form-group">
								...
								<div class="form-group">
									<label class="control-label" for="" name="CAPTION-DOKUMENDOEKSTERNAl">Dokumen DO Eksternal</label>
									<input type="text" id="input_do_eks" class="form-control input-sm" name="input_do_eks" autocomplete="off" placeholder="input dokumen do eksternal" value="<?= $header['do_eksternal_no'] ?>">
								</div>
							</div>
						</div>
					</div>
					...
					<table class="table table-striped" id="table-sku-delivery-only">
						<thead>
							<tr class="bg-primary">
								...
								<?php if ($header['delivery_order_draft_reff_id'] != null) :  ?>
									<th class="text-center" style="color:white;" name="CAPTION-QTYSISA">Qty Sisa</th>
									<th class="text-center" style="color:white;" name="CAPTION-JUMLAHBARANG">Jumlah Barang</th>
								<?php else : ?>
							...
							if (!empty($DODetail)) { ?>
								...
								?>
									<tr id="row-<?= $i ?>" class="row-item">
										...
										<td class="text-center"><input <?= $header['delivery_order_draft_reff_id'] != null ? 'readonly' : '' ?> min="0" type="number" id="item-<?= $i ?>-DeliveryOrderDetailDraft-sku_qty" class="form-control sku-qty" value="<?= $header['delivery_order_draft_reff_id'] != null ? $detail['sku_qty_sisa'] : $detail['sku_qty']; ?>" onchange="UpdateTotalHarga('<?= $i ?>')" /></td>
										<?php if ($header['delivery_order_draft_reff_id'] != null) :  ?>
											<td class="text-center"><input onchange="changeQtySisa(this.value,<?= $i ?>, '<?= $detail['sku_qty'] + $detail['sku_qty_sisa']; ?>')" min="0" type="number" id="item-<?= $i ?>-DeliveryOrderDetailDraft-sku_qty_ambil" class="form-control sku-qty-ambil" value="<?= $detail['sku_qty']; ?>" /></td>
										<?php endif; ?>
										<!-- <td class="text-center">
											<?php
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="row" style="margin-top: 10px;">
					<div class="col-xs-12">
						<table class="table table-striped" id="table-sku" style="width: 100% !important;">
							<thead>
								<tr class="bg-primary">

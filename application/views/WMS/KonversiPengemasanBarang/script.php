<script type="text/javascript">
	...
	$("#btn-search-data-konversi").on("click", function() {
		...
		$.ajax({
			type: 'POST',
			url: "<?= base_url('WMS/KonversiPengemasanBarang/GetKonversiSKUByFilter') ?>",
			data: {
			...
			success: function(response) {
				...
				if (response != 0) {
					$.each(response, function(i, v) {
						if (v.tr_konversi_sku_status == "Completed" || v.tr_konversi_sku_status == "Canceled") {
							$("#table_list_data_konversi > tbody").append(`
								<tr>
									...
									<td class="text-center">
                                        <a href="<?= base_url() ?>WMS/KonversiPengemasanBarang/detail/?id=${v.tr_konversi_sku_id}" class="btn btn-success btn-small btn-delete-sku"><i class="fa fa-search"></i></a>
                                        <a href="<?= base_url() ?>WMS/KonversiPengemasanBarang/cetak/?id=${v.tr_konversi_sku_id}" class="btn btn-danger btn-small" target="_blank"><i class="fas fa-print"></i></a>
                                    </td>
								</tr>
							`);
						} else {
							$("#table_list_data_konversi > tbody").append(`
								<tr>
									...
									<td class="text-center">${v.tipe_konversi_nama}</td>
									<td class="text-center">${v.tr_konversi_sku_status}</td>
									<td class="text-center"><a href="<?= base_url() ?>WMS/KonversiPengemasanBarang/edit/?id=${v.tr_konversi_sku_id}" class="btn btn-primary btn-small btn-delete-sku"><i class="fa fa-pencil"></i></a></td>
								</tr>
							`);
						}
					});
	...
	function GetSKUKonversi(index) {
		...
		$.ajax({
			type: 'POST',
			url: "<?= base_url('WMS/KonversiPengemasanBarang/GetKonversiDetailPallet2') ?>",
			data: {
	...
	function GetFormKonversiSKUDetail2(tr_konversi_sku_detail2_id, index) {
		...
		$.ajax({
			type: 'POST',
			url: "<?= base_url('WMS/KonversiPengemasanBarang/GetViewkonversiskudetail2') ?>",
			data: {
	...
	function GetListPalletAsal(tr_konversi_sku_detail_id) {

		$.ajax({
			url: "<?= base_url('WMS/KonversiPengemasanBarang/GetKonversiPalletAsal'); ?>",
			type: "POST",
			async: false,
			data: {
					...
					$.each(response, function(i, v) {
						$("#table-pallet-asal > tbody").append(`
							<tr id="row-${i}">
								...
								<td style="text-align: center; vertical-align: middle;">${v.pallet_kode}</td>
								<td style="text-align: center; vertical-align: middle;">${v.sku_stock_qty_ambil}</td>
							</tr>
						`);
					});
				}

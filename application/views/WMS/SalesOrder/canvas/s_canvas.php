<script type="text/javascript">
	...
	// $("#btnsavecanvasdowms").click(function() {
	// 	var tgl = $("#datesofas").val();
	...
	$("#btnsavecanvasdowms").click(function() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$("#modal_result_message").modal('show');

				$("#table_result_message > tbody").html('');
				$("#table_result_message > tbody").empty();

				if ($.fn.DataTable.isDataTable('#table_result_message')) {
					$('#table_result_message').DataTable().clear();
					$('#table_result_message').DataTable().destroy();
				}

				// console.log(response);
				$.each(response, function(i, v) {
					$("#table_result_message > tbody").append(`
							<tr>
								<td class="text-center">${i+1}</td>
								<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}">${v.StatusMsg}</td>
								<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}">${v.ResultMsg}</td>
							</tr>
						`);
				});

				$('#table_result_message').DataTable({
					lengthMenu: [
						[10, 50, 100],
						[10, 50, 100]
					],
	...
	$("#btnrefreshcanvas").click(function() {
		GetSalesOrderFAS();
	});

			...
			<div class="header">
				<table class="header-table">
					<tr>
						...
						<th rowspan="2">Faktur - Surat Jalan</th>
						<th rowspan="2">REG</th>
					...
					<tr>
						...
						<td>: </td>
						<td>JATUH TEMPO</td>
						<td>: <?= $header['delivery_order_tipe_pembayaran'] == 'TUNAI' ? $header['delivery_order_tgl_rencana_kirim'] : $header['delivery_order_tgl_expired_do'] ?></td>
						<td>NPWP PENERIMA</td>
						<td>: <?= $header['client_pt_npwp'] ?></td>
					</tr>
				</table>
					...
					<tfoot>
						...
						?>
								...
								<tr>
									...
									<td style="border-top :0px;border-left :0px;border-right :0px;border-bottom :1px dashed;">PPN</td>
								...
								<?php foreach ($FDJRBank as $key => $bank) :
									if ($header['delivery_order_kode'] == $bank['kode']) {
								?>
										<tr>
											<td colspan="4" style="border :0px;"><?= $bank['bank'] ?></td>
											<td colspan="6" style="border :0px;">PEMBAYARAN SAH BILA DILAMPIRKAN FAKTUR ASLI (DO/INVOICE)</td>
										</tr>

										<tr>
											<!-- <td colspan="10" style="border :0px;"><?= $FDJRBank['kode'] ?></td> -->
											<td colspan="10" style="border :0px;">
												<img src="https://api.qrserver.com/v1/create-qr-code/?data=<?= $bank['kode'] ?>&amp;size=50x50" />
												<p class="text-left" style="margin-top:10px; font-size: 10px"><strong><?= $bank['kode'] ?></strong></p>

												<!-- <div style="border: 1px dashed black; padding: 10px;">
											<img src="https://api.qrserver.com/v1/create-qr-code/?data=<?= $FDJRBank['kode'] ?>&amp;size=20x20" />
											<p class="text-center" style="margin-top:10px; font-size: 10px"><strong><?= $FDJRBank['kode'] ?></strong>
											</p>
										</div> -->
											</td>
										</tr>

<script type="text/javascript">
	...
	function getListDOBatch() {
		...
		table_list_do_batch = $('#table_list_data_do_batch').DataTable({
			...
			// 	console.log(resp);
			// 	arrayDetail = []
			// 	for (var i = 0; i <i resp.data.length; i++) {
			...
			'columns': [{
				...
				},
				{
					render: function(data, type, row, meta) {
						...
						// 	str += `<a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/detail/?id=${row.delivery_order_batch_id}" class="btn btn-primary btn-sm"><i class="fa fa-search"></i></a>`
						// } else if (row.delivery_order_batch_status == "in progress item request") {
						// 	str += `<a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/detail/?id=${row.delivery_order_batch_id}" class="btn btn-primary btn-sm"><i class="fa fa-search"></i></a><a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/print/?id=${row.delivery_order_batch_id}" target="_blank" class="btn btn-danger btn-sm" title="Cetak FDJR"><i class="fa fa-file-pdf"></i></a><a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/print_by_do/?id=${row.delivery_order_batch_id}" target="_blank" class="btn btn-info btn-sm" title="Cetak By Do"><i class="fa fa-print"></i></a>`
						// 	str += `<button onclick="batalkan('${row.delivery_order_batch_id}', '${row.delivery_order_batch_update_tgl}')" class="btn btn-danger" title="Batalkan FDJR"><i class="fa fa-cancel"></i></button>`
						// } else {
						// 	str += `<a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/detail/?id=${row.delivery_order_batch_id}" class="btn btn-primary btn-sm"><i class="fa fa-search"></i></a><a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/print/?id=${row.delivery_order_batch_id}" target="_blank" class="btn btn-danger btn-sm" title="Cetak FDJR"><i class="fa fa-file-pdf"></i></a><a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/print_by_do/?id=${row.delivery_order_batch_id}" target="_blank" class="btn btn-info btn-sm" title="Cetak By Do"><i class="fa fa-print"></i></a>`
						// }
						...
						if (row.delivery_order_batch_tipe_layanan_nama == 'Pickup and Delivery') {
							str += `<a href="<?= base_url() ?>WMS/Distribusi/DeliveryOrderBatch/print_surat_jalan_pengambilan_barang/?id=${row.delivery_order_batch_id}" target="_blank" class="btn btn-primary btn-sm" title="Word Surat Jalan Pengambilan Barang"><i class="fa fa-file-word"></i></a>`
							str += `<button onclick="ViewAttachmentDO('${row.delivery_order_batch_id}','${row.delivery_order_batch_kode}','${row.delivery_order_batch_tanggal_kirim}','${row.karyawan_nama}','${row.kendaraan_nopol}')" class="btn btn-primary" title="Attachment Permintaan Pengiriman"><i class="fa fa-file"></i></button>`
						}
				...
				},
				{
					render: function(data, type, row, meta) {
						...
						if (row.delivery_order_batch_status == "Draft") {
							...
							str += `<button class="btn btn-warning btn-sm" onclick="btnReAssignmentDriver('${row.delivery_order_batch_tanggal_kirim2}','${row.delivery_order_batch_id}', '${row.delivery_order_batch_update_tgl}', '${row.karyawan_id}', '${row.kendaraan_id}', '${row.delivery_order_batch_h_id}', '${row.delivery_order_batch_h_ritasi}')"><i class="fa fa-pencil"></i></button>`
						} else {
							str += ``
						}
	...
	function batalkan(do_batch_id, delivery_order_batch_update_tgl) {
		Swal.fire({
			...
			if (result.value == true) {
				//ajax save data
				requestAjax("<?= base_url('WMS/Distribusi/DeliveryOrderBatch/batalkan_do_batch_after_laksanakan'); ?>", {
					delivery_order_batch_id: do_batch_id,
					tglLastUpdate: delivery_order_batch_update_tgl,
					mode: 'after'
	...
	function btnReAssignmentDriver(tlg_kirim, do_batch_id, do_batch_update_tgl, karyawan_id, kendaraan_id, delivery_order_batch_h_id, ritasi) {
		// $.ajax({
		// 	type: "POST",
		// 	url: "<?= base_url('WMS/Distribusi/DeliveryOrderBatch/checkLastUpdate') ?>",
		// 	data: {
		...
		$("#delivery_order_batch_h_id").val(delivery_order_batch_h_id)
		$("#do_batch_id").val(do_batch_id)
		$("#last_update").val(do_batch_update_tgl)
		$("#ubahDriverKendaraanRitasi").val(ritasi)
		$("#ubahDriverKendaraanDriver").val(karyawan_id).trigger("change");
		$("#ubahDriverKendaraanKendaraan").val(kendaraan_id).trigger("change");
		$('#ubahDriverKendaraanTanggal').val(tlg_kirim);
		$('#ubah_driver').val(karyawan_id);
		$('#ubah_kendaraan').val(kendaraan_id);

		$("#modalUbahDriverKendaraan").modal().show();
	}

	$('#btn-search-data-ubah-driver-kendaraan').on('click', function() {
		var before_driver = $("#ubah_driver").val();
		var before_kendaraan = $("#ubah_kendaraan").val();
		var tgl_kirim = $('#ubahDriverKendaraanTanggal').val();
		var kendaraan = $('#ubahDriverKendaraanKendaraan').val();
		var driver = $('#ubahDriverKendaraanDriver').val();
		var ritasi = $('#ubahDriverKendaraanRitasi').val();
		...
		if (before_driver == driver && before_kendaraan == kendaraan) {
			message("Info!", "Harap Mengubah Driver atau Kendaraan!", "info");
			return false;
		}
		...
		$.ajax({
			...
			dataType: "JSON",
			success: function(response) {
				$('#table-ubah-driver-kendaraan > tbody').empty();

				if (response != 0) {
					$('#table-ubah-driver-kendaraan > tbody').append(`
					<tr>
						...
						<td class="text-center"><button type="button" class="btn btn-success" onclick="saveUbahDriverKendaraan('${response.delivery_order_batch_h_id}')"><i class="fas fa-check"></i> <label name="CAPTION-PILIH">Pilih</label></button></td>
					</tr>
					`);

					$("#mode").val(1);
					$(`#btn_ubah_driver_kendaraan`).hide();
				} else {
					$('#table-ubah-driver-kendaraan > tbody').append(`
					<tr>
						<td class="text-center" style="color:red;" colspan="7">Data Kosong</td>
					</tr>
					`);

					$("#mode").val(0);
					$(`#btn_ubah_driver_kendaraan`).show();
				}
			}
		})
	});

	function saveUbahDriverKendaraan(do_batch_h_id_tujuan) {
		var do_batch_h_id_asal = $("#delivery_order_batch_h_id").val();
		var do_batch_id = $("#do_batch_id").val();
		...
		Swal.fire({
			...
			if (result.value == true) {
				$.ajax({
					type: "POST",
					url: "<?= base_url('WMS/Distribusi/DeliveryOrderBatch/saveUbahDriverKendaraan') ?>",
					data: {
						do_batch_h_id_asal: do_batch_h_id_asal,
						do_batch_id: do_batch_id,
						last_update: last_update,
						driver: driver,
						kendaraan: kendaraan,
						mode: mode,
						do_batch_h_id_tujuan: do_batch_h_id_tujuan
					},
					dataType: "JSON",
					success: function(response) {
						...
						if (response.status == 200) {
							$("#modalUbahDriverKendaraan").modal().hide();
	...
	function ViewAttachmentDO(delivery_order_batch_id, delivery_order_batch_kode, delivery_order_batch_tanggal_kirim, karyawan_nama, kendaraan_nopol) {

		$("#modal_attachment_delivery_order").modal('show');
		$("#filter_attachment_delivery_order_batch_kode").val(delivery_order_batch_kode);
		$("#filter_attachment_tanggal_pengiriman").val(delivery_order_batch_tanggal_kirim);
		$("#filter_attachment_pengemudi").val(karyawan_nama);
		$("#filter_attachment_nopol_kendaraan").val(kendaraan_nopol);

		$.ajax({
			type: 'GET',
			url: "<?= base_url('WMS/Distribusi/DeliveryOrderBatch/Get_attachment_delivery_order') ?>",
			data: {
			...
			success: function(response) {
				if (response.length > 0) {
					$("#table-attachment-delivery-order > tbody").empty();

					if ($.fn.DataTable.isDataTable('#table-attachment-delivery-order')) {
						$('#table-attachment-delivery-order').DataTable().clear();
						$('#table-attachment-delivery-order').DataTable().destroy();
					}

					$.each(response, function(i, v) {

						if (v.delivery_order_attachment != "") {
							$("#table-attachment-delivery-order > tbody").append(`
								<tr>
									<td class="text-center">${i+1}</td>
									<td class="text-center">${v.delivery_order_kode}</td>
									<td class="text-center">
										<a class="btn btn-primary" href="<?= base_url('assets/images/uploads/Invoice/') ?>${v.delivery_order_attachment}" target="_blank"><i class="fa fa-file"></i> ${v.delivery_order_attachment}</a>
									</td>
								</tr>
							`);
						} else {
					...
					$('#table-attachment-delivery-order').DataTable({
						lengthMenu: [
							[50, 100, 200, -1],
							[50, 100, 200, 'All']
						],

<script type="text/javascript">
				...
				if (response != 0) {
					...
					$.each(response, function(i, v) {
						$("#table-konversi-detail > tbody").append(`
							<tr id="tr_${i}" ${v.tr_konversi_sku_detail2_qty > 0 ? 'class="bg-success"' : '' }>
								...
								<td style="text-align: center; vertical-align: middle;">
								<input onclick="checkKonversi(this.checked, '${i}', '${sku_stock_expired_date}', '${v.pallet_id}', '${tr_konversi_sku_detail_id}')" type="checkbox" name="CheckboxKonversi" id="check_konversi_${i}" value="1" ${v.tr_konversi_sku_detail2_qty > 0 ? 'checked' : '' }>
	...
	function checkKonversi(checked, index, sku_stock_expired_date, pallet_id, tr_konversi_sku_detail_id) {
		if (checked) {
			$("#btn_form_konversi_" + index).show();
		} else {

			var newArray = arr_konversi_detail2.filter(x => x.sku_stock_expired_date == sku_stock_expired_date && x.pallet_id_asal == pallet_id && x.tr_konversi_sku_detail_id == tr_konversi_sku_detail_id);

			if (newArray.length > 0) {
				Swal.fire({
					title: "Apakah anda yakin?",
					text: "Data konversi yang telah diinput akan hilang.",
					icon: "warning",
					showCancelButton: true,
					confirmButtonColor: "#3085d6",
					cancelButtonColor: "#d33",
					...
					if (result.value == true) {
						$.ajax({
							async: false,
							url: "<?= base_url('WMS/PembongkaranBarang/delete_tr_konversi_sku_detail2_temp_by_sku_stock_id'); ?>",
							type: "POST",
							data: {
								detail: newArray
							},
							...
							success: function(data) {
								if (data == 1) {
									message("Success", "Data Berhasil Dibatalkan", "success");

									arr_konversi_detail2 = arr_konversi_detail2.filter(x =>
										!(
											x.sku_stock_expired_date == sku_stock_expired_date &&
											x.pallet_id_asal == pallet_id &&
											x.tr_konversi_sku_detail_id == tr_konversi_sku_detail_id
										)
									);

									$(`#item-${index}-konversiskudetail-sku_stock_ambil`).text('0');
									$(`#tr_${index}`).removeClass('bg-success');

									$("#btn_form_konversi_" + index).hide();

								} else {
									message("Error!", "Data Gagal Dibatalkan", "error");
								}
							}
						});
					} else {
						$("#check_konversi_" + index).prop('checked', true);
					}
				})
			} else {
				$("#btn_form_konversi_" + index).hide();

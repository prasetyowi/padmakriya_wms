<script type="text/javascript">
						...
						if (v.tr_konversi_sku_status == "Completed" || v.tr_konversi_sku_status == "canceled") {
							$("#table_list_data_konversi > tbody").append(`
								<tr>
									...
									<td class="text-center">
										...
										${v.tr_konversi_sku_status == "Completed" ? `<a href="<?= base_url() ?>WMS/PembongkaranBarang/cetak/?id=${v.tr_konversi_sku_id}" class="btn btn-danger btn-small" target="_blank"><i class="fas fa-print"></i></a>` : ``}
                                    </td>
								</tr>
							`);
						} else {
							$("#table_list_data_konversi > tbody").append(`
								<tr>
									...
									<td class="text-center">${v.tr_konversi_sku_status}</td>
									<td class="text-center">
										<a href="<?= base_url() ?>WMS/PembongkaranBarang/edit/?id=${v.tr_konversi_sku_id}" class="btn btn-primary btn-small btn-delete-sku"><i class="fa fa-pencil"></i></a>
										<button class="btn btn-warning btn-small" title="Batalkan Data" onclick="btnBatal('${v.tr_konversi_sku_id}')"><i class="fa fa-close"></i></button>
									</td>
								</tr>
							`);
						}
	...
	function btnBatal(tr_konversi_sku_id) {
		Swal.fire({
			...
			if (result.value == true) {
				$.ajax({
					type: 'POST',
					url: "<?= base_url('WMS/PembongkaranBarang/batalkanData') ?>",
					data: {
						tr_konversi_sku_id: tr_konversi_sku_id,
					},
					dataType: "JSON",
					success: function(response) {
						if (response == 1) {

<script>
	...
	$(document).ready(function() {
		...
		requestAjax("<?= base_url('WMS/Transaksi/MutasiAntarUnit/PengambilanBarangMutasiAntarUnit/getDataKoreksiEditById'); ?>", {
			id: global_id
		}, "POST", "JSON", function(response) {
			const header = response.header;
			const detail = response.detail;

			$("#koreksi_draft_id_edit").val(header.tr_mutasi_depo_id);
			$("#gudang_id_edit").val(header.depo_detail_id);


			$("#gudang_nama_edit").val(header.gudangAsal);
			$("#tgl_edit").val(header.tgl);

			$("#no_koreksi_draft_edit").val(header.tr_mutasi_depo_kode);
			$("#depo_asal_edit").val(header.depoAsal);
			$("#gudang_asal_edit").val(header.gudangAsal);
			$("#depo_tujuan_edit").val(header.depoTujuan);
			$("#ekspedisi_edit").val(`${header.ekspedisi_kode} - ${header.ekspedisi_nama}`);
			$("#pengemudi_edit").val(header.karyawan_nama);
			$("#kendaraan_edit").val(`${header.kendaraan_model} - ${header.kendaraan_nopol}`);
			$("#status_edit").val(header.tr_mutasi_depo_status);
			$("#keteranganPersiapan_edit").val(header.tr_mutasi_depo_keterangan == null ? '' : header.tr_mutasi_depo_keterangan);
			$("#keterangan_edit").val(header.tr_mutasi_depo_detail_3_keterangan == null ? '' : header.tr_mutasi_depo_detail_3_keterangan);
			...
			requestAjax("<?= base_url('WMS/Transaksi/MutasiAntarUnit/PengambilanBarangMutasiAntarUnit/check_exist_in_tr_koreksi_draft'); ?>", {
		...
		if (data.length != 0) {
            ...
            <tr>
                ...
                <td class="text-center">
                    <button type="button" data-toggle="tooltip" data-placement="top" title="Pilih Pallet" data-detail2Id="${v.detail2_id}" data-id="${v.id}" data-gudang="${gudang}" data-gudang-id="${gudang_id}" data-ed="${v.ed}" data-qty-plan="${v.qty_plan}" data-sku-kode="${v.sku_kode}" data-sku-nama="${v.sku_nama}" data-qty-aktual="${qyt_aktual}" class="btn btn-info pilihpallet_edit" ><i class="fas fa-hand-point-up"></i></button> 
                </td>
            </tr>
        `);
			});
	...
	$(document).on("click", ".pilihpallet_edit", function() {
		...
		$("#start_scan_edit").attr('data-detail2Id', detail2Id);
		...
		$("#input_manual_edit").attr('data-detail2Id', detail2Id);
	...
	function get_data(id, detail2Id, gudang_id, ed, qty_plan, sku_kode, gudang) {
		...
		$.ajax({
			url: "<?= base_url('WMS/Transaksi/MutasiAntarUnit/PengambilanBarangMutasiAntarUnit/getDataPalletBySkuStockId_edit'); ?>",
			type: "POST",
			data: {
				id: id,
				detail2Id,
			...
			success: function(response) {
				...
				if (response.length > 0) {
					...
					$.each(response, function(i, v) {
						...
						if (v.scan == null) {
							...
							action += `<button type='button' class='form-control btn btn-danger batalkan_pilih_pallet_edit' name='batalkan_pilih_pallet_edit' id='batalkan_pilih_pallet_edit' title="batalkan pilih pallet" data-detail2Id="${v.detail2_id}" data-id="${id}" data-gudang-id="${gudang_id}" data-qty-plan="${qty_plan}" data-ed="${ed}"  data-id-temp="${v.id_temp}" data-pallet-id="${v.pallet_id}" data-sku-stock-id="${v.sku_stock_id}" data-sku-kode="${sku_kode}"><i class="fas fa-xmark"></i></button>`;
              ...
              <tr class="text-center" style="${style}">
                ...
                <td>${v.ed}</td>
                <td>${v.qty}</td>
                <td><input type='text' class='form-control numeric  UpdateQtyAmbil_edit' ${is_scan} name='qty_ambil[]' id='qty_ambil[]' data-detail2Id="${v.detail2_id}" data-id-temp="${v.id_temp}" data-pallet-id="${v.pallet_id}" data-qty-plan="${qty_plan}" data-qty="${v.qty}" data-id="${id}" data-sku-stock-id="${v.sku_stock_id}" data-ed="${ed}" data-sku-kode="${sku_kode}" data-index="${i}" value='${is_value}'/></td>
                <td>${is_lock}</td>
                <td>${action}</td>
              </tr>
          `);
	...
	$(document).on("click", "#input_manual_edit", function() {
		...
		$("#check_kode_edit").attr('data-detail2Id', detail2Id);
	...
	$(document).on("click", "#simpan_koreksi_edit", function() {
		...
		if (is_zero == count) {
			...
			messageBoxBeforeRequest('Pastikan data yang sudah anda input benar!', 'Iya, Simpan', 'Tidak, Tutup').then((result) => {
				if (result.value == true) {
					requestAjax("<?= base_url('WMS/Transaksi/MutasiAntarUnit/PengambilanBarangMutasiAntarUnit/save_data_edit'); ?>", {
			...
			// 		//ajax save data
			// 		$.ajax({
			// 			url: "<?= base_url('WMS/Transaksi/MutasiAntarUnit/PengambilanBarangMutasiAntarUnit/save_data_edit'); ?>",
			// 			type: "POST",
			// 			data: {
			// 				koreksi_id: koreksi_id,
			// 				koreksi_draft_id: koreksi_draft_id,
	...
	$(document).on("click", "#konfirmasi_koreksi_edit", function() {
		...
		if (error == 1) {
			...
			messageBoxBeforeRequest('Pastikan data yang sudah anda input benar!', 'Iya, Konfirmasi', 'Tidak, Tutup').then((result) => {
				if (result.value == true) {
					requestAjax("<?= base_url('WMS/Transaksi/MutasiAntarUnit/PengambilanBarangMutasiAntarUnit/confirm_data'); ?>", {
						koreksi_draft_id: koreksi_draft_id,
						tipe_id: tipe_id,
	...
	$("#search_kode_pallet_edit").on('click', function() {
		const typeScan = $("#tempValForScan").val();
		if ($("#kode_barcode_edit").val() != "") {
			fetch('<?= base_url('WMS/Transaksi/MutasiAntarUnit/PengambilanBarangMutasiAntarUnit/getKodeAutoComplete?params='); ?>' + $("#kode_barcode_edit").val())
				.then(response => response.json())
				.then((results) => {
					if (!results[0]) {
						document.getElementById('table-fixed').style.display = 'none';

<script>
	...
	// 		// {
	// 		// 	data: function(row, type, val, meta) {
	// 		// 		var check = mutasiAntarDepoDetail.find((item) => item.sku_id_induk == row.sku_id && item.ed == row.ed);

	// 		// 		if (check) {
	// 		// 			return `<button class="btn btn-success" id="btn_detail_qty_ambil_sku_${row.sku_id}" onclick="modalDetailQtyAmbilSKU(${currentCounterRow}, '${row.sku_stock_id}', '${row.sku_konversi_group}', '${row.sku_satuan}', '${row.sku_nama_produk}','${row.ed}','${row.sku_qty_pcs}','${row.sku_id}')"><i class="fa fa-eye"></i></button>`
	// 		// 		} else {
	// 		// 			return `<button class="btn btn-primary" id="btn_detail_qty_ambil_sku_${row.sku_id}" onclick="modalDetailQtyAmbilSKU(${currentCounterRow}, '${row.sku_stock_id}', '${row.sku_konversi_group}', '${row.sku_satuan}', '${row.sku_nama_produk}','${row.ed}','${row.sku_qty_pcs}','${row.sku_id}')"><i class="fa fa-eye"></i></button>`
	// 		// 		}
	// 		// 	},
	// 		// 	orderable: false,
	// 		// 	className: 'text-center'
			...
			success: function(response) {
				$('#tableListSKU').DataTable({
					...
					columns: [{
						...
						{
							data: function(row, type, val, meta) {
								var check = mutasiAntarDepoDetail.find((item) => item.sku_konversi_group == row.sku_konversi_group && item.ed == row.sku_expired_date);

								if (check) {
									return `<button class="btn btn-success" id="btn_detail_qty_ambil_sku_${row.sku_konversi_group}_${row.sku_expired_date}" onclick="modalDetailQtyAmbilSKU(${currentCounterRow}, '${row.principle_kode}', '${row.sku_konversi_group}', '${row.composite}', '${row.sku_nama_produk}','${row.sku_expired_date}','${row.QtyInPcs}')"><i class="fa fa-eye"></i></button>`
								} else {
									return `<button class="btn btn-primary" id="btn_detail_qty_ambil_sku_${row.sku_konversi_group}_${row.sku_expired_date}" onclick="modalDetailQtyAmbilSKU(${currentCounterRow}, '${row.principle_kode}', '${row.sku_konversi_group}', '${row.composite}', '${row.sku_nama_produk}','${row.sku_expired_date}','${row.QtyInPcs}')"><i class="fa fa-eye"></i></button>`
								}
							},
							orderable: false,
							className: 'text-center'
	...
	function modalDetailQtyAmbilSKU(currentCounterRow, principle_kode, sku_konversi_group, composite, sku_nama_produk, ed, sku_qty_pcs) {
		$("#title_sku").html(sku_nama_produk + ' | ' + ed);
		$("#total_pcs").val(sku_qty_pcs);
		$("#sku_konversi_group").val(sku_konversi_group);
		$("#ed_produk").val(ed);
		$("#principle_kode").val(principle_kode);
		$("#currentCounterRow").val(currentCounterRow);

		$.ajax({
			type: 'POST',
			url: "<?= base_url('WMS/Transaksi/MutasiAntarUnit/PersiapanMutasiAntarUnit/GetSKUSatuan') ?>",
			data: {
				sku_konversi_group
			},
			dataType: 'JSON',
			success: function(response) {
				$(`#tableskusatuan tbody`).empty();

				$.each(response, function(i, v) {
					var item = mutasiAntarDepoDetail.find(item => item.sku_id == v.sku_id && item.ed == ed);
					$(`#tableskusatuan tbody`).append(`
						<tr>
							<td hidden>
							<input id="sku_id" value="${v.sku_id}" />
							<input id="sku_kode" value="${v.sku_kode}" />
							<input id="sku_nama_produk" value="${sku_nama_produk}" />
							<input id="sku_konversi_faktor" value="${parseInt(v.sku_konversi_faktor)}" />
							</td>
							<td>${v.sku_satuan}</td>
							<td><input type="number" class="form-control" value="${item ? item.qtyAmbil : ''}"></td>
						</tr>
						`)
				})

				$(`#modalDetailQtyAmbilSKU`).modal('show');
	...
	function btnSaveDetail() {
		var cek = 0;
		var total = 0;
		var total_pcs = parseInt($("#total_pcs").val());
		var sku_konversi_group = $("#sku_konversi_group").val();
		var ed = $("#ed_produk").val();
		var principle_kode = $("#principle_kode").val();
		var total_pcs_induk = parseInt($("#total_pcs_induk").val());
		var counter = parseInt($("#currentCounterRow").val())

		mutasiAntarDepoDetail = mutasiAntarDepoDetail.filter((item) => item.sku_konversi_group != sku_konversi_group || item.ed != ed);

		$(`#tableskusatuan tbody tr`).each(function() {
			var sku_id = $(this).find("td:eq(0) input[id='sku_id']").val();
			var sku_kode = $(this).find("td:eq(0) input[id='sku_kode']").val();
			var sku_nama_produk = $(this).find("td:eq(0) input[id='sku_nama_produk']").val();
			var sku_konversi_faktor = $(this).find("td:eq(0) input[id='sku_konversi_faktor']").val();
			var sku_satuan = $(this).find("td:eq(1)").text();
			var qtyAmbil = parseInt($(this).find("td:eq(2) input").val());

			if (parseInt(qtyAmbil) > 0) {
				...
				total += parseInt(sku_konversi_faktor) * parseInt(qtyAmbil)
				cek++;
			}
		})
		...
		mutasiAntarDepoDetail.forEach(item => {
			if (item.sku_konversi_group === sku_konversi_group && item.ed === ed) {
				item.sisa_total_pcs = parseInt(total_pcs) - parseInt(total)
			}
		})

		if (cek == 0) {
			$(`#btn_detail_qty_ambil_sku_${sku_konversi_group}_${ed}`).addClass('btn-primary');
			$(`#btn_detail_qty_ambil_sku_${sku_konversi_group}_${ed}`).removeClass('btn-success');
		} else {
			$(`#btn_detail_qty_ambil_sku_${sku_konversi_group}_${ed}`).removeClass('btn-primary');
			$(`#btn_detail_qty_ambil_sku_${sku_konversi_group}_${ed}`).addClass('btn-success');
		}

		$(`#modalDetailQtyAmbilSKU`).modal('hide');
	...
	const handlerChoosenSKUInChecked = (event) => {
		...
		mutasiAntarDepoDetail = mutasiAntarDepoDetail.filter((item) => item.qtyAmbil != 0);
				...
				$("#initDetailSKUMutasiAntarDepo tbody").append(`
							<tr>
									...
									<?php if ($this->input->get('mode') !== 'view') { ?>
										<td width="10%" class="text-center">
												<i class="fas fa-times text-danger btnCancelDetailSKUMutasiDepo${currentCounterRow}${value.sku_id}${value.ed}" onclick="handlerCancelDetailSKUMutasiDepo('${currentCounterRow}', '${value.sku_id}', '${value.ed}')" style="cursor: pointer;font-size: 1.2em;margin-right:5px;display:none"></i>
												<i class="fas fa-save text-success btnSaveDetailSKUMutasiDepo${currentCounterRow}${value.sku_id}${value.ed}" onclick="handlerSimpanDetailSKUMutasiDepo('${currentCounterRow}', '${value.sku_id}', '${value.ed}', '${value.sku_konversi_group}')" style="cursor: pointer;font-size: 1.2em;margin-right:5px;display:none"></i>
												<i class="fas fa-pencil text-warning btnEditDetailSKUMutasiDepo${currentCounterRow}${value.sku_id}${value.ed}" onclick="handlerEditDetailSKUMutasiDepo('${currentCounterRow}', '${value.sku_id}', '${value.ed}')" style="cursor: pointer;font-size: 1.2em;margin-right:5px"></i>
												<i class="fas fa-trash text-danger" onclick="handlerDeleteDetailSKUMutasiDepo('${currentCounterRow}', '${value.sku_id}', '${value.ed}')" style="cursor: pointer;font-size: 1.2em"></i>
										</td>
									<?php } ?>
	...
	const handlerSimpanDetailSKUMutasiDepo = (currentCounterRow, sku_id, ed, sku_konversi_group) => {
		const newQtyAmbil = $(`#qtyAmbilDetail${currentCounterRow}${sku_id}${ed}`).val() === '' ? 0 : parseInt($(`#qtyAmbilDetail${currentCounterRow}${sku_id}${ed}`).val());

		if (newQtyAmbil === 0) return message('Error!', 'Qty ambil tidak boleh kosong atau 0', 'error');
		...
		const sisa_pcs = parseInt(findDataSKU.sisa_total_pcs) + (parseInt(findDataSKU.qtyAmbil) * parseInt(findDataSKU.sku_konversi_faktor))

		if ((parseInt(newQtyAmbil) * parseInt(findDataSKU.sku_konversi_faktor)) > sisa_pcs) return message('Error!', `Qty Ambil melebih Qty Sistem`, 'error');

		mutasiAntarDepoDetail[findIndexDataSKU]['qtyAmbil'] = newQtyAmbil

		mutasiAntarDepoDetail.forEach(item => {
			if (item.sku_konversi_group === sku_konversi_group && item.ed === ed) {
				item.sisa_total_pcs = sisa_pcs - (parseInt(newQtyAmbil) * parseInt(findDataSKU.sku_konversi_faktor))
			}
		})
				...
				if (response.mutasiDepoDetail2) {
					response.mutasiDepoDetail2.map((childMutasiDepo, idx) => {
						if (value.tr_mutasi_depo_detail_id === childMutasiDepo.tr_mutasi_depo_detail_id) {
							...
							mutasiAntarDepoDetail.push({
								principle_kode: childMutasiDepo.principle_kode,
								sku_konversi_group: childMutasiDepo.sku_konversi_group,
								sku_id: childMutasiDepo.sku_id,
								sku_kode: childMutasiDepo.sku_kode,
								sku_nama_produk: childMutasiDepo.sku_nama_produk,
								sku_satuan: childMutasiDepo.sku_satuan,
								ed: childMutasiDepo.ed,
								sku_konversi_faktor: childMutasiDepo.sku_konversi_faktor,
								qtyAmbil: childMutasiDepo.qtyAmbil,
								total_pcs: parseInt(childMutasiDepo.qtySistem),
								sisa_total_pcs: parseInt(childMutasiDepo.qtySistem) - parseInt(childMutasiDepo.qtyAmbilPcs),
								counter: counterData,
								type: 'fix'
							})
						}

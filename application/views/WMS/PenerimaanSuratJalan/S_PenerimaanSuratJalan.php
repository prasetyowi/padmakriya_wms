<script type="text/javascript">
    ...
    function select2() {
        $(".select2:not(.select2All)").select2({
            width: "100%"
        });
    }
    ...
    function get_tipe_gudang() {
        $.ajax({
            ...
            success: function(data) {
                $('.tipe_gudang').empty();
                $('.tipe_gudang').append('<option value="">--Pilih Tipe Gudang--</option>');
                $.each(data, function(i, v) {
                    $('.tipe_gudang').append('<option value="' + v.tipegudang_kode + '">' + v.tipegudang_nama + '</option>');
                });
            }
        });
    }
    ...
    $("#principle").on("change", function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.principle_use_segment != '1') {
                    $("#all_tipe_segmen").val('8B77F52B-79AA-4599-9374-0E2D2DC9A854').trigger('change');
                    $("#all_tipe_segmen").prop('disabled', true);
                    $("#principle_use_segmen").val(0);
                } else {
                    $("#all_tipe_segmen").prop('disabled', false);
                    $("#principle_use_segmen").val(1);
                }
            }
        });
    ...
    function append_list_data_setelah_pilih_sku(response, arrSkuId) {
        $.each(response.data, function(i, v) {
            ...
            var use_segmen = $("#principle_use_segmen").val();
            ...
            $("#table_list_data_sku > tbody").append(`
                <tr>
                    ...
                    <td>
                        <select ${use_segmen != '1' ? 'disabled' : ''} class="form-control select2 tipe_segment" name="tipe_segment" id="tipe_segment" required></select>
                    </td>
    ...
    function get_data_tipe_segment(id) {
        let id_ = id == undefined ? $('#all_tipe_segmen').val() : id;
    ...
    $("#all_tipe_segmen").on("change", function() {
        let txt = $(this).children("option").filter(":selected").text();
        let txt_id = $(this).val();

        get_data_tipe_segment(txt_id);
    });
    ...
    $(document).on('click', '#simpan-penerimaan', function() {
        ...
        let tipe_gudang = $('#tipe_gudang').val();

<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$("#tipe_gudang_edit").val(data.tipe_gudang_kode).trigger('change.select2');
	...
	function append_edit_data_detail(data) {
		$.each(data.data, function(i, v) {
			...
			var output = ''
			$.each(data.data2, function(i2, v2) {
				if (v.client_pt_segmen_id == v2.id) {
					output += '<option value="' + v2.id + '" selected>' + v2.nama + '</option>';
				} else {
					output += '<option value="' + v2.id + '">' + v2.nama +
						'</option>';
				}
			})

			var use_segmen = $("#principle_use_segmen_edit").val();
          ...
          <tr>
			...
			<td>
					<select ${use_segmen != '1' ? 'disabled' : ''} class="form-control select2 tipe_segment_edit" name="tipe_segment_edit" id="tipe_segment_edit" required>
						${output}
					</select>
			</td>
	...
	function get_data_tipe_segmen_edit(id) {
		let id_ = id == undefined ? $('#all_tipe_segmen_edit').val() : id;

		$.ajax({
			url: "<?= base_url('WMS/PenerimaanSuratJalan/get_data_tipe_segment') ?>",
			type: "GET",
			dataType: "JSON",
			success: function(data) {
				$('.tipe_segment_edit').empty();
				let output = '';
				let arr_temp_tipe_id = [];
				$.each(data, function(i, v) {
					if (typeof id_ !== 'undefined') {
						if (id_ == v.id) {
							output += '<option value="' + v.id + '" selected>' + v.nama + '</option>';
						} else {
							output += '<option value="' + v.id + '">' + v.nama +
								'</option>';
						}
					} else {
						output += '<option value="' + v.id + '">' + v.nama + '</option>';
					}
				});

				$('.tipe_segment_edit').append(output);
			}
		});
	}

	$("#all_tipe_segmen_edit").on("change", function() {
		let txt = $(this).children("option").filter(":selected").text();
		let txt_id = $(this).val();

		get_data_tipe_segmen_edit(txt_id);
	});
	...
	function append_list_data_setelah_pilih_sku(response, arrSkuId) {
		$.each(response.data, function(i, v) {
			...
			var output = ''
			$.each(response.data2, function(i2, v2) {
				output += '<option value="' + v2.id + '">' + v2.nama +
					'</option>';
			})
	...
	function get_data_tipe_gudang() {
		$.ajax({
			url: "<?= base_url('WMS/PenerimaanSuratJalan/get_tipe_gudang') ?>",
			type: "GET",
			dataType: "JSON",
			async: false,
			success: function(data) {
				$('.tipe_gudang_edit').empty();
				$('.tipe_gudang_edit').append('<option value="">--Pilih Tipe Gudang--</option>');
				$.each(data, function(i, v) {
					$('.tipe_gudang_edit').append('<option value="' + v.tipegudang_kode + '">' + v.tipegudang_nama + '</option>');
				});
			}
		});
	}
	...
	$(document).on('click', '#edit-data', function() {
		...
		let tipe_gudang = $('#tipe_gudang_edit').val();
		...
		if (perusahaan == '') {
			...
			if (file == "") {
				...
				data(arr_sku_id, arr_ed, arr_batch_no, arr_sku_tipe, arr_jml_brng, arr_sku_tipe_segmen)
			}
		} else {
			data(arr_sku_id, arr_ed, arr_batch_no, arr_sku_tipe, arr_jml_brng, arr_sku_tipe_segmen)
		}
		...
		if (error.val() != 0) {
			...
			if (arr_sku_id != null) {
				for (let index = 0; index < arr_sku_id.length; index++) {
					arr_detail_psj.push({
						...
						'sku_tipe_segmen': arr_sku_tipe_segmen[index],
			...
			new_form.append('tipe_gudang', tipe_gudang);
		...
		function data(arr_sku_id, arr_ed, arr_batch_no, arr_sku_tipe, arr_jml_brng, arr_sku_tipe_segmen) {
			$("#table_list_data_sku_edit > tbody tr").each(function(index, value) {
				...
				let sku_tipe_segmen = $(this).find("td:eq(8) select").children("option").filter(":selected");
				...
				if (ed.val() == '') {
					...
					sku_tipe_segmen.map(function() {
						arr_sku_tipe_segmen.push($(this).val());
					}).get();

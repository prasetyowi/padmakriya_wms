<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$("#tipe_gudang_view").val(data.tipe_gudang_kode).trigger('change.select2');
	...
	function get_data_tipe_gudang() {
		$.ajax({
			...
			async: false,
			success: function(data) {
				$('.tipe_gudang_view').empty();
				$('.tipe_gudang_view').append('<option value="">--Pilih Tipe Gudang--</option>');
				$.each(data, function(i, v) {
					$('.tipe_gudang_view').append('<option value="' + v.tipegudang_kode + '">' + v.tipegudang_nama + '</option>');
				});
			}
		});
	}
	...
	function append_view_data_detail(data) {
		$.each(data.data, function(i, v) {
          ...
          <tr>
			  ...
			  <td>
					<select disabled class="form-control select2 tipe_segment_edit" name="tipe_segment_edit" id="tipe_segment_edit" required>
						${output}
					</select>
			</td>
          </tr>

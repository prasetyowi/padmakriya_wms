<script type="text/javascript">	
	...
	function AturStock( SKUInduk_ID, SKUInduk_Nama )
	{
		$("#hdaturstock_SKUInduk_ID").val( SKUInduk_ID );
		$("#txtaturstock_SKUInduk_Nama").val( SKUInduk_Nama );
		...
		{
			type: 'POST',    
			url: "<?= base_url('SKUHadiah/GetSKUHadiah_AturStockMenu') ?>",
			data: 
			{
	...
	function ChAturStockMenu( JSONAturStock )
	{
		...
		for( i=0 ; i< AturStock.AturStock_SKUMenu.length ; i++ )
		{
			...
			/*
			strsku = strsku + '<div class="form-group">';
			strsku = strsku + '		<input type="hidden" class="hdaturstock_sku_id" value="'+ SKU_ID +'" />';
			strsku = strsku + '		<div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 col-xl-7 nopadding">';
			strsku = strsku + '			<input type="text" class="form-control" value="'+ SKU_NamaProduk +'" disabled />';
			strsku = strsku + '		</div>';
			strsku = strsku + '		<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2 col-xl-2 nopadding">';
			strsku = strsku + '			<input type="number" class="form-control txtaturstock_MinStock" value="0" />';
			strsku = strsku + '		</div>';
			strsku = strsku + '		<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2 col-xl-2 nopadding">';
			strsku = strsku + '			<input type="number" class="form-control txtaturstock_TotalStock" value="0" disabled />';
			strsku = strsku + '		</div>';
			strsku = strsku + '		<div class="col-xs-1 col-sm-1 col-md-1 col-lg-1 col-xl-1 nopadding">';
			strsku = strsku + '			<button type="button" class="btn btn-primary form-control" onclick="ViewDetailAturStock('+ i +')"><i class="fa fa-eye"></i></button>';
			strsku = strsku + '		</div>';
			strsku = strsku + '</div>';
			*/
		...
		function()
		{
			...
			else
			{
				...
				else
				{
					...
					{
						type: 'POST',    
						url: "<?= base_url('SKUHadiah/SimpanAturStok') ?>",
						data: 
						{
							SKUInduk_ID : SKUInduk_ID,

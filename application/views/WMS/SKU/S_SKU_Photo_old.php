<script type="text/javascript">	
	...
	function ChUploadPhotoMenu( JSONPhoto )
	{
		...
		if( Photo.PhotoMenu != 0)
		{
			...
			{	
				var photo_keterangan 	= Photo.PhotoMenu[i].photo_keterangan;
				var photo_id 			= Photo.PhotoMenu[i].photo_id;
				var photo_detail_id 	= Photo.PhotoMenu[i].photo_detail_id;
				var photo_detail_dir 	= Photo.PhotoMenu[i].photo_detail_dir;
				var photo_detail_urut 	= Photo.PhotoMenu[i].photo_detail_urut;
				
				var filetype = photo_detail_dir.split('.');
				filetype = filetype[1];
				...
				SP_photo_id = photo_id;
				if( photo_detail_dir == "" ){	var Photos = '<?= base_url() ?>assets/images/img.png';	}
				else				{	var Photos = '<?= base_url() ?>'+ photo_detail_dir;				}
				...
				str = str + '					<image class="up_imgs" src="<?= base_url() ?>'+ photo_detail_dir +'?'+ new Date().getTime() +'" />';
				...
				str = str + '		<input type="hidden" id="txtup_images_name_'+ Rand +'" 		class="txtup_images_name" value="'+ photo_detail_dir +'" />';
				str = str + '		<input type="hidden" id="txtup_images_filetype_'+ Rand +'" 	class="txtup_images_filetype" value="'+ filetype +'" />';
				
				str = str + '		<input type="hidden" id="txtup_images_photoid_'+ Rand +'" 	class="txtup_images_photoid" value="'+ photo_id +'" />';
				str = str + '		<input type="hidden" id="txtup_images_photo1id_'+ Rand +'" 	class="txtup_images_photo1id" value="'+ photo_detail_id +'" />';
				
				str = str + '		<input type="hidden" id="txtup_images_RandID_'+ Rand +'" 	class="txtup_images_RandID" value="'+ Rand +'" />';
				
				if( photo_detail_urut == 0 )	{	var isutama = 1;	}
				else 					{	var isutama = 0;	}
		...
		if( filetype == 'image/png' || filetype == 'image/jpg' || filetype == 'image/jpeg' )
		{
			if (input.files && input.files[0]) 
			{
				...
				reader.onload = function(e) 
				{
					...
					str = str + '		<input type="hidden" id="txtup_images_photoid_'+ Rand +'" 	class="txtup_images_photoid" value="" />';
					str = str + '		<input type="hidden" id="txtup_images_photo1id_'+ Rand +'" 	class="txtup_images_photo1id" value="" />';
	...
	*/
	...
	(
		function()
		{
			...
			var lphotou = $(".txtup_images_name").length;
	
			var IsLoad = $(".txtup_images_isfromdb").eq(0).val();
			...
			if( IsLoad == 0 )
			{
				...
				var photo = $(".txtup_images_name").eq(0).val();
				
				var filetype = $(".txtup_images_filetype").eq(0).val();
	...
	function DeletePhoto( Idx, photo_detail_dir, Mode, ID )
	{	
		var strisu = '';
		...
		{
			SP_PhotoIdx = Idx;
			SP_photo_detail_dir = photo_detail_dir;
		...
		else
		{
			if( ID == 0 )
			{
				...
				$("#ups_Utama_"+ photo_detail_dir ).remove();
			}
			else if( ID == 1 )
			{
				$("#ups_"+ SP_PhotoIdx ).html('');
				$("#ups_"+ photo_detail_dir ).remove();
			}
		}
	}
	...
	(
		function()
		{
			...
			$.ajax(
			{
				...
				data: {
						sku_induk_id : SP_sku_induk_id ,
						photo_detail_dir : SP_photo_detail_dir

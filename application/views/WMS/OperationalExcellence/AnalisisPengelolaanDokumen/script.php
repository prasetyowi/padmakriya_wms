<script>
	...
	function GetPrincipleByPerusahaan() {
		...
		$.ajax({
			type: 'GET',
			url: "<?= base_url('WMS/OperationalExcellence/AnalisisPengelolaanDokumen/GetPrincipleByPerusahaan') ?>",
			data: {
	...
	const handlerFilterData = (event) => {
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url('WMS/OperationalExcellence/AnalisisPengelolaanDokumen/Get_laporan_monitoring_delivery_order') ?>",
			data: {
			...
			success: function(response) {
				...
				if (response.Kolom.length != 0) {
					...
					$('#table_laporan_monitoring').fadeOut("slow", function() {
						...
						if (response.AnalisisPengelolaanDokumen.length != 0) {

							$.each(response.AnalisisPengelolaanDokumen, function(i, v) {
								tbody_str += `<tr>
									<td style="vertical-align: middle;text-align:center;">${v.urutan}</td>

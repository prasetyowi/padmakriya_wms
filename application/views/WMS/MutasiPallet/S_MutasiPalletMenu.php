<script type="text/javascript">
	...
	function ChPencarianMutasiPalletTable(JSONMutasiPallet) {
		...
		if (MutasiPallet.MutasiPallet != 0) {
			...
			for (i = 0; i < MutasiPallet.MutasiPallet.length; i++) {
				...
				if (tr_mutasi_pallet_status == 'Completed' || tr_mutasi_pallet_status == 'canceled') {
					strmenu = strmenu + '	<td class="text-center"><a href="<?= base_url(); ?>WMS/MutasiPallet/MutasiPalletDetail/?tr_mutasi_pallet_id=' + tr_mutasi_pallet_id + '" class="btn btn-success" target="_blank"><i class="fa fa-eye"></i></a></td>';
				} else {
					strmenu = strmenu + '	<td class="text-center"><a href="<?= base_url(); ?>WMS/MutasiPallet/MutasiPalletDetail/?tr_mutasi_pallet_id=' + tr_mutasi_pallet_id + '" class="btn btn-primary" target="_blank"><i class="fa fa-pencil"></i></a><button class="btn btn-warning btn-small" title="Batalkan Data" onclick="btnBatal(\'' + tr_mutasi_pallet_id + '\')"><i class="fa fa-close"></i></button></td>';
				}
	...
	function btnBatal(tr_mutasi_pallet_id) {
		Swal.fire({
			...
			if (result.value == true) {
				$.ajax({
					type: 'POST',
					url: "<?= base_url('WMS/MutasiPallet/batalkanData') ?>",
					data: {
						tr_mutasi_pallet_id: tr_mutasi_pallet_id,
					},
					dataType: "JSON",
					success: function(response) {
						if (response == 1) {

<script>
						...
						$.each(response, function(i, v) {
							...
							$("#table_export_fdjr > tbody").append(`
								<tr>
									...
									<td style="vertical-align: middle;text-align:center;"><button class="btn btn-primary" onclick="getDetail('${v.sales_eksternal_id}', '${v.delivery_order_batch_tanggal_kirim}','${v.kendaraan_nopol}', '${v.rit}', '${v.gudang}')"><i class="fa fa-eye"></i></button></td>
								</tr>
							`);
						});
	...
	function getDetail(sales_eksternal_id, delivery_order_batch_tanggal_kirim, kendaraan_nopol, rit, gudang) {
		$("#title_sj").text(sales_eksternal_id);
		$.ajax({
			type: "POST",
			url: "<?= base_url('WMS/ExportData/Bosnet/Get_bosnet_do_by_id') ?>",
			data: {
				karyawan_eksternal_id: sales_eksternal_id,
				tanggal: delivery_order_batch_tanggal_kirim,
			...
			success: function(response) {
				...
				if ($.fn.DataTable.isDataTable('#table_detail_fdjr_bosnet')) {
					$('#table_detail_fdjr_bosnet').DataTable().clear();
					$('#table_detail_fdjr_bosnet').DataTable().destroy();
				}

				$('#table_detail_fdjr_bosnet > tbody').empty('');

				// $('#table_detail_fdjr_bosnet').fadeOut("slow", function() {
				// 	$(this).hide();
				...
				if (response.length != 0) {
					...
					$.each(response, function(i, v) {
						tgl_kirim = v.delivery_order_batch_tanggal_kirim;
						karyawan_nama = v.karyawan_nama;
						...
						$("#table_detail_fdjr_bosnet > tbody").append(`
								<tr>
									<td style="vertical-align: middle;text-align:center;">${v.sales_eksternal_id}</td>
									<td style="vertical-align: middle;text-align:center;">${v.delivery_order_batch_tanggal_kirim}</td>
									<td style="vertical-align: middle;text-align:center;">${v.sales_order_no_po}</td>
					...
					$("#table_detail_fdjr_bosnet").DataTable({
						paging: false,
						ordering: false,
						info: false,
						searching: false,
						...
						buttons: [{
							...
							// }, 
							{
								...
								text: '<i class="fa fa-file-excel-o"></i> Excel',
								className: 'btn btn-success',
								filename: `${tgl_kirim}_${karyawan_nama}_${rit}`
							},
							{
								extend: 'csvHtml5',
								text: '<i class="fa fa-file"></i> CSV',

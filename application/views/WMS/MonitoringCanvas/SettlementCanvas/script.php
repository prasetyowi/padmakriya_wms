<script type="text/javascript">
    ...
    function GetSuratTugasPengirimanMenu() {
        $.ajax({
            type: 'POST',
            url: "<?= base_url('WMS/MonitoringCanvas/SettlementCanvas/GetSuratTugasPengirimanMenu') ?>",
    ...
    $("#btnviewfdjr").click(function() {
        ...
        $.ajax({
            // async: false,
            type: 'POST',
            url: "<?= base_url('WMS/MonitoringCanvas/SettlementCanvas/get_settlement_pengiriman_by_filter') ?>",
            data: {
                Tgl_FDJR: Tgl_FDJR,
                No_FDJR: No_FDJR,
                        ...
                        $.each(response, function(i, v) {
                            if (arr[v.delivery_order_batch_kode]) {
                                if (cek_do_batch_kode == v.delivery_order_batch_kode) {
                                    ...
                                    // if (v.settlement_status == 'Settlement Success') {
                                    //     cbdisabled = 'disabled';
                                    ...
                                    $("#tablefdjrmenu > tbody").append(`
                                            <tr>
                                                <td class="text-center" style="vertical-align: middle;" rowspan="${arr[v.delivery_order_batch_kode]}">
                                                    <input type="checkbox" name="CheckboxFDJR_${cbnumb}" id="check_fdjr_${i}" data-LastUpdate="${v.delivery_order_batch_update_tgl}" value="${v.delivery_order_batch_id}" ${cbdisabled}>
                                                    <input type="hidden" id="fdjr_kode_${i}" value="${v.delivery_order_batch_kode}">
                                                </td>
                                                ...
                                                <td class="text-center" style="vertical-align: middle;" rowspan="${arr[v.delivery_order_batch_kode]}">
                                                    <a href="<?php echo base_url(); ?>WMS/MonitoringCanvas/SettlementCanvas/SettlementMenu/?delivery_order_batch_id=${v.delivery_order_batch_id}" class="btn btn-primary" target="_blank">Settlement</a>
                                                </td>
                                            </tr>
								        `);
                                }
                ...
                for (var i = 0; i < jml_fdjr; i++) {
                    ...
                    if (checked > 0) {
                        ...
                        $.ajax({
                            async: false,
                            type: 'GET',
                            url: "<?= base_url('WMS/MonitoringCanvas/SettlementCanvas/get_settlement_status_by_fdjr') ?>",
                            data: {
                                fdjr_id: fdjr_id
                            },
                            dataType: "JSON",
                            success: function(response) {
                                ...
                                if (response.PenerimaanTunai != 0) {
                                    $.each(response.PenerimaanTunai, function(i, v) {
                                        ...
                                        if (i == response.PenerimaanTunai.length - 1) {
                                            if (nilai_tunai == 0) {
                                                cek_tunai++;
                                            }
                                        }
                                ...
                                if (cek_error == 0) {
                                    requestAjax(
                                        "<?= base_url('WMS/MonitoringCanvas/SettlementCanvas/CekLastUpdateFDJR'); ?>", {

					...
					<div class="row">
						<div class="col-xs-4">
							<div class="form-group field-konfirmasipengirimancanvas-delivery_order_batch_kode">
								...
								<input disabled type="text" id="konfirmasipengirimancanvas-delivery_order_batch_kode" class="form-control" name="konfirmasipengirimancanvas[delivery_order_batch_kode]" autocomplete="off" value="<?= $header->delivery_order_batch_kode ?>">
							</div>
						</div>
						<div class="col-xs-4">
							<div class="form-group field-konfirmasipengirimancanvas-delivery_order_kode">
								...
								<input disabled type="text" id="konfirmasipengirimancanvas-delivery_order_kode" class="form-control" name="konfirmasipengirimancanvas[delivery_order_kode]" autocomplete="off" value="<?= $header->delivery_order_kode ?>">
								<input disabled type="hidden" id="konfirmasipengirimancanvas-delivery_order_batch_id" class="form-control" name="konfirmasipengirimancanvas[delivery_order_batch_id]" autocomplete="off" value="<?= $header->delivery_order_batch_id ?>">
								<input disabled type="hidden" id="konfirmasipengirimancanvas-delivery_order_id" class="form-control" name="konfirmasipengirimancanvas[delivery_order_id]" autocomplete="off" value="<?= $header->delivery_order_id ?>">
								<input disabled type="hidden" id="konfirmasipengirimancanvas-delivery_order_update_tgl" class="form-control" name="penerimaanpenjualan[delivery_order_update_tgl]" autocomplete="off" value="<?= $header->delivery_order_update_tgl ?>">
							</div>
						</div>
						<div class="col-xs-4">
							<div class="form-group field-konfirmasipengirimancanvas-delivery_order_kode">
								...
								<input disabled type="text" id="konfirmasipengirimancanvas-sales_order_no_po" class="form-control" name="konfirmasipengirimancanvas[sales_order_no_po]" autocomplete="off" value="<?= $header->sales_order_no_po ?>">
							</div>
						</div>
					</div>
					<div class="row">
						...
						<div class="col-xs-6">
							<div class="form-group field-konfirmasipengirimancanvas-karyawan_id">
								...
								<select id="konfirmasipengirimancanvas-karyawan_id" class="form-control select2" name="konfirmasipengirimancanvas[karyawan_id]" <?= $header->karyawan_id != '' ? 'disabled' : ''; ?>>
									<?php foreach ($checkers as $row) : ?>
										<option value="<?= $row->karyawan_id ?>" <?= $header->karyawan_id == $row->karyawan_id ? 'selected' : ''; ?>><?= $row->karyawan_nama ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-xs-6">
							<div class="form-group field-konfirmasipengirimancanvas-penerimaan_penjualan_kode">
								...
								<input disabled type="text" id="konfirmasipengirimancanvas-penerimaan_penjualan_kode" class="form-control" name="konfirmasipengirimancanvas[penerimaan_penjualan_kode]" autocomplete="off" placeholder="autogenerate" value="">
							</div>
						</div>
						<div class="col-xs-6">
							<div class="form-group field-konfirmasipengirimancanvas-penerimaan_penjualan_tgl">
								<label for="konfirmasipengirimancanvas-penerimaan_penjualan_tgl" class="control-label" name="CAPTION-TGLBTB">Tanggal BTB</label>
								<input type="text" id="konfirmasipengirimancanvas-penerimaan_penjualan_tgl" class="form-control datepicker" name="konfirmasipengirimancanvas[penerimaan_penjualan_tgl]" autocomplete="off" value="<?= $header->penerimaan_penjualan_tgl ?>" disabled>
							</div>
						</div>
						<div class="col-xs-6">
							<div class="form-group field-konfirmasipengirimancanvas-depo_detail_id">
								...
								<select id="konfirmasipengirimancanvas-depo_detail_id" class="form-control select2" name="konfirmasipengirimancanvas[depo_detail_id]">
					...
					<table id="tabledoretur" width="100%" class="table table-bordered">
						<thead>
							<tr class="bg-info">
								...
								<th class="text-center"><span>QTY Terima</span></th>
							...
							if ($detail != "0") {
								...
								foreach ($detail as $detail) : ?>
									<tr>
										...
										<td class="text-center" width="15%"><?= $detail->principle ?></td>
										<td class="text-center" width="15%"><?= $detail->sku_konversi_group ?> / <?= $detail->sku_satuan ?></td>
										<td class="text-center" width="25%"><?= $detail->sku_nama_produk ?></td>
										<td class="text-center" width="10%"><?= $detail->sku_kemasan ?></td>
										<td class="text-center" width="10%"><?= $detail->sku_satuan ?></td>
										<td class="text-center" width="10%">
											<input type="text" name="qty_terima" id="qty_terima<?= $no ?>" onchange="handlerUpdatedQtyTerima(event, '<?= $detail->sku_id ?>')" class="form-control numeric qty_terima<?= $no ?>">
										</td>
										<td class="text-center" width="10%">
											<button class="btn btn-danger btn-sm btnRemoveListDO" data-id="<?= $detail->sku_id ?>"><i class="fas fa-trash"></i></button>
										</td>
									</tr>
							<?php $no++;
								endforeach;
			...
			<div class="panel panel-default col-xs-12 col-sm-12 col-md-8 col-lg-8 col-xl-8" id="viewpalletdoretur">
				...
				<div class="panel-body form-horizontal form-label-left" style="margin-left:-20px;margin-right:-20px;">
					<table id="tablepallet" width="100%" class="table table-bordered">
						<thead>
							<tr class="bg-info">
								<th class="text-center" style="width: 40%;" colspan="2" name="CAPTION-LOKASIBINTUJUAN">Lokasi Bin Tujuan</th>
		...
		<div class="row" style="margin-bottom:10px;margin-left:5px;margin-right:5px;">
			<div class="panel panel-default col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12" id="viewdetailpallet" style="display: none;">
				...
				<div class="panel-body form-horizontal form-label-left" style="margin-left:-15px;margin-right:-20px;">
					<input type="hidden" id="konfirmasipengirimancanvasdetail-pallet_id" value="">
			...
			<div class="row pull-right" style="margin-top:20px;">
				...
				<a href="<?php echo base_url() ?>WMS/MonitoringCanvas/KonfirmasiPengirimanCanvas/formClosing/<?= $delivery_order_batch_id; ?>?mode=closing" type="button" id="btn_back" class="btn btn-danger"><i class="fa fa-undo"></i> <span name="CAPTION-BACK">Kembali</span></a>
			</div>
		</div>
	</div>
</div>

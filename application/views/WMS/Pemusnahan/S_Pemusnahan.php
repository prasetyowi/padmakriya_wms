<script>
	...
	$(document).on("click", "#search_filter_data", function() {
		...
		$.ajax({
			type: 'POST',
			url: "<?= base_url('WMS/Pemusnahan/get_data_koreksi_stok_by_filter') ?>",
			data: {
		...
		if (response.length > 0) {
			...
			$.each(response, function(i, v) {
				...
				if (v.status == "In Progress") {
					str += `<button type="button" data-href='<?= base_url('WMS/Pemusnahan/edit/') ?>${ v.id }' class="btn btn-warning edit_data"><i class="fas fa-edit"></i> Edit</button>`;
				}
				if (v.status == "Completed") {
					str += `<button type="button" data-href='<?= base_url('WMS/Pemusnahan/view/') ?>${ v.id }' class="btn btn-primary detail_data"><i class="fas fa-eye"></i> Detail</button>`;
				}

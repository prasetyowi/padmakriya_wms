<script type="text/javascript">
		...
		setTimeout(() => {
			$.ajax({
				...
				data: {
					...
					client_wms_id: $("#filter-do-perusahaan").val(),
					principle_id: $("#filter-do-principle").val()
				},
				dataType: "JSON",
				success: function(response) {
	...
	function ErrorMsgDOMasal() {
		...
		console.log(arr_list_do_msg_error);

		$("#modal-pesan-error").modal('show');

		if (arr_list_do_msg_error.length > 0) {
			...
			$('#table-pesan-error').fadeOut("slow", function() {
				...
				// console.log(response);
				$.each(arr_list_do_msg_error, function(i, v) {
					$("#table-pesan-error > tbody").append(`
						<tr>
							<td class="text-center">${i+1}</td>
							<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}" style="text-align: center; vertical-align: middle;">${v.delivery_order_kode}</td>
							<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}" style="text-align: center; vertical-align: middle;">${v.StatusMsg}</td>
							<td class="text-center ${v.StatusMsg == '200' ? 'text-success' : 'text-danger'}" style="text-align: center; vertical-align: middle;">${v.ErrMsg}</td>
						</tr>
					`).fadeIn("slow");
				});
			...
			$("#table-pesan-error > tbody").append(`
				<tr>
					<td class="text-center" colspan="4"><strong class="text-danger">DATA NOT FOUND</strong></td>
				</tr>
			`);
		}
	}
				...
				if (result.value == true) {
					...
					Swal.fire({
						...
						if (result.value == true) {
							...
							$.ajax({
								type: 'POST',
								url: "<?= base_url('WMS/KelolaDOGagal/process_delivery_order_reschedule') ?>",
								dataType: "JSON",
								data: {
									arr_do: arr_do
								},
								...
								success: function(response) {
									...
									setTimeout(() => {
										...
										if (response.length > 0) {
											...
											$("#CAPTION-JUMLAHERRORMSG").html(response.length);

											arr_list_do_msg_error = response;
										} else {
											document.getElementById("btnerrormsg").classList.add('btn-primary');
										...
										Swal.fire({
											title: 'Success',
											text: "Reschedule process completed",
											icon: 'success',
			...
			if (result.value == true) {
				...
				Swal.fire({
					...
					if (result.value == true) {
						...
						$.ajax({
							type: 'POST',
							url: "<?= base_url('WMS/KelolaDOGagal/process_delivery_order_not_reschedule') ?>",
							dataType: "JSON",
							data: {
								arr_do: arr_do_not_reschedule
							},
	...
	function getPrincipleByPerusahaan() {
		var perusahaan_id = $("#filter-do-perusahaan").val();
		if (perusahaan_id != null && perusahaan_id != "") {
			$.ajax({
				url: "<?= base_url('WMS/KelolaDOGagal/getPrincipleByPerusahaan'); ?>",
				type: "POST",
				data: {
				...
				success: function(response) {
					$("#filter-do-principle").empty();
					$("#filter-do-principle").append('<option value=""><label name="CAPTION-SEMUA">Semua</label></option>');
					$.each(response, function(index, value) {
						$("#filter-do-principle").append('<option value="' + value.principle_id + '">' + value.principle_kode + '</option>');
					});
				},
				error: function(xhr, ajaxOptions, thrownError) {
					message("Error", "Error 500 Internal Server Connection Failure", "error");

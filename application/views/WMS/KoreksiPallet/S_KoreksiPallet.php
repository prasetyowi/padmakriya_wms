<script>
	...
	const handleSearchFilterData = () => {
		...
		postDataRequest('<?= base_url('WMS/KoreksiPallet/getDataKoreksiPalletByFilter') ?>', {
	...
	function initDataSearch(response) {
		if (response.length > 0) {
			...
			$.each(response, function(i, v) {
				let str = "";
				str += `<button type="button" data-href='<?= base_url('WMS/KoreksiPallet/view/') ?>${ v.id }' class="btn btn-primary detail_data"><i class="fas fa-eye"></i> Detail</button>`;

<script type="text/javascript">
	...
	function Get_list_komparasi_lokasi_stock_pallet() {
		...
		$.ajax({
			type: 'POST',
			url: "<?= base_url('WMS/Maintenance/KomparasiLokasiStockPalletGlobal2/Get_list_komparasi_lokasi_stock_pallet') ?>",
			beforeSend: function() {
				Swal.fire({
					title: 'Loading ...',
				...
				if (response.length != 0) {
					$.each(response, function(i, v) {
                        ...
                        <tr>
                            ...
                            <td class="text-center">${v.depodetailpallet}</td>
                            <td class="text-center">${v.depodetailsummary}</td>
                            <td class="text-center">${v.flag}</td>
                            <td class="text-center">
                                <button type="button" id="btn_edit_komparasi_lokasi_stock_pallet" class="btn btn-sm btn-warning" onclick="EditListKomparasiStockPallet('${v.iddepodetailsummary}','${v.pallet_detail_id}')"><i class="fa fa-pencil"></i></button>
                            </td>
                        </tr>
                    `);
					});
	...
	function UpdateListSKUStock(index, input) {
		...
		let total = 0;
		total = parseInt($("#item-" + index + "-KomparasiLokasiStockPalletGlobalDetail-sku_stock_qty").val()) - parseInt($(
			"#item-" + index + "-KomparasiLokasiStockPalletGlobalDetail-sku_stock_ambil").val()) + parseInt($("#item-" +
			index + "-KomparasiLokasiStockPalletGlobalDetail-sku_stock_in").val()) - parseInt($("#item-" + index +
			"-KomparasiLokasiStockPalletGlobalDetail-sku_stock_out").val()) + parseInt($("#item-" + index +
			"-KomparasiLokasiStockPalletGlobalDetail-sku_stock_terima").val());

		$("#item-" + index + "-KomparasiLokasiStockPalletGlobalDetail-qty").val(total);

		arr_list_sku_stock[index] = ({
			'pallet_detail_id': $("#item-" + index + "-KomparasiLokasiStockPalletGlobalDetail-pallet_detail_id")
				.val(),
			...
			'sku_stock_ambil': $("#item-" + index + "-KomparasiLokasiStockPalletGlobalDetail-sku_stock_ambil")
				.val(),
			...
			'sku_stock_terima': $("#item-" + index + "-KomparasiLokasiStockPalletGlobalDetail-sku_stock_terima")
				.val(),
			'qty': total
		});
	...
	function EditListKomparasiStockPallet(depo_detail_id, pallet_detail_id) {
		console.log(pallet_detail_id);
		$("#modal_edit_komparasi_lokasi_stock_pallet").modal('show');
		$("#btn_update_pallet").attr('data-palletdetailid', '')
		$("#btn_update_pallet").attr('data-palletdetailid', pallet_detail_id)
		$.ajax({
			type: 'GET',
			url: "<?= base_url('WMS/Maintenance/KomparasiLokasiStockPalletGlobal2/Get_komparasi_lokasi_stock_pallet_by_id') ?>",
			data: {
			...
			dataType: "JSON",
			success: function(response) {
				$('#table_komparasi_pallet > tbody').empty('');

				if ($.fn.DataTable.isDataTable('#table_komparasi_pallet')) {
					$('#table_komparasi_pallet').DataTable().clear();
					$('#table_komparasi_pallet').DataTable().destroy();
				}

				if (response.length != 0) {
					$.each(response, function(i, v) {
						$("#table_komparasi_pallet > tbody").append(`
                        <tr>
                            <td class="text-center" style="width: 5%;">
                                <input type="checkbox" class="form-control chk-pallet" id="chk-pallet" value="${v.pallet_id}"/>
                            </td>
                            <td class="text-center" style="vertical-align: middle;">${v.depo_detail_nama}</td>
                            <td class="text-center" style="vertical-align: middle;">${v.pallet_kode}</td>
                            <td class="text-center" style="vertical-align: middle;">${v.rak_lajur_detail_nama}</td>
                        </tr>
                    `);
					});
				}

				$("#table_komparasi_pallet").DataTable({
					lengthMenu: [
						[50, 100, 200, -1],
						[50, 100, 200, 'All'],
					],
	...
	$("#btn_update_pallet").on("click", function() {
		var checkbox = $("#chk-pallet").val();
		let pallet_detail_id = $(this).attr('data-palletdetailid')
		...
		let cb = $('.chk-pallet:checkbox:checked').val()

		if (cb === undefined) {
			messageNew('info', 'Pilih data terlebih dahulu', 'info');
			return false
		}
		setTimeout(() => {
			...
			Swal.fire({
				...
				if (result.value == true) {
					$.ajax({
						type: 'POST',
						url: "<?= base_url('WMS/Maintenance/KomparasiLokasiStockPalletGlobal2/update_pallet') ?>",
						dataType: "JSON",
						data: {
							// pallet_id: $("#KomparasiLokasiStockPalletGlobal-pallet_id").val(),
							// rak_lajur_detail_id: $(
							// 	"#KomparasiLokasiStockPalletGlobal-rak_lajur_detail").val()
							pallet_id: cb,
						...
						error: function(xhr, ajaxOptions, thrownError) {
							messageNew("Error", "Error 500 Internal Server Connection Failure",
								"error");
							$("#btn_update_pallet").prop("disabled", false);
						},
						complete: function() {

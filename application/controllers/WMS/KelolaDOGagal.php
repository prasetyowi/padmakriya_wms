...
class KelolaDOGagal extends MenuController
{
	...
	public function GetKelolaDOGagalByFilter()
	{
		...
		$totalData = $this->M_KelolaDOGagal->GetTotalKelolaDOGagalByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $search, $so_eksternal, $is_reschedule, $client_wms_id, $principle_id);

		$totalFiltered = $totalData;

		if (empty($this->input->post('search')['value'])) {
			$data = $this->M_KelolaDOGagal->GetKelolaDOGagalByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $start, $end, $search, $so_eksternal, $is_reschedule, $client_wms_id, $principle_id);
		} else {
			$search = $_POST['search']['value'];
			$data = $this->M_KelolaDOGagal->GetKelolaDOGagalByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $start, $end, $search, $so_eksternal, $is_reschedule, $client_wms_id, $principle_id);

			$datacount = $this->M_KelolaDOGagal->GetTotalKelolaDOGagalByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $search, $so_eksternal, $is_reschedule, $client_wms_id, $principle_id);
	...
	public function GetKelolaDOGagalNotRescheduleByFilter()
	{
		...
		$data = $this->M_KelolaDOGagal->GetKelolaDOGagalNotRescheduleByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $so_eksternal, $client_wms_id, $principle_id);
	...
	public function GetKelolaDOGagalNotRescheduleBuatDraftByFilter()
	{
		...
		$data = $this->M_KelolaDOGagal->GetKelolaDOGagalNotRescheduleBuatDraftByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $so_eksternal, $client_wms_id, $principle_id);
	...
	public function GenerateDONotReschedule()
	{
		...
		foreach ($arr_do_not_reschedule as $value) {
			...
			$this->M_KelolaDOGagal->Generate_do_not_reschedule_detail($delivery_order_draft_id, $value['delivery_order_id'], $tipe_delivery_order_id);

			if ($tipe_delivery_order_id == 'C5BE83E2-01E8-4E24-B766-26BB4158F2CD') {
				$this->M_KelolaDOGagal->Generate_do_not_reschedule_detail2($delivery_order_draft_id, $value['delivery_order_id'], $tipe_delivery_order_id);
			};
			//}
		}
	...
	public function process_delivery_order_reschedule()
	{
		$data = array();
		$arr_list_do_msg_error = array();
		$arr_do = $this->input->post('arr_do');

		if (isset($arr_do) && count($arr_do) > 0) {
			foreach ($arr_do as $key => $value) {
				# code...

				$do_id = $value['do_id'];
				$tgl_kirim_ulang = $value['tgl_kirim_ulang'];
				$pengguna_username = $this->session->userdata('pengguna_username');

				$data = $this->M_KelolaDOGagal->process_delivery_order_reschedule($do_id, $tgl_kirim_ulang, $pengguna_username);

				foreach ($data as $key => $value) {
					# code...
					array_push($arr_list_do_msg_error, array("idx" => $value['idx'], "delivery_order_kode" => $value['delivery_order_kode'], "StatusMsg" => $value['StatusMsg'], "ErrMsg" => $value['ErrMsg']));
				}
			}
		}
	...
	public function process_delivery_order_not_reschedule()
	{
		$data = array();
		$arr_list_do_msg_error = array();
		$arr_do = $this->input->post('arr_do');
		...
		if (isset($arr_do) && count($arr_do) > 0) {
			foreach ($arr_do as $key => $value) {
				...
				$do_id = $value['delivery_order_id'];
				$tgl_kirim_ulang = $value['tanggal_rencana_kirim'];
				$pengguna_username = $this->session->userdata('pengguna_username');

				$data = $this->M_KelolaDOGagal->process_delivery_order_not_reschedule($do_id, $tgl_kirim_ulang, $pengguna_username);
	...
	public function getPrincipleByPerusahaan()
	{
		...
		$data = $this->M_KelolaDOGagal->getPrincipleByPerusahaan($perusahaan_id);

...
class PenerimaanSuratJalan extends MenuController
{
	...
	public function PenerimaanSuratJalanMenu()
	{
		...
		$data['tipe_segmen'] = $this->M_PenerimaanSuratJalan->get_data_tipe_segment();
			...
			foreach ($dataPost as $key => $val) {
				if ($value['sku_id'] == $val['id']) {
					array_push($result, [
						...
						'qty' => $val['qty'],
						'principle_use_segment' => $value['principle_use_segment'],
					]);
				}
			}
		}

		$data2 = $this->M_PenerimaanSuratJalan->get_data_tipe_segment();
	...
	public function get_tipe_gudang()
	{
		$result = $this->M_PenerimaanSuratJalan->get_tipe_gudang();
		echo json_encode($result);
	}
	...
	public function get_data_tipe_segment()
	{
		$result = $this->M_PenerimaanSuratJalan->get_data_tipe_segment();
		echo json_encode($result);
	}
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('tipe_gudang', 'Tipe Gudang', 'trim|required', [
			'required' => 'Tipe Gudang tidak boleh kosong!'
		]);
						...
						if (!$didUpload) {
							...
							if ($no_surat_jalan_counter == "") {
								...
								if ($check_duplicate_no_sj > 0) {
									...
									$this->saveToDb($sj_id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
								}
							} else {
								$this->saveToDb($sj_id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
							}
	...
	private function saveToDb($sj_id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang)
	{
		//generate kode
		$date_now = date('Y-m-d h:i:s');
		$param =  'KODE_SJ';
		...
		$result = $this->M_PenerimaanSuratJalan->save($sj_id, $principle, $perusahaan, $tipe_penerimaan, $generate_kode, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
		if (!$result) {
			echo json_encode(array('status' => false, 'message' => 'Data gagal disimpan!'));
		} else {
			...
			// $this->M_PenerimaanSuratJalan->updateClientPtSegmenDetail($sj_id, $item);
			...
			if (!isset($file_db)) {
								...
								if (!$didUpload) {
									...
									if ($no_surat_jalan_counter == "") {
										...
										if ($check_duplicate_no_sj > 0) {
											...
											$this->editToDb($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
										}
									} else {
										$this->editToDb($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
									}
				...
				if ($no_surat_jalan_counter == "") {
					...
					if ($check_duplicate_no_sj > 0) {
						...
						$this->editToDb($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, null, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
					}
				} else {
					$this->editToDb($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, null, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
				}
	...
	private function editToDb($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang)
	{
		if ($name_file != null) {
			$result = $this->M_PenerimaanSuratJalan->update($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $name_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
		} else {
			$result = $this->M_PenerimaanSuratJalan->update2($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang);
		}

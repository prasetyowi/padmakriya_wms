...
class PermintaanPembongkaranByQtyDO extends MenuController
{
	...
	public function __construct()
	{
		...
		// $this->MenuKode = "128001100";
		$this->load->model('M_Menu');
		$this->load->model('WMS/M_PermintaanPembongkaranByQtyDO', 'M_PermintaanPembongkaranByQtyDO');
	...
	public function PermintaanPembongkaranByQtyDOMenu()
	{
		$data = array();
		...
		$data['TipeDeliveryOrder'] = $this->M_PermintaanPembongkaranByQtyDO->GetTipeDeliveryOrder();
		$data['Gudang'] = $this->M_DepoDetail->findByDepo_array();
		$data['Principle'] = $this->M_Principle->Get_Principle();
		$data['TipeGudang'] = $this->M_PermintaanPembongkaranByQtyDO->GetTipeGudang();
		...
		$this->load->view('WMS/PermintaanPembongkaranByQtyDO/index', $data);
		$this->load->view('layouts/sidebar_footer', $data);

		$this->load->view('master/S_GlobalVariable', $data);
		$this->load->view('WMS/PermintaanPembongkaranByQtyDO/script', $data);
	}
	...
	public function getDataSearch()
	{
		...
		$data 		= $this->M_PermintaanPembongkaranByQtyDO->getDataSearch($tgl, $tipe_do, $tipe_gudang);
	...
	public function showDetailDOBySKU()
	{
		...
		$tipe_do 	= $this->input->post('filter_tipedo');
		$tgl 		= $this->input->post('filter_tanggal');
		$data 		= $this->M_PermintaanPembongkaranByQtyDO->showDetailDOBySKU($sku_id, $tgl, $tipe_do);
	...
	public function HitungQtyByKemasan()
	{
		...
		foreach ($arr_detail as $key => $value) {
			...
			$cekKonversiLevel = $this->M_PermintaanPembongkaranByQtyDO->cekKonversiLevel($sku_konversi_group, $depo_detail_id, $sku_konversi_level);
			// echo json_encode($cekKonversiLevel);
			// die;
			if (count($cekKonversiLevel) > 0) {
				// $this->M_PermintaanPembongkaranByQtyDO->insertSKUKonversiTemp($id_temp, $sku_id, $sku_expired_date, $sku_qty, $sku_qty_composite);
				$resultKonversi = $this->M_PermintaanPembongkaranByQtyDO->HitungQtyByKemasan($sku_id, (int)$value['sku_qty_draft']);
	...
	public function requestPembongkaran()
	{
		$arr_detail = $this->input->post("arr_detail");
		$this->db->trans_begin();
		...
		foreach ($arr_detail as $key => $value) {
			...
			$depoPrefix = $this->M_PermintaanPembongkaranByQtyDO->getDepoPrefix($depo_id);
			...
			$this->M_PermintaanPembongkaranByQtyDO->insert_tr_konversi_sku($id_temp, $client_wms_id, $depo_detail_id, NULL, $tr_konversi_sku_kode, NULL, NULL, NULL, NULL, NULL, 0);

			foreach ($data as $key => $value2) {
				$tr_konversi_sku_detail_id = $this->M_Vrbl->Get_NewID();
				$tr_konversi_sku_detail_id = $tr_konversi_sku_detail_id[0]['NEW_ID'];

				$getDataSKU = $this->M_PermintaanPembongkaranByQtyDO->getDataSKU($value2['sku_id']);
				$getSkuIDByKonversiGrup = $this->M_PermintaanPembongkaranByQtyDO->getSkuIDByKonversiGrup($getDataSKU['sku_konversi_group']);

				$this->M_PermintaanPembongkaranByQtyDO->insert_tr_konversi_sku_detail($tr_konversi_sku_detail_id, $id_temp, $value2, $getSkuIDByKonversiGrup['sku_id']);
			}
		}
	...
	public function Insert_tr_konversi_sku_temp()
	{
		...
		$this->M_PermintaanPembongkaranByQtyDO->Insert_tr_konversi_sku_temp($tr_konversi_sku_temp_id, $client_wms_id, $sku_id, $depo_detail_id, $sku_stock_id, $sku_stock_expired_date, $tr_konversi_sku_detail_qty_plan);
	...
	public function Get_tr_konversi_sku_temp_by_id()
	{
		...
		$data = $this->M_PermintaanPembongkaranByQtyDO->Get_tr_konversi_sku_temp_by_id($tr_konversi_sku_temp_id);
		echo json_encode($data);
	}

	public function GetListPembongkaran()
	{
		$principle = $this->input->post('principle');
		$flag = $this->input->post('flag');
		$arr_list_pembongkaran = $this->input->post('arr_list_pembongkaran');

		$data = $this->M_PermintaanPembongkaranByQtyDO->GetListPembongkaran($principle, $flag, $arr_list_pembongkaran);
		echo json_encode($data);
	}
	...
	public function Delete_tr_konversi_sku_temp_by_id()
	{
		...
		$this->M_PermintaanPembongkaranByQtyDO->Delete_tr_konversi_sku_temp_by_id($tr_konversi_sku_temp_id);

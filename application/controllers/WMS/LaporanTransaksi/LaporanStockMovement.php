...
class LaporanStockMovement extends ParentController
{
	...
	public function __construct()
	{
		...
		$this->load->model('WMS/LaporanTransaksi/M_LaporanStockMovement', 'M_LaporanStockMovement');
		$this->load->model('M_Menu');
		$this->load->model('M_Function');
		$this->load->model('M_MenuAccess');
	}

	public function LaporanStockMovementMenu()
	{
		$this->load->model('M_Menu');
		...
		$data['Gudang'] = $this->M_LaporanStockMovement->Get_Gudang();
		$data['Principle'] = $this->M_LaporanStockMovement->Get_Principle();
		$data['ClientWMS'] = $this->M_LaporanStockMovement->Get_ClientWMS();
		...
		$this->load->view('reports/LaporanTransaksi/LaporanStockMovement/main', $data);
		$this->load->view('layouts/sidebar_footer', $query);
		$this->load->view('master/S_GlobalVariable', $query);
		$this->load->view('reports/LaporanTransaksi/LaporanStockMovement/script', $data);
	...
	public function getDetail()
	{
		...
		$tgl1 = date('Y-m-d', strtotime($this->input->post('filter_stock_tanggal')));
		$tgl2 = date('Y-m-d', strtotime($this->input->post('filter_stock_tanggal')));

		$tgl_1 = date('m/d/Y', strtotime($this->input->post('filter_stock_tanggal')));
		$tgl_2 = date('m/d/Y', strtotime($this->input->post('filter_stock_tanggal')));
		...
		$total_data = $this->M_LaporanStockMovement->count_all_data($tgl1, $tgl2, $client_wms, $depo_id, $depo_detail_id, $principle_id, $sku_kode, $sku_nama, $tgl_1, $tgl_2, $mode);

		$records_filtered = $total_data;
		// $data = $this->M_LaporanStockMovement->GetDetail($start, $length, $search_value, $order_column, $order_dir, $tgl1, $tgl2, $client_wms, $depo_id, $depo_detail_id, $principle_id, $sku_kode, $sku_nama, $tgl_1, $tgl_2);
		$data = $this->M_LaporanStockMovement->GetDetailNewProcedure($start, $length, $search_value, $order_column, $order_dir, $tgl1, $tgl2, $client_wms, $depo_id, $depo_detail_id, $principle_id, $sku_kode, $sku_nama, $tgl_1, $tgl_2, $mode);
		$response = array(
			'draw' => $draw,
	...
	public function GetLaporanStockMovementDetailData()
	{
		$draw = 50;

		// $tgl = explode(" - ", $this->input->post('filter_stock_tanggal'));

		$tgl1 = date('m/d/Y', strtotime($this->input->post('filter_stock_tanggal')));
		$tgl2 = date('m/d/Y', strtotime($this->input->post('filter_stock_tanggal')));
		...
		$data_laporan_stock = $this->M_LaporanStockMovement->Get_laporan_transaksi_stock_movement_detail($tipe_report, $tgl1, $tgl2, $tipe_transaksi, $client_wms, $depo_id, $depo_detail_id, $principle_id, $sku_id, $sku_kode, $sku_nama, $sku_stock_expired_date, $newDataStockAwal, $mode);

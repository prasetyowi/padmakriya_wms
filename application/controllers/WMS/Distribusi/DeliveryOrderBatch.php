...
class DeliveryOrderBatch extends MenuController
{
	// private $MenuKode;
	...
	public function GetDeliveryOrderBatchByFilter()
	{
		...
		,FORMAT(do.delivery_order_batch_tanggal_kirim, 'yyyy-MM-dd') AS delivery_order_batch_tanggal_kirim2
		...
		,dobh.delivery_order_batch_h_id
		...
		FORMAT(do.delivery_order_batch_tanggal_kirim, 'yyyy-MM-dd'),
		do.tipe_delivery_order_id,
		do.delivery_order_batch_status,
		tipe_delivery_order.tipe_delivery_order_alias,
		karyawan.karyawan_id,
		...
		if (COUNT($response['data']) != 0) {
			foreach ($response['data'] as $key => $value) {
				...
				array_push($fixData, [
					...
					'delivery_order_batch_tanggal_kirim2' => $value['delivery_order_batch_tanggal_kirim2'],
					...
					'delivery_order_batch_h_id' => $value['delivery_order_batch_h_id'],
	...
	public function batalkan_do_batch_after_laksanakan()
	{
		$delivery_order_batch_id = $this->input->post('delivery_order_batch_id');
		$tglLastUpdate = $this->input->post('tglLastUpdate');
		...
		$this->M_DeliveryOrderBatch->deletePickingList($delivery_order_batch_id);
	...
	public function print()
	{
		...
		$data['FDJRBank'] = $this->M_DeliveryOrderBatch->GetCetakHeaderFDJRBankById($id);
	...
	public function print_surat_jalan_pengambilan_barang()
	{
		$id = $this->input->get('id');
		$data['header'] = $this->M_DeliveryOrderBatch->getHeaderDOBatch($id);
		$data['detail'] = $this->M_DeliveryOrderBatch->getDetailDOBatch($id);

		// Set headers untuk file Word
		header("Content-Type: application/vnd.ms-word");
		header("Content-Disposition: attachment;filename=Surat_Jalan_Pengambilan_Barang.doc");
		header("Cache-Control: private, no-store, no-cache, must-revalidate, post-check=0, pre-check=0");
		header("Pragma: no-cache");

		$this->load->view('WMS/DeliveryOrderBatch/view_cetak_surat_jalan_pengambilan_barang', $data);
	}
	...
	public function saveUbahDriverKendaraan()
	{
		$do_batch_h_id_asal = $this->input->post('do_batch_h_id_asal');
		$do_batch_id = $this->input->post('do_batch_id');
		$last_update = $this->input->post('last_update');
		$kendaraan = $this->input->post('kendaraan');
		$driver = $this->input->post('driver');
		$mode = $this->input->post('mode');
		$do_batch_h_id_tujuan = $this->input->post('do_batch_h_id_tujuan');

		$lastUpdatedChecked = checkLastUpdatedData((object) [
			'table' => "delivery_order_batch",
			'whereField' => "delivery_order_batch_id",
			'whereValue' => $do_batch_id,
			'fieldDateUpdate' => "delivery_order_batch_update_tgl",
			'fieldWhoUpdate' => "delivery_order_batch_update_who",
		...
		if ($mode == 0) {
			$this->M_DeliveryOrderBatch->insertDOBH($do_batch_id, $do_batch_h_id_asal, $kendaraan, $driver);
		} else {
			$this->M_DeliveryOrderBatch->updateDOBH($do_batch_id, $do_batch_h_id_asal, $do_batch_h_id_tujuan);
		}
	...
	public function Get_attachment_delivery_order()
	{
		$delivery_order_batch_id = $this->input->get('delivery_order_batch_id');

		$data = $this->M_DeliveryOrderBatch->Get_attachment_delivery_order($delivery_order_batch_id);

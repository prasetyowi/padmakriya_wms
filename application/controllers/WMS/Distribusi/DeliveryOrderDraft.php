...
class DeliveryOrderDraft extends MenuController
{
	...
	public function index()
	{
		...
		$data['TipeGudang'] = $this->M_DeliveryOrderDraft->GetTipeGudang();
	...
	public function dosplit()
	{
		...
		$this->load->view('layouts/sidebar_header', $query);
		$this->load->view('WMS/DeliveryOrderDraft/dosplit', $data);
		$this->load->view('layouts/sidebar_footer', $query);
		$this->load->view('master/S_GlobalVariable', $query);
		$this->load->view('WMS/DeliveryOrderDraft/script', $data);
	}
	...
	public function GetSelectedSKU()
	{
		...
		$do_draft_id = $this->input->post('do_draft_id');
		$act = $this->input->post('act');
		$do_reff_id = $this->input->post('do_reff_id');

		$data = $this->M_DeliveryOrderDraft->GetSelectedSKU($sku_id, $do_draft_id, $act, $do_reff_id);
	...
	public function search_filter_chosen_sku()
	{
		...
		$arr_sku_do_split = $this->input->post('arr_sku_do_split');
		$dod_id = $this->input->post('dod_id');
		$dod_reff_id = $this->input->post('dod_reff_id');
		$act = $this->input->post('act');

		$data = $this->M_DeliveryOrderDraft->search_filter_chosen_sku($client_pt, $perusahaan, $tipe_pembayaran, $brand, $principle, $sku_induk, $sku_nama_produk, $sku_kemasan, $sku_satuan, $arr_sku_do_split, $dod_id, $dod_reff_id, $act);
	...
	public function insert_delivery_order_draft()
	{
		...
		$do_eksternal = $this->input->post('do_eksternal');
			...
			foreach ($data_detail as $key => $value) {
				if (!empty(get_object_vars($value))) {
					...
					if ($tipe_delivery_order_id == "C5BE83E2-01E8-4E24-B766-26BB4158F2CD") {
						$this->M_DeliveryOrderDraft->insert_delivery_order_detail_retur_draft($dod_id_, $dod_id, $value);
					} else {
	...
	public function update_delivery_order_draft()
	{
		...
		foreach ($result as $key => $value) {
			if (!empty(get_object_vars($value))) {
				...
				if ($tipe_delivery_order_id == "C5BE83E2-01E8-4E24-B766-26BB4158F2CD") {
					$this->M_DeliveryOrderDraft->insert_delivery_order_detail_retur_draft($dod_id, $delivery_order_draft_id, $value);
				} else {
	...
	public function confirm_delivery_order_draft()
	{
		...
		*/
		...
		if ($tipe_delivery_order_id == 'C42B0674-75B3-468F-B377-E4B23D9B8E12') {
	...
	public function getDataSearchEditDO()
	{
		...
		$TipeDeliveryOrder = $this->M_DeliveryOrderDraft->GetTipeDeliveryOrder();

		$response = [
			'data' => $data,
			'tipe' => $TipeDeliveryOrder
		];
	...
	public function getDataSearchUbahKeDraft()
	{
		...
		$data = $this->M_DeliveryOrderDraft->getDataSearchUbahKeDraft($tgl1, $tgl2, $area, $principle);
	...
	public function updateDateRencanaKirim()
	{
		...
		if (count($arrData) > 0) {
			foreach ($arrData as $key => $value) {
				...
				// 	'tipe_gudang' => $value['TipeGudang']
				// ];
				$updateData = [];

				if (!empty($value['aktualRencanaKirim'])) {
					$updateData['delivery_order_draft_tgl_rencana_kirim'] = $value['aktualRencanaKirim'] . " 00:00:00.000";
				}

				if (!empty($value['TipeGudang'])) {
					$updateData['tipe_gudang'] = $value['TipeGudang'];
				}

				if (!empty($value['TipeEditGlobal'])) {
					$updateData['tipe_delivery_order_id'] = $value['TipeEditGlobal'];
				}

				// Lakukan update pada database
				$this->db->update("delivery_order_draft", $updateData, ['delivery_order_draft_id' => $value['valueId']]);
			}
		}
	...
	public function GetDeliveryOrderDraftPrioritasStokByFilter()
	{
		...
		$tipe_gudang = $this->input->post('filter_tipe_gudang');
		$tgl = $this->input->post('filter_tanggal');

		$data = $this->M_DeliveryOrderDraft->GetDeliveryOrderDraftPrioritasStokByFilter($tgl, $tipe_do, $tipe_gudang);
	...
	public function Update_delivery_order_draft_prioritas_stok()
	{
		...
		foreach ($arr_list_do_detail as $key => $value) {
			...
			$sku_id = $value['sku_id'];
			$sku_qty = $value['sku_qty'];
			$is_revisi = $value['is_revisi'];
			$log_tipe = 'edit do draft';

			$data = $this->M_DeliveryOrderDraft->proses_revisi_delivery_order_detail_draft($delivery_order_detail_draft_id, $delivery_order_draft_id, $sku_id, $sku_qty, $log_tipe, $pengguna_username);

			foreach ($data as $key => $value) {
				array_push($data_msg, array("StatusMsg" => $value['StatusMsg'], "ResponseMsg" => $value['ResponseMsg'], "Msg" => $value['Msg']));
			}
	...
	public function proses_approval_delivery_order_draft()
	{
		$data = array();
		$arr_list_do_msg_error = array();
		$arr_do_draft = $this->input->post('arr_do_draft');

		if (isset($arr_do_draft) && count($arr_do_draft) > 0) {
			foreach ($arr_do_draft as $key => $value) {
				# code...

				$delivery_order_draft_id = $value['do_id'];
				$delivery_order_draft_update_tgl = $value['tgl_update'];
				$pengguna_username = $this->session->userdata('pengguna_username');

				$this->M_DeliveryOrderDraft->proses_approval_delivery_order_draft($delivery_order_draft_id, $delivery_order_draft_update_tgl, $pengguna_username);

				array_push($arr_list_do_msg_error, "'" . $delivery_order_draft_id . "'");
			}

			$data = $this->M_DeliveryOrderDraft->Get_delivery_order_draft_detail_msg_by_list_do($arr_list_do_msg_error);
		}

		echo json_encode($data);
	}

	public function proses_delivery_order_ubah_ke_draft()
	{
		$data = array();
		$arr_list_do_msg_error = array();
		$arr_do_draft = $this->input->post('arr_do_draft');
		...
		if (isset($arr_do_draft) && count($arr_do_draft) > 0) {
			foreach ($arr_do_draft as $key => $value) {
				...
				$delivery_order_draft_id = $value;
				$pengguna_username = $this->session->userdata('pengguna_username');

				$data = $this->M_DeliveryOrderDraft->proses_delivery_order_ubah_ke_draft($delivery_order_draft_id, $pengguna_username);

				foreach ($data as $key => $value) {
					array_push($arr_list_do_msg_error, array("StatusMsg" => $value['StatusMsg'], "ErrMsg" => $value['ErrMsg'], "delivery_order_draft_kode" => $value['delivery_order_draft_kode']));
				}
			}
		}

		echo json_encode($arr_list_do_msg_error);
	}

	public function proses_pembatalan_delivery_order_draft()
	{
		$data = array();
		$arr_list_do_msg_error = array();
		$arr_do_draft = $this->input->post('arr_do_draft');
		...
		if (isset($arr_do_draft) && count($arr_do_draft) > 0) {
			foreach ($arr_do_draft as $key => $value) {
				...
				$data = $this->M_DeliveryOrderDraft->proses_pembatalan_delivery_order_draft($delivery_order_draft_id, $pengguna_username);
	...
	public function Get_sku_group_by_sku_konversi_group()
	{
		$delivery_order_detail_draft_id = $this->input->get('delivery_order_detail_draft_id');
		$sku_konversi_group = $this->input->get('sku_konversi_group');
		$sku_id = $this->input->get('sku_id');
		$tipe_gudang = $this->input->get('tipe_gudang');
		$tipe_do = $this->input->get('tipe_do');

		$data = $this->M_DeliveryOrderDraft->Get_sku_group_by_sku_konversi_group($delivery_order_detail_draft_id, $sku_konversi_group, $tipe_gudang, $tipe_do, $sku_id);

		echo json_encode($data);
	}

	public function Get_delivery_order_detail_revisi_temp_by_do_id()
	{
		$delivery_order_detail_draft_id = $this->input->get('delivery_order_detail_draft_id');

		$data = $this->M_DeliveryOrderDraft->Get_delivery_order_detail_revisi_temp_by_do_id($delivery_order_detail_draft_id);

		echo json_encode($data);
	}

	public function insert_delivery_order_detail_revisi_temp()
	{
		$message = array();
		$arr_do_detail_revisi = $this->input->post('arr_do_detail_revisi');

		$this->db->trans_begin();

		foreach ($arr_do_detail_revisi as $key => $value) {
			# code...
			$dod_revisi_id = $this->M_Vrbl->Get_NewID();
			$dod_revisi_id = $dod_revisi_id[0]['NEW_ID'];

			$delivery_order_detail_draft_id = $value['delivery_order_detail_draft_id'];
			$delivery_order_draft_id = $value['delivery_order_draft_id'];
			$sku_id_asal = $value['sku_id_asal'];
			$sku_id = $value['sku_id'];
			$sku_qty = $value['sku_qty'];

			$this->db->where("delivery_order_draft_id", $delivery_order_draft_id);
			$this->db->where("sku_id_asal", $sku_id_asal);
			$this->db->where("sku_id", $sku_id);

			$this->db->delete("delivery_order_detail_revisi_temp");

			$this->M_DeliveryOrderDraft->insert_delivery_order_detail_revisi_temp($dod_revisi_id, $delivery_order_detail_draft_id, $delivery_order_draft_id, $sku_id_asal, $sku_id, $sku_qty);
		}

		if ($this->db->trans_status() === false) {
			$this->db->trans_rollback();
			$message = array("status" => 400, "message" => "error", "data" => "Data gagal disimpan");
		} else {
			$this->db->trans_commit();
			$message = array("status" => 200, "message" => "success", "data" => "Data berhasil disimpan");
		}

		echo json_encode($message);
	}

	public function delete_delivery_order_detail_revisi_temp()
	{
		$message = "";
		$arr_delivery_order_detail_prioritas = $this->input->post('arr_delivery_order_detail_prioritas');

		$this->db->trans_begin();

		if (isset($arr_delivery_order_detail_prioritas) && count($arr_delivery_order_detail_prioritas) > 0) {

			foreach ($arr_delivery_order_detail_prioritas as $key => $value) {
				# code...

				$this->db->where("delivery_order_detail_draft_id", $value['delivery_order_detail_draft_id']);

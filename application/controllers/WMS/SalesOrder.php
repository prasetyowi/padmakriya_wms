...
class SalesOrder extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$menu_link_main = $currentdirectory . $currentcontroller . '/' . $currentfunction;
	...
	public function SaveSalesOrderFAS()
	{
		...
		if ($switched == "0") {
			...
			$this->M_SalesOrder->insert_sales_order_detail_promo_internal($tgl, $perusahaan, $principle);
			...
			if (!isset($get_sales_order['message'])) {
				...
				if ($arr_list_so != "0") {
					...
					foreach ($arr_list_so as $key => $value) {
						...
						// insert so detail promo
						$url3 = Get_FAS_URL() . "API/SistemEksternal/SalesOrderDetailPromo?sales_order_id=" . $value['sales_order_id'];

						$header3 = array('Content-Type: application/json');

						//构建curl请求
						$c3 = curl_init();
						curl_setopt($c3, CURLOPT_URL, $url3);
						curl_setopt($c3, CURLOPT_HTTPHEADER, $header3);
						curl_setopt($c3, CURLOPT_RETURNTRANSFER, 1);
						curl_setopt($c3, CURLOPT_BUFFERSIZE, 10240);

						//获取到数据 json格式
						$datajson3 = curl_exec($c3);

						$get_sales_order_detail_promo = json_decode($datajson3, true);

						// echo var_dump($get_sales_order_detail) . "<br>";

						// if ($get_sales_order_detail !== NULL) {

						foreach ($get_sales_order_detail_promo as $key3 => $value3) {
							if (isset($value3['sales_order_id'])) {
								// echo $value2['sales_order_id'];
								$this->M_SalesOrder->Insert_sales_order_detail_promo($value3);
								// $this->M_SalesOrder->Insert_sales_order_detail2($dodd_id, $dod_id, $value['sales_order_id']);
							}
						}
	...
	public function SaveDeliveryOrder()
	{
		...
		$data = $this->M_SalesOrder->Exec_proses_generate_so_fas_do_wms($tgl, $perusahaan_eksternal, $principle);
	...
	public function SaveCanvasDeliveryOrder()
	{
		...
		$data = $this->M_SalesOrder->Exec_proses_generate_canvas_fas_do_wms($tgl, $perusahaan_eksternal);
	...
	public function Generate_retur_supplier()
	{
		...
		$data = $this->M_SalesOrder->Exec_proses_generate_retur_supplier($tgl, $tipe_transaksi, $perusahaan, $sistem_eksternal);

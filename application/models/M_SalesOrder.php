...
class M_SalesOrder extends CI_Model
{
	...
	public function Get_SalesOrderNotInDO($tgl, $status, $perusahaan, $principle)
	{
		...
		$query = $this->db->query("SELECT DISTINCT
									...
									AND ts.tipe_sales_order_nama IN ('RETUR', 'BELI')
	...
	public function Get_sales_order_wms($tgl, $perusahaan_eksternal, $principle)
	{
		...
		$query = $this->db->query("SELECT
									...
									ISNULL(hdr.sales_order_tipe_pembayaran, '') sales_order_tipe_pembayaran,
									ISNULL(CONVERT(nvarchar(36), hdr.tipe_sales_order_id), '') tipe_sales_order_id,
									ISNULL(tl.tipe_layanan_nama, '') tipe_layanan_nama,
									ISNULL(hdr.sales_order_no_po, '') sales_order_no_po,
									ISNULL(hdr.sales_order_who_create, '') sales_order_who_create,
									ISNULL(hdr.sales_order_tgl_create, '') sales_order_tgl_create,
									hdr.sales_order_is_downloaded,
									...
									LEFT JOIN tipe_layanan tl
									ON tl.tipe_layanan_id = hdr.tipe_layanan_id
									LEFT JOIN tipe_sales_order ts
									ON ts.tipe_sales_order_id = hdr.tipe_sales_order_id
									WHERE FORMAT(hdr.sales_order_tgl_kirim, 'yyyy-MM-dd') = '$tgl'
									AND hdr.sales_order_status = 'Approved'
									AND ts.tipe_sales_order_nama IN ('RETUR', 'BELI') " . $perusahaan_eksternal . " " . $principle . "
									GROUP BY hdr.sales_order_id,
											hdr.depo_id,
											hdr.sales_order_kode,
											ISNULL(CONVERT(nvarchar(36), dtl.client_wms_id), ''),
	...
	public function Insert_sales_order_detail_promo($data)
	{
		$sales_order_detail_promo_id = $data['sales_order_detail_promo_id'] == '' ? null : $data['sales_order_detail_promo_id'];
		$sales_order_id = $data['sales_order_id'] == '' ? null : $data['sales_order_id'];
		...
		$sku_konversi_group_bonus = $data['sku_konversi_group_bonus'] == '' ? null : $data['sku_konversi_group_bonus'];
		$sku_promo_diskon_amount = $data['sku_promo_diskon_amount'] == '' ? null : $data['sku_promo_diskon_amount'];
		$sku_promo_qty_bonus = $data['sku_promo_qty_bonus'] == '' ? null : $data['sku_promo_qty_bonus'];
		$sku_konversi_group_reff = $data['sku_konversi_group_reff'] == '' ? null : $data['sku_konversi_group_reff'];
		$no_reff_external = $data['no_reff_external'] == '' ? null : $data['no_reff_external'];
		$external_system = $data['external_system'] == '' ? null : $data['external_system'];
		...
		$this->db->set('sku_konversi_group_bonus', $sku_konversi_group_bonus);
		$this->db->set('sku_promo_diskon_amount', $sku_promo_diskon_amount);
		$this->db->set('sku_promo_qty_bonus', $sku_promo_qty_bonus);
		$this->db->set('sku_konversi_group_reff', $sku_konversi_group_reff);
		$this->db->set('no_reff_external', $no_reff_external);
		$this->db->set('external_system', $external_system);
	...
	public function insert_sales_order_detail_promo_internal($tgl, $perusahaan, $principle)
	{

		$perusahaan = $perusahaan != "" ? $perusahaan = " AND perusahaan.client_wms_kode = '$perusahaan' " : "";
		$principle = $principle != "" ? $principle = " AND principle.principle_kode = '$principle' " : "";

		$query = $this->db->query("INSERT INTO sales_order_detail_promo
									SELECT
										so.*
										FROM FAS.dbo.sales_order_detail_promo so
										INNER JOIN FAS.dbo.sales_order a 
										ON so.sales_order_id = a.sales_order_id
										INNER JOIN FAS.dbo.client_wms perusahaan
										ON perusahaan.client_wms_id = a.client_wms_id
										INNER JOIN principle on a.principle_id = principle.principle_id
										WHERE so.sales_order_id IN (SELECT
										sales_order_id
										FROM FAS.dbo.sales_order a
										INNER JOIN tipe_sales_order b on a.tipe_sales_order_id = b.tipe_sales_order_id
										WHERE FORMAT(sales_order_tgl_kirim,'yyyy-MM-dd') = '$tgl' AND sales_order_status = 'Approved'
										...
										" . $perusahaan . "
										" . $principle . "
										AND so.sales_order_detail_promo_id NOT IN (select sales_order_detail_promo_id from sales_order_detail_promo)");
	...
	public function Insert_delivery_order($dod_id, $do_kode, $client_wms_id, $delivery_order_draft_nominal_tunai, $client_pt_id, $client_pt_nama, $client_pt_telepon, $client_pt_alamat, $client_pt_kelurahan, $client_pt_kecamatan, $client_pt_kota, $client_pt_propinsi, $client_pt_kodepos, $area_nama, $data, $type)
	{
		...
		$sales_order_tgl_sj = $data['sales_order_tgl_sj'] == '' ? null : $data['sales_order_tgl_sj'];
		$sales_order_tgl_kirim = $data['sales_order_tgl_kirim'] == '' ? null : $data['sales_order_tgl_kirim'];
		$tipe_layanan_nama = $data['tipe_layanan_nama'] == '' ? null : $data['tipe_layanan_nama'];
		...
		$this->db->set('delivery_order_draft_tipe_layanan', $tipe_layanan_nama);
	...
	public function Get_canvas_wms($tgl, $perusahaan)
	{
		$query = $this->db->query("SELECT
									...
									client_pt.client_pt_kelurahan,
									client_pt.client_pt_kodepos,
									'Delivery Only' AS tipe_layanan_nama,
									--area.area_kode
	...
	public function Exec_proses_generate_so_fas_do_wms($tgl, $perusahaan_eksternal, $principle)
	{
		$pengguna_username = $this->session->userdata('pengguna_username');
		$query = $this->db->query("exec proses_generate_so_fas_do_wms '$tgl','$principle','$perusahaan_eksternal','$pengguna_username' ");
	...
	public function Exec_proses_generate_canvas_fas_do_wms($tgl, $perusahaan_eksternal)
	{
		$depo_id = $this->session->userdata('depo_id');
		$pengguna_username = $this->session->userdata('pengguna_username');

		$query = $this->db->query("exec proses_generate_canvas_fas_do_wms '$tgl','$depo_id','$perusahaan_eksternal','$pengguna_username' ");
	...
	public function Exec_proses_generate_retur_supplier($tgl, $tipe_transaksi, $perusahaan_eksternal, $sistem_eksternal)
	{
		$depo_id = $this->session->userdata('depo_id');
		$pengguna_username = $this->session->userdata('pengguna_username');

		$query = $this->db->query("exec proses_generate_retur_supplier '$tgl','$depo_id','$perusahaan_eksternal','$tipe_transaksi','$sistem_eksternal','$pengguna_username' ");

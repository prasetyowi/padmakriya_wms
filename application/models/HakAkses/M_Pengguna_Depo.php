...
class M_Pengguna_Depo extends CI_Model
{
	...
	public function getPenggunaGrupID($depo_id)
	{
		$query = $this->db->query("SELECT pengguna_grup_id FROM pengguna_depo WHERE pengguna_id = '" . $this->session->userdata('pengguna_id') . "' AND depo_id = '$depo_id'");

		if ($query->num_rows() == 0) {
			$query = 0;
		} else {
			$query = $query->row('pengguna_grup_id');
		}

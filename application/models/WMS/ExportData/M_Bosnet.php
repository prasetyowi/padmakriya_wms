...
class M_Bosnet extends CI_Model
{
	...
	public function Get_PrincipleExistSku()
	{
		$depo_id = $this->session->userdata('depo_id');
		$query = $this->db->query("SELECT p.principle_id,p.principle_nama,p.principle_kode
										FROM principle p
										WHERE EXISTS (
											SELECT 1
											FROM sku s
											INNER JOIN sku_stock ss ON ss.sku_id = s.sku_id
											WHERE s.principle_id = p.principle_id
	...
	public function Get_bosnet_do_by_id($tgl1, $tgl2, $karyawan_eksternal_id, $rit, $gudang, $kendaraan_nopol)
	{
		set_time_limit(0);
		...
		$query = $this->db->query("select
                                    ...
                                    AND ISNULL(kse.sales_eksternal_id, '') <> '' 
                                    AND ISNULL(so.sales_order_no_po, '') <> ''
                                    AND ISNULL(bdo.szVehicleId, '') = '$kendaraan_nopol'
                                    group by fdjr.karyawan_id,
										    karyawan.karyawan_nama,
                                            ISNULL(kse.sales_eksternal_id, ''),
                                            FORMAT(fdjr.delivery_order_batch_tanggal_kirim,'dd-MM-yyyy'),
	...
	public function Get_bosnet_penerimaan_surat_jalan_by_filter($tgl1, $tgl2, $principle_id, $principle_kode)
	{
		...
		$query_so_bosnet = $bosnet->query("SELECT supp.szSuppId AS ID_Principle,
                                                    ...
                                                    CASE 
                                                        WHEN price.szTaxTypeId = 'PPN' THEN 
                                                            ISNULL(price.decPurchPrice,0) * 1.11
                                                        ELSE 
                                                            price.decPurchPrice 
                                                        END AS Harga_Satuan,
		...
		$query = $this->db->query("select 
                                    ...
                                    order by FORMAT(hdr.penerimaan_surat_jalan_tgl, 'dd/MM/yyyy') DESC");
	...
	public function Get_bosnet_penerimaan_surat_jalan_by_id($tgl, $principle_id, $principle_kode, $penerimaan_surat_jalan_id)
	{
		set_time_limit(0);
		...
		$query = $this->db->query("select 
										...
										hdr.penerimaan_surat_jalan_no_sj,
                                        FORMAT(hdr.penerimaan_surat_jalan_tgl, 'dd/MM/yyyy') AS penerimaan_surat_jalan_tgl,
                                        FLOOR(SUM(ISNULL(dtl.sku_jumlah_barang, 0) * ISNULL(sku.sku_konversi_faktor, 0) / ISNULL(sku2.sku_konversi_faktor, 0))) AS sku_jumlah_barang,
                                        ISNULL(bosnet.Harga_Satuan, 0) AS sku_harga
                                    from penerimaan_surat_jalan hdr
                                    left join penerimaan_surat_jalan_detail dtl
                                    on dtl.penerimaan_surat_jalan_id = hdr.penerimaan_surat_jalan_id
                                    left join sku
                                    on sku.sku_id = dtl.sku_id
                                    left join sku as sku2
									on sku2.sku_konversi_group = sku.sku_konversi_group 
									left join MIDDLEWARE.dbo.bosnet_sku_by_perbusahaan bosnet
									on bosnet.ID_Produk = sku.sku_konversi_group
                                    left join principle p
                                    on p.principle_id = sku.principle_id
                                    ...
                                    and bosnet.ID_Produk IS NOT NULL
                                    " . $principle_id . "
                                    and hdr.penerimaan_surat_jalan_id = '$penerimaan_surat_jalan_id'
                                    and sku2.sku_konversi_level = (
										SELECT MAX(sku.sku_konversi_level)
										FROM sku  
										WHERE sku.sku_konversi_group = sku2.sku_konversi_group
									)
                                    group by hdr.depo_id,
                                        de.depo_eksternal_kode,
                                        sku.principle_id,
	...
	public function Get_bosnet_delivery_order_by_filter($tgl1, $tgl2, $filter_principle)
	{
		set_time_limit(0);


		$query = $this->db->query("exec report_delivery_order '$tgl1', '$tgl2', '$filter_principle'");
	...
	public function Get_bosnet_penerimaan_barang_penjualan_by_filter($tgl1, $tgl2, $filter_principle, $filter_driver)
	{
		set_time_limit(0);

		$query = $this->db->query("exec report_export_penerimaan_penjualan '$tgl1','$tgl2','$filter_principle','" . $this->session->userdata('depo_id') . "','BOSNET', '$filter_driver'");
	...
	public function Get_driver()
	{
		...
		$query = $this->db->query("select karyawan_id, karyawan_nama from karyawan where convert(nvarchar(36), depo_id) = '$depo_id' and karyawan_is_aktif = '1' order by karyawan_nama asc");

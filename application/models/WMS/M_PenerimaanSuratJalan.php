...
class M_PenerimaanSuratJalan extends CI_Model
{
	...
	{
		return  $this->db->select("penerimaan_surat_jalan.*, principle.principle_use_segment")->from("penerimaan_surat_jalan")->join("principle", "principle.principle_id = penerimaan_surat_jalan.principle_id", "left")->where('penerimaan_surat_jalan_id', $id)->get()->row();
	}
	...
	{
		$query = $this->db->select("p.principle_use_segment, p.principle_id as id, p.principle_kode as kode, p.principle_nama as nama, cwp.client_wms_principle_top as tempo")
			->from("principle p")
	...
	{
		$data = $this->db->query("SELECT tipegudang_kode, tipegudang_nama FROM gettipegudang() WHERE tipegudang_is_aktif = 1")->result();
		return $data;
	}
	...
	{
		$dataTipePenerimaan = $this->db->select("client_pt_segmen_id as id, client_pt_segmen_nama as nama")
	...
	public function get_sku_by_id($id)
	{
		if (isset($id)) {
			$query = $this->db->select("sku_id,
                                        ...
                                        sku_satuan,
                                        sku_harga_jual,
										principle.principle_use_segment")
				->from("sku")
	...
	public function save($sj_id, $principle, $perusahaan, $tipe_penerimaan, $generate_kode, $no_surat_jalan, $tgl, $status, $keterangan, $nama_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang)
	{
		$this->db->set("penerimaan_surat_jalan_id", $sj_id);
		$this->db->set("depo_id", $this->session->userdata("depo_id"));
		$this->db->set("principle_id", $principle);
		$this->db->set("client_wms_id", $perusahaan);
		$this->db->set("penerimaan_tipe_id", $tipe_penerimaan);
		$this->db->set("tipe_gudang_kode", $tipe_gudang);
		$this->db->set("penerimaan_surat_jalan_kode", $generate_kode);
	...
	public function save_detail($sj_id, $value)
	{
		...
		$this->db->set("client_pt_segmen_id ", $value->sku_tipe_segmen);
	...
	public function updateClientPtSegmenDetail($sj_id, $item)
	{

		$this->db->query("UPDATE penerimaan_surat_jalan_detail
							SET client_pt_segmen_id = ori.client_pt_segmen_id
							FROM penerimaan_surat_jalan_detail det
							INNER JOIN penerimaan_surat_jalan_detail_ori ori 
								ON det.penerimaan_surat_jalan_id = ori.penerimaan_surat_jalan_id
							INNER JOIN sku sku1 ON det.sku_id = sku1.sku_id
							INNER JOIN sku sku2 ON ori.sku_id = sku2.sku_id
							WHERE sku1.sku_konversi_group = sku2.sku_konversi_group
							AND det.sku_exp_date = ori.sku_exp_date AND det.penerimaan_surat_jalan_id = '$sj_id'");
	...
	public function update($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $nama_file, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang)
	{
		$tgl_ = $tgl . " " . date('H:i:s');
	...
	public function update2($id, $principle, $perusahaan, $tipe_penerimaan, $no_surat_jalan, $tgl, $status, $keterangan, $no_surat_jalan_counter, $no_kendaraan, $tipe_gudang)
	{

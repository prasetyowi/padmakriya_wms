...
class M_LaporanSetoranDriver extends CI_Model
{
	...
	public function GetDetailLaporanSetoranDriver($tgl1, $tgl2, $client_wms_id, $depo_id, $principle_id, $driver)
	{
		if ($client_wms_id == "") {
			$client_wms_id = "";
		} else {
		...
		if ($principle_id == "") {
			...
			$principle_id = "and principle.principle_id = '$principle_id' ";
		}

		if ($driver != "") {
			$driver = " AND karyawan.karyawan_id = '" . $driver . "' ";
		} else {
			$driver = "";
		}
		...
		$query =  $this->db->query("SELECT 
										...
										SUM(ISNULL(CAST(CASE WHEN do.delivery_order_tipe_pembayaran = 0 THEN do.delivery_order_nominal_tunai ELSE 0 END AS bigint), 0)) AS delivery_order_nominal_tunai,
										SUM(ISNULL(CAST(do.delivery_order_jumlah_bayar AS bigint), 0)) AS delivery_order_jumlah_bayar
									FROM delivery_order_batch fdjr
									LEFT JOIN delivery_order do ON do.delivery_order_batch_id = fdjr.delivery_order_batch_id
									LEFT JOIN delivery_order_detail do_detail ON do_detail.delivery_order_id = do.delivery_order_id
									LEFT JOIN sku ON sku.sku_id = do_detail.sku_id
									LEFT JOIN principle ON principle.principle_id = sku.principle_id
									LEFT JOIN karyawan ON karyawan.karyawan_id = fdjr.karyawan_id
									WHERE FORMAT(fdjr.delivery_order_batch_tanggal_kirim, 'yyyy-MM-dd') BETWEEN '$tgl1' AND '$tgl2' AND fdjr.depo_id = '$depo_id' AND principle.principle_id IS NOT NULL AND karyawan.karyawan_id IS NOT NULL
									" . $client_wms_id . "
									" . $principle_id . "
									" . $driver . "
									GROUP BY ISNULL(karyawan.karyawan_nama, ''), ISNULL(principle.principle_kode, '') WITH ROLLUP
									ORDER BY GROUPING(ISNULL(karyawan.karyawan_nama, '')),ISNULL(karyawan.karyawan_nama, ''), GROUPING(ISNULL(principle.principle_kode, '')), ISNULL(principle.principle_kode, '')");

...
class M_LaporanAgingStock extends CI_Model
{
    ...
    public function Get_tipe_gudang()
    {
        $query = $this->db->query("SELECT
										*
									FROM gettipegudang()
                                    WHERE ISNULL(tipegudang_is_aktif, 0) = '1'
									ORDER BY tipegudang_kode ASC");
    ...
    public function Get_laporan_stock_aging($tglawal, $tglakhir, $client_wms_id, $principle_id, $depo_detail_id, $tipe, $tipe_gudang)
    {
        set_time_limit(0);
        ...
        $query_laporan =  $this->db->query("exec report_sku_stock_aging_by_group '$tglawal','$tglakhir','$client_wms_id','" . $this->session->userdata('depo_id') . "','$principle_id','$depo_detail_id','$tipe','$tipe_gudang'");

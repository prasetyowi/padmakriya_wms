...
class M_DeliveryOrderBatch extends CI_Model
{
	...
	public function GetCetakHeaderFDJRById($id)
	{
		...
		$query = $this->db->query("EXEC proses_cetak_fdjr 'header', '$id'");
	...
	public function GetCetakHeaderFDJRDetailById($id)
	{
		...
		return $this->db->query("EXEC proses_cetak_fdjr 'detail', '$id'")->result_array();
	}
	...
	public function GetCetakHeaderFDJRTotalById($id)
	{
		...
		$query = $this->db->query("EXEC proses_cetak_fdjr 'total', '$id'");
	...
	public function GetCetakHeaderFDJRBankById($id)
	{

		$query = $this->db->query("EXEC proses_cetak_fdjr 'bank', '$id'");
	...
	public function GetDODraftByFilter($tgl, $area, $tipe, $tipe_layanan, $kelas_jalan, $kendaraan)
	{
		...
		if (empty($area)) {
			...
			$areaWhereIn = "AND (CASE 
										WHEN delivery_order_draft_tipe_layanan = 'Pickup Only' THEN delivery_order_draft_ambil_area
										ELSE delivery_order_draft_kirim_area
									END) IN (" . implode(',', $areaArray) . ")";
		}
		...
		if ($tipe == "") {
			...
			// $tipe = "AND do.tipe_delivery_order_id IN ('0E626A53-82FC-4EA6-A4A2-1265279D6E1C','ADF55030-9802-4C27-9658-F9D37AA01F95', 'C5BE83E2-01E8-4E24-B766-26BB4158F2CD', 'EE5CD3F7-F7E3-475E-A7B4-67FD5AB65975') ";
			$tipe = "AND tipe_delivery_order_id IN ('0E626A53-82FC-4EA6-A4A2-1265279D6E1C','ADF55030-9802-4C27-9658-F9D37AA01F95', 'C5BE83E2-01E8-4E24-B766-26BB4158F2CD', 'EE5CD3F7-F7E3-475E-A7B4-67FD5AB65975') ";
		} else {
			$tipe = "AND tipe_delivery_order_id = '$tipe' ";
		}

		if ($tipe_layanan == "") {
			$tipe_layanan = "";
		} else {
			$tipe_layanan = "AND delivery_order_draft_tipe_layanan = '$tipe_layanan' ";
		}
		...
		if ($kelas_jalan == 'true') {
			...
			$kelas_jalan_berat_muatan = "AND kelas_jalan_berat_muatan >= $kelas_jalan_berat_muatan";
		} else {
			$kelas_jalan_berat_muatan = '';
		}

		$query = $this->db->query("WITH filtered_do AS (
			SELECT *
			FROM delivery_order_draft
			WHERE 
				delivery_order_draft_tgl_rencana_kirim <= '$tgl'
				AND NOT EXISTS (
					SELECT 1 
					FROM delivery_order 
					WHERE delivery_order_draft_id = delivery_order_draft.delivery_order_draft_id
				)
			...
			SELECT
				...
				(
					SELECT
						STUFF((
							SELECT 
								'; ' + CAST(SUM(dodd2.sku_qty) AS VARCHAR(100)) + ' ' + 
								CASE 
									WHEN PATINDEX('%[0-9]%', sku2.sku_satuan) > 0 
										THEN LEFT(sku2.sku_satuan, PATINDEX('%[0-9]%', sku2.sku_satuan) - 1)
										ELSE sku2.sku_satuan
								END
							FROM delivery_order_detail_draft dodd2
							INNER JOIN sku sku2 ON dodd2.sku_id = sku2.sku_id
							INNER JOIN delivery_order_draft do2 ON dodd2.delivery_order_draft_id = do2.delivery_order_draft_id
							INNER JOIN tipe_delivery_order tdo2 ON do2.tipe_delivery_order_id = tdo2.tipe_delivery_order_id
							WHERE 
								dodd2.delivery_order_draft_id = do.delivery_order_draft_id
								AND tdo2.tipe_delivery_order_tipe = 'OUT'
							GROUP BY 
								CASE 
									WHEN PATINDEX('%[0-9]%', sku2.sku_satuan) > 0 
										THEN LEFT(sku2.sku_satuan, PATINDEX('%[0-9]%', sku2.sku_satuan) - 1)
		...
		SELECT 
			...
			END AS delivery_order_draft_kirim_alamat,
			CASE 
				WHEN do.delivery_order_draft_tipe_layanan = 'Pickup Only' THEN do.delivery_order_draft_ambil_kecamatan
				ELSE do.delivery_order_draft_kirim_kecamatan
			END AS delivery_order_draft_kirim_kecamatan,
			CASE 
				WHEN do.delivery_order_draft_tipe_layanan = 'Pickup Only' THEN do.delivery_order_draft_ambil_area
				ELSE do.delivery_order_draft_kirim_area
			END AS delivery_order_draft_kirim_area,
			so.sales_order_no_po,
			pc.principle_kode,
			tipe.tipe_delivery_order_nama,
			SUM(ISNULL(dodd.sku_weight, 0) * ISNULL(dodd.sku_qty, 0)) AS sku_weight,
			SUM(ISNULL(dodd.sku_volume, 0) * ISNULL(dodd.sku_qty, 0)) AS sku_volume,
			ISNULL(cd.composite, 0) AS composite,
			ISNULL(kj.kelas_jalan_nama, '') AS kelas_jalan_nama
		FROM filtered_do do
		LEFT JOIN sales_order so ON do.sales_order_id = so.sales_order_id
		...
		LEFT JOIN client_pt cpt ON do.client_pt_id = cpt.client_pt_id
		LEFT JOIN kelas_jalan kj ON cpt.kelas_jalan_id = kj.kelas_jalan_id
		LEFT JOIN composite_data cd ON do.delivery_order_draft_id = cd.delivery_order_draft_id
		GROUP BY 
			do.delivery_order_draft_tipe_layanan,
			do.delivery_order_draft_ambil_nama,
			do.delivery_order_draft_kirim_nama,
		...
		ORDER BY 
			...
			delivery_order_draft_kirim_alamat,
			delivery_order_draft_kirim_nama ASC;
		");
...
ORDER BY do.delivery_order_no_urut_rute ASC");
	...
	function insertDOBH($do_batch_id, $do_batch_h_id_asal, $kendaraan_tujuan, $driver)
	{
		$batch_h = $this->db->query("select * from delivery_order_batch_h where delivery_order_batch_h_id = '$do_batch_h_id_asal'")->row();
		$kendaraan = $this->db->query("select kendaraan_kap_vol, kendaraan_kap_weight from kendaraan where kendaraan_id = '$kendaraan_tujuan'")->row();
		$terpakai = $this->db->query("select SUM(CAST(ISNULL(b.sku_weight, 0) AS INT) * CAST(ISNULL(b.sku_qty, 0) AS INT)) as sku_weight, SUM(CAST(ISNULL(b.sku_volume, 0) AS INT) * CAST(ISNULL(b.sku_qty, 0) AS INT)) as sku_volume
										from delivery_order a
										left join delivery_order_detail b on a.delivery_order_id = b.delivery_order_id
										where a.delivery_order_batch_id = '$do_batch_id'")->row();

		$delivery_order_batch_h_id_new = $this->M_Vrbl->Get_NewID();
		$delivery_order_batch_h_id_new = $delivery_order_batch_h_id_new[0]['NEW_ID'];

		// insert batch h
		$this->db->set("delivery_order_batch_h_id", $delivery_order_batch_h_id_new);
		$this->db->set("delivery_order_batch_h_tanggal_kirim", $batch_h->delivery_order_batch_h_tanggal_kirim);
		$this->db->set("karyawan_id", $driver);
		$this->db->set("kendaraan_id", $kendaraan_tujuan);
		$this->db->set("delivery_order_batch_h_ritasi", $batch_h->delivery_order_batch_h_ritasi);
		$this->db->set("kendaraan_volume_cm3_max", $kendaraan->kendaraan_kap_vol);
		$this->db->set("kendaraan_berat_gr_max", $kendaraan->kendaraan_kap_weight);
		$this->db->set("kendaraan_volume_cm3_terpakai", $terpakai->sku_volume);
		$this->db->set("kendaraan_berat_gr_terpakai", $terpakai->sku_weight);
		$this->db->set("kendaraan_volume_cm3_sisa", intval($kendaraan->kendaraan_kap_vol) - intval($terpakai->sku_volume));
		$this->db->set("kendaraan_berat_gr_sisa", intval($kendaraan->kendaraan_kap_weight) - intval($terpakai->sku_weight));
		$this->db->insert("delivery_order_batch_h");

		// update batch
		$this->db->set("delivery_order_batch_h_id", $delivery_order_batch_h_id_new);
		$this->db->where("delivery_order_batch_id", $do_batch_id);
		$this->db->update("delivery_order_batch");

		// cek total fdjr pada batch h
		$fdjr = $this->db->query("select COUNT(delivery_order_batch_id) as jumlah from delivery_order_batch where delivery_order_batch_h_id = '$do_batch_h_id_asal'")->row('jumlah');

		if (intval($fdjr) > 0) {
			$this->db->where("delivery_order_batch_h_id", $do_batch_h_id_asal);
			$this->db->set("kendaraan_volume_cm3_terpakai", intval($batch_h->kendaraan_volume_cm3_terpakai) - intval($terpakai->sku_volume));
			$this->db->set("kendaraan_berat_gr_terpakai", intval($batch_h->kendaraan_berat_gr_terpakai) - intval($terpakai->sku_weight));
			$this->db->set("kendaraan_volume_cm3_sisa", intval($batch_h->kendaraan_volume_cm3_sisa) + intval($terpakai->sku_volume));
			$this->db->set("kendaraan_berat_gr_sisa", intval($batch_h->kendaraan_berat_gr_sisa) + intval($terpakai->sku_weight));
			$this->db->update("delivery_order_batch_h");
		} else {
			$this->db->where("delivery_order_batch_h_id", $do_batch_h_id_asal);
			$this->db->delete("delivery_order_batch_h");
		}
	}

	function updateDOBH($do_batch_id, $do_batch_h_id_asal, $do_batch_h_id_tujuan)
	{
		$batch_h_asal = $this->db->query("select kendaraan_volume_cm3_terpakai, kendaraan_berat_gr_terpakai, kendaraan_volume_cm3_sisa, kendaraan_berat_gr_sisa from delivery_order_batch_h where delivery_order_batch_h_id = '$do_batch_h_id_tujuan'")->row();
		$batch_h_tujuan = $this->db->query("select kendaraan_volume_cm3_terpakai, kendaraan_berat_gr_terpakai, kendaraan_volume_cm3_sisa, kendaraan_berat_gr_sisa from delivery_order_batch_h where delivery_order_batch_h_id = '$do_batch_h_id_tujuan'")->row();
		$terpakai = $this->db->query("select SUM(CAST(ISNULL(b.sku_weight, 0) AS INT) * CAST(ISNULL(b.sku_qty, 0) AS INT)) as sku_weight, SUM(CAST(ISNULL(b.sku_volume, 0) AS INT) * CAST(ISNULL(b.sku_qty, 0) AS INT)) as sku_volume
										from delivery_order a
										left join delivery_order_detail b on a.delivery_order_id = b.delivery_order_id
										where a.delivery_order_batch_id = '$do_batch_h_id_asal'")->row();

		$delivery_order_batch_h_id = $this->M_Vrbl->Get_NewID();
		$delivery_order_batch_h_id = $delivery_order_batch_h_id[0]['NEW_ID'];

		// update batch h tujuan
		$this->db->set("kendaraan_volume_cm3_terpakai", intval($batch_h_tujuan->kendaraan_volume_cm3_terpakai) + intval($terpakai->sku_volume));
		$this->db->set("kendaraan_berat_gr_terpakai", intval($batch_h_tujuan->kendaraan_berat_gr_terpakai) + intval($terpakai->sku_weight));
		$this->db->set("kendaraan_volume_cm3_sisa", intval($batch_h_tujuan->kendaraan_volume_cm3_sisa) - intval($terpakai->sku_volume));
		$this->db->set("kendaraan_berat_gr_sisa", intval($batch_h_tujuan->kendaraan_berat_gr_sisa) - intval($terpakai->sku_weight));
		$this->db->where("delivery_order_batch_id", $do_batch_h_id_tujuan);
		$this->db->update("delivery_order_batch_h");

		// update batch asal
		$this->db->set("delivery_order_batch_h_id", $do_batch_h_id_tujuan);
		$this->db->where("delivery_order_batch_id", $do_batch_id);
		$this->db->update("delivery_order_batch");

		// cek total fdjr pada batch h asa;
		$fdjr = $this->db->query("select COUNT(delivery_order_batch_id) as jumlah from delivery_order_batch where delivery_order_batch_h_id = '$do_batch_h_id_asal'")->row('jumlah');

		if (intval($fdjr) > 0) {
			$this->db->where("delivery_order_batch_h_id", $do_batch_h_id_asal);
			$this->db->set("kendaraan_volume_cm3_terpakai", intval($batch_h_asal->kendaraan_volume_cm3_terpakai) - intval($terpakai->sku_volume));
			$this->db->set("kendaraan_berat_gr_terpakai", intval($batch_h_asal->kendaraan_berat_gr_terpakai) - intval($terpakai->sku_weight));
			$this->db->set("kendaraan_volume_cm3_sisa", intval($batch_h_asal->kendaraan_volume_cm3_sisa) + intval($terpakai->sku_volume));
			$this->db->set("kendaraan_berat_gr_sisa", intval($batch_h_asal->kendaraan_berat_gr_sisa) + intval($terpakai->sku_weight));
			$this->db->update("delivery_order_batch_h");
		} else {
			$this->db->where("delivery_order_batch_h_id", $do_batch_h_id_asal);
			$this->db->delete("delivery_order_batch_h");
	...
	function deletePickingList($delivery_order_batch_id)
	{
		$this->db->query("DELETE picking_list_detail_2 WHERE picking_list_detail_id in (SELECT picking_list_detail_id FROM picking_list_detail WHERE picking_list_id = (SELECT picking_list_id FROM picking_list WHERE delivery_order_batch_id = '$delivery_order_batch_id'))");
		$this->db->query("DELETE picking_list_detail WHERE picking_list_id = (SELECT picking_list_id FROM picking_list WHERE delivery_order_batch_id = '$delivery_order_batch_id')");
		$this->db->query("DELETE picking_list WHERE delivery_order_batch_id = '$delivery_order_batch_id'");
	}

	function getHeaderDOBatch($id)
	{
		return	$this->db->query("SELECT
							b.client_wms_nama
							,ISNULL(b.do_eksternal_no, '') AS do_eksternal_no
							,ISNULL(b.so_eksternal_no, '') AS so_eksternal_no
							,d.karyawan_nama
							,e.kendaraan_nopol
							,CONVERT(VARCHAR, DAY(a.delivery_order_batch_tanggal_kirim)) + ' ' + 
								CASE 
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'January' THEN 'JANUARI'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'February' THEN 'FEBRUARI'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'March' THEN 'MARET'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'April' THEN 'APRIL'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'May' THEN 'MEI'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'June' THEN 'JUNI'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'July' THEN 'JULI'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'August' THEN 'AGUSTUS'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'September' THEN 'SEPTEMBER'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'October' THEN 'OKTOBER'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'November' THEN 'NOVEMBER'
									WHEN DATENAME(MONTH, a.delivery_order_batch_tanggal_kirim) = 'December' THEN 'DESEMBER'
								END + ' ' + 
								CONVERT(VARCHAR, YEAR(a.delivery_order_batch_tanggal_kirim)) AS tgl_kirim
							FROM delivery_order_batch a
							LEFT JOIN (SELECT TOP 1 delivery_order_batch_id, client_wms_nama, dod.do_eksternal_no, dod.so_eksternal_no FROM delivery_order do LEFT JOIN client_wms cw ON do.client_wms_id = cw.client_wms_id INNER JOIN delivery_order_draft dod ON do.delivery_order_draft_id = dod.delivery_order_draft_id WHERE do.delivery_order_batch_id = '$id') b 
							ON a.delivery_order_batch_id = b.delivery_order_batch_id
							LEFT JOIN delivery_order_batch_h c
							ON a.delivery_order_batch_h_id = c.delivery_order_batch_h_id
							LEFT JOIN karyawan d
							ON c.karyawan_id = d.karyawan_id
							LEFT JOIN kendaraan e
							ON c.kendaraan_id = e.kendaraan_id
							WHERE a.delivery_order_batch_id = '$id'")->row();
	}

	function getDetailDOBatch($id)
	{
		return	$this->db->query("SELECT
									sku_kode
									,sku_nama_produk
									,SUM(sku_qty) AS jumlah
									FROM delivery_order_detail
									WHERE delivery_order_batch_id = '$id'
									GROUP BY sku_kode
											,sku_nama_produk
											,SUBSTRING(sku_kode, CHARINDEX('-', sku_kode) + 1, LEN(sku_kode))")->result();
	}

	public function Get_attachment_delivery_order($delivery_order_batch_id)
	{
		$query = $this->db->query("select
									...
									where fdjr.delivery_order_batch_id = '$delivery_order_batch_id'
									order by fdjr.delivery_order_batch_kode asc");

...
class M_KonfirmasiPenerimaanTunai extends CI_Model
{
	...
	public function Get_list_delivery_order($delivery_order_batch_id, $data)
	{
		...
		$query = $this->db->query("SELECT 
									...
									do.client_pt_id,
									ISNULL(do.delivery_order_jumlah_bayar, 0) as delivery_order_jumlah_bayar,
	...
	public function GetListPembayaranByID($id)
	{
		$query = $this->db->query("SELECT
		tp.tipe_pembayaran_id, tp.tipe_pembayaran_nama
	   ,SUM(dopd.delivery_order_jumlah_bayar) AS delivery_order_jumlah_bayar
	   ,SUM(DISTINCT payment.delivery_order_payment_value) AS payment_value
	  FROM delivery_order_payment_driver dopd
	  LEFT JOIN delivery_order do
		ON do.delivery_order_id = dopd.delivery_order_id
	  LEFT JOIN delivery_order_batch dob
		ON do.delivery_order_batch_id = dob.delivery_order_batch_id
	  LEFT JOIN tipe_pembayaran tp
		ON tp.tipe_pembayaran_id = dopd.tipe_pembayaran_id
	  LEFT JOIN (SELECT dopd.delivery_order_id, dop.tipe_pembayaran_id, dop.delivery_order_payment_value 
	  			 FROM delivery_order_payment dop LEFT JOIN delivery_order_payment_detail dopd ON dopd.delivery_order_payment_id = dop.delivery_order_payment_id) 
		AS payment ON payment.delivery_order_id = dopd.delivery_order_id AND payment.tipe_pembayaran_id = dopd.tipe_pembayaran_id
	  WHERE dob.delivery_order_batch_id = '$id'
	  GROUP BY tp.tipe_pembayaran_id, tp.tipe_pembayaran_nama");
	...
	public function GetListDODetail($id, $tipe_pembayaran)
	{
		$query = $this->db->query("SELECT do.delivery_order_id,
		do.delivery_order_kode,
		do.sales_order_id,
		ISNULL(so.sales_order_kode,'') AS sales_order_kode,
		ISNULL(so.sales_order_no_po,'') AS sales_order_no_po,
		do.client_pt_id,
		-- ISNULL(do.delivery_order_jumlah_bayar, 0) as delivery_order_jumlah_bayar,
		ISNULL(dopd.delivery_order_jumlah_bayar, 0) as delivery_order_jumlah_bayar,
		client_pt.client_pt_nama,
		-- ISNULL(do.delivery_order_tipe_pembayaran,'0') AS delivery_order_tipe_pembayaran,
		dopd.tipe_pembayaran_id AS delivery_order_tipe_pembayaran,
		tdo.tipe_delivery_order_alias,
		ISNULL(dopd.delivery_order_payment_driver_keterangan,'') AS keterangan
		FROM delivery_order_payment_driver dopd
		LEFT JOIN delivery_order do ON do.delivery_order_id = dopd.delivery_order_id
		...
		LEFT JOIN tipe_delivery_order tdo ON do.tipe_delivery_order_id = tdo.tipe_delivery_order_id
		WHERE do.delivery_order_batch_id = '$id' AND dopd.tipe_pembayaran_id = '$tipe_pembayaran'
		ORDER BY format(do.delivery_order_tgl_buat_do, 'yyyy-MM-dd') ASC");
	...
	public function GetListDOByID($id)
	{
		$query = $this->db->query("SELECT 
										...
										format(delivery_order_tgl_rencana_kirim, 'dd-MM-yyyy') as delivery_order_tgl_rencana_kirim,
										delivery_order_kode,
										sales_order.sales_order_kode as no_so,
										sales_order.sales_order_no_po as no_so_eksternal,
									...
									FROM delivery_order
									LEFT JOIN tipe_pembayaran ON delivery_order.delivery_order_tipe_pembayaran = tipe_pembayaran.tipe_pembayaran_id
									LEFT JOIN sales_order ON delivery_order.sales_order_id = sales_order.sales_order_id
									LEFT JOIN tipe_delivery_order ON delivery_order.tipe_delivery_order_id = tipe_delivery_order.tipe_delivery_order_id
									WHERE delivery_order_batch_id = '$id'
									ORDER BY format(delivery_order_tgl_buat_do, 'yyyy-MM-dd') ASC");
	...
	public function getDO($delivery_order_batch_id, $tipe_pembayaran_id)
	{
		$query = $this->db->query("SELECT dopd.delivery_order_id 
		FROM delivery_order_payment_driver dopd
		LEFT JOIN delivery_order do ON dopd.delivery_order_id = do.delivery_order_id
		WHERE do.delivery_order_batch_id = '$delivery_order_batch_id' AND dopd.tipe_pembayaran_id = '$tipe_pembayaran_id'");
	...
	public function GetKeterangan($delivery_order_batch_id)
	{
		$query = $this->db->query("SELECT dopd.delivery_order_id, dopd.tipe_pembayaran_id, dopd.delivery_order_payment_driver_keterangan 
		FROM delivery_order_payment_driver dopd
		LEFT JOIN delivery_order do ON dopd.delivery_order_id = do.delivery_order_id
		WHERE do.delivery_order_batch_id = '$delivery_order_batch_id'");
	...
	public function Insert_delivery_order_payment_detail($delivery_order_payment_id, $delivery_order_id)
	{
		$this->db->set("delivery_order_payment_detail_id", "NEWID()", FALSE);
		$this->db->set("delivery_order_payment_id", $delivery_order_payment_id);
		$this->db->set("delivery_order_id", $delivery_order_id);
	...
	public function Update_keterangan_delivery_order_payment_driver($data)
	{
		$keterangan = $data['keterangan'] == "" ? null : $data['keterangan'];

		$this->db->set("delivery_order_payment_driver_keterangan", $keterangan);

		$this->db->where("delivery_order_id", $data['do_id']);
		$this->db->where("tipe_pembayaran_id", $data['tipe_pembayaran_id']);
		$queryupdate = $this->db->update("delivery_order_payment_driver");

...
class M_NonAktifPallet extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT a.pallet_id, a.pallet_kode, d.depo_detail_nama, b.rak_lajur_detail_nama
                                            FROM pallet a
                                    INNER JOIN rak_lajur_detail b ON a.rak_lajur_detail_id = b.rak_lajur_detail_id
                                    INNER JOIN rak c ON b.rak_id = c.rak_id
                                    INNER JOIN depo_detail d ON c.depo_detail_id = d.depo_detail_id
                                    WHERE pallet_id NOT IN (SELECT pallet_id FROM pallet_detail)
                                    AND c.depo_id = '" . $this->session->userdata('depo_id') . "'
                                    ORDER BY a.pallet_kode");
    ...
    public function deletePallet($pallet_id)
    {
    ...
    public function nonAktifPallet($pallet_id)
    {
        $this->db->trans_begin();
        ...
        $this->db->where('pallet_generate_detail2_id ', $pallet_id);
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $result = array(
                'status' => 'error',
                'message' => 'Gagal menon-aktifkan pallet.'
            );
        } else {
            $this->db->trans_commit();
            $result = array(
                'status' => 'success',
                'message' => 'Berhasil menon-aktifkan pallet.'
            );
        }

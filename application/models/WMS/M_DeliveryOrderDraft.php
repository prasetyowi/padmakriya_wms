...
class M_DeliveryOrderDraft extends CI_Model
{
	...
	{
		$query = $this->db->query("select * from gettipegudang() where tipegudang_is_aktif = '1' order by tipegudang_kode asc");
	...
	public function GetSelectedSKU($sku_id, $do_draft_id, $act, $do_reff_id)
	{
		$sku_id = implode("','", $sku_id);

		if ($act == 'dosplit' || $do_reff_id != null) {
			$id = $do_reff_id != null ? $do_reff_id : $do_draft_id;
			...
			WHERE dod.delivery_order_draft_id <> '$do_draft_id'
			AND dod.delivery_order_draft_status not in ('canceled')
			GROUP BY dod.delivery_order_draft_reff_id,
			dodd.sku_id) dodd2
				ON dodd2.delivery_order_draft_reff_id = convert(nvarchar(36),do.delivery_order_draft_id)
				AND dodd2.sku_id = do.sku_id
			LEFT JOIN sku
			  ON sku.sku_id = do.sku_id
			WHERE delivery_order_draft_id = '$id'
			AND do.sku_id in ('" . $sku_id . "')
	...
	public function GetDeliveryOrderDraftHeaderById($id)
	{
		$query = $this->db->query("SELECT
									...
									ISNULL(do.tipe_gudang,'') AS tipe_gudang,
									...
									ISNULL(do.so_eksternal_no,'') AS so_eksternal_no,
									ISNULL(do.do_eksternal_no,'') AS do_eksternal_no
	...
	{
		$delivery_order_draft_reff_id = $this->db->query("SELECT delivery_order_draft_reff_id FROM delivery_order_draft WHERE delivery_order_draft_id = '$id'")->row('delivery_order_draft_reff_id');

		$dod_reff_id = '';
		$left_join = '';
		$select = '';
		if ($delivery_order_draft_reff_id != null) {
			$dod_reff_id = $delivery_order_draft_reff_id;
			$left_join = "LEFT JOIN (SELECT
										dod.delivery_order_draft_reff_id,
										dodd.sku_id,
										SUM(dodd.sku_qty) as sku_qty
										FROM delivery_order_draft dod
										...
										GROUP BY dod.delivery_order_draft_reff_id,
										dodd.sku_id) dodd2
											ON dodd2.delivery_order_draft_reff_id = '$dod_reff_id'
											AND dodd2.sku_id = do.sku_id
									LEFT JOIN (SELECT delivery_order_draft_id, sku_id, sku_qty FROM delivery_order_detail_draft) dodd3 
										ON dodd3.delivery_order_draft_id = dodd2.delivery_order_draft_reff_id 
										AND dodd3.sku_id = dodd2.sku_id";
			$select = "	ISNULL(dodd3.sku_qty, '0') AS sku_qty_induk,
									ISNULL(dodd3.sku_qty, '0') - ISNULL(abs(dodd2.sku_qty), 0) AS sku_qty_sisa,";
		}
	...
	// public function search_filter_chosen_sku($client_pt, $perusahaan, $tipe_pembayaran, $brand, $principle, $sku_induk, $sku_nama_produk, $sku_kemasan, $sku_satuan, $arr_sku_do_split, $dod_id, $dod_reff_id, $act)
	// {
	// 	if ($tipe_pembayaran == "0") {
	// 		$tipe_pembayaran = "";
	// 	} else {
	// 		$tipe_pembayaran = "AND client_pt_principle.client_pt_principle_is_kredit = '1' ";
	// 	}
	...
	// 	if ($act == "dosplit" || $dod_reff_id != null) {
	// 		$from = "FROM delivery_order_detail_draft do LEFT JOIN sku ON do.sku_id = sku.sku_id";
	// 		// $where_do_draft = "AND delivery_order_draft_id = '$dod_id'";
	// 		$id = $dod_reff_id != null ? $dod_reff_id : $dod_id;
	// 		$where_do_draft = "AND sku.sku_id in (SELECT sku_id FROM delivery_order_detail_draft where delivery_order_draft_id = '$id')";

	// 		if ($arr_sku_do_split != null) {
	// 			$implode = "'" . implode("', '", $arr_sku_do_split) . "'";
	// 			$sku = "AND do.sku_id NOT IN ($implode)";
	// 		} else {
	// 			$sku = "";
	// 		}
	// 	} else {
	...
	// 								LEFT JOIN client_pt_principle
	// 		  						ON client_pt_principle.principle_id = principle.principle_id
	...
	public function search_filter_chosen_sku($client_pt, $perusahaan, $tipe_pembayaran, $brand, $principle, $sku_induk, $sku_nama_produk, $sku_kemasan, $sku_satuan, $arr_sku_do_split, $dod_id, $dod_reff_id, $act)
	{
		...
		$sql = "SELECT
									ROW_NUMBER () OVER (ORDER BY principle.principle_kode, sku.sku_kode ASC) AS idx,
									sku.sku_id,
									sku.sku_kode,
									sku.sku_nama_produk,
									sku.principle_id,
									...
									WHERE sku_stock.sku_id IS NOT NULL
									AND depo_client_wms.depo_id = '" . $this->session->userdata('depo_id') . "'
									AND sku.client_wms_id = '" . $perusahaan . "'
									" . $tipe_pembayaran . "
									" . $brand . "
									" . $principle . "
									" . $sku_induk . "
	...
	public function insert_delivery_order_draft($delivery_order_draft_id, $sales_order_id, $delivery_order_draft_kode, $delivery_order_draft_yourref, $client_wms_id, $delivery_order_draft_tgl_buat_do, $delivery_order_draft_tgl_expired_do, $delivery_order_draft_tgl_surat_jalan, $delivery_order_draft_tgl_rencana_kirim, $delivery_order_draft_tgl_aktual_kirim, $delivery_order_draft_keterangan, $delivery_order_draft_status, $delivery_order_draft_is_prioritas, $delivery_order_draft_is_need_packing, $delivery_order_draft_tipe_layanan, $delivery_order_draft_tipe_pembayaran, $delivery_order_draft_sesi_pengiriman, $delivery_order_draft_request_tgl_kirim, $delivery_order_draft_request_jam_kirim, $tipe_pengiriman_id, $nama_tipe, $confirm_rate, $delivery_order_draft_reff_id, $delivery_order_draft_reff_no, $delivery_order_draft_total, $unit_mandiri_id, $depo_id, $client_pt_id, $delivery_order_draft_kirim_nama, $delivery_order_draft_kirim_alamat, $delivery_order_draft_kirim_telp, $delivery_order_draft_kirim_provinsi, $delivery_order_draft_kirim_kota, $delivery_order_draft_kirim_kecamatan, $delivery_order_draft_kirim_kelurahan, $delivery_order_draft_kirim_kodepos, $delivery_order_draft_kirim_area, $delivery_order_draft_kirim_invoice_pdf, $delivery_order_draft_kirim_invoice_dir, $pabrik_id, $delivery_order_draft_ambil_nama, $delivery_order_draft_ambil_alamat, $delivery_order_draft_ambil_telp, $delivery_order_draft_ambil_provinsi, $delivery_order_draft_ambil_kota, $delivery_order_draft_ambil_kecamatan, $delivery_order_draft_ambil_kelurahan, $delivery_order_draft_ambil_kodepos, $delivery_order_draft_ambil_area, $delivery_order_draft_update_who, $delivery_order_draft_update_tgl, $delivery_order_draft_approve_who, $delivery_order_draft_approve_tgl, $delivery_order_draft_reject_who, $delivery_order_draft_reject_tgl, $delivery_order_draft_reject_reason, $tipe_delivery_order_id, $delivery_order_draft_input_pembayaran_tunai, $name_file, $tipe_gudang, $so_eksternal, $do_eksternal)
	{
		...
		$tipe_gudang = $tipe_gudang == "" ? null : $tipe_gudang;
		$so_eksternal = $so_eksternal == "" ? null : $so_eksternal;
		$do_eksternal = $do_eksternal == "" ? null : $do_eksternal;
		...
		$this->db->set("tipe_gudang", $tipe_gudang);
		$this->db->set("so_eksternal_no", $so_eksternal);
		$this->db->set("do_eksternal_no", $do_eksternal);
	...
	public function insert_delivery_order_detail_retur_draft($dod_id, $delivery_order_draft_id, $data)
	{
		// $getData = $this->db->select("sku_weight_product, sku_weight_product_unit")->from("sku")->where("sku_id", $data['sku_id'])->get()->result();
	...
	public function getDataSearchEditDO($tgl1, $tgl2, $area, $status, $principle)
	{
		...
		$query = $this->db->query("SELECT
									...
									ISNULL(do.delivery_order_draft_kirim_area,'') AS delivery_order_draft_kirim_area,
									do.delivery_order_draft_status,
									tp.tipe_delivery_order_id,
									tp.tipe_delivery_order_nama,
									ISNULL(tp.tipe_delivery_order_alias, '') AS tipe_delivery_order_alias,
									ISNULL(tipe_gudang.tipegudang_nama,'') AS tipegudang_nama
									FROM delivery_order_draft do
									LEFT JOIN sales_order so ON so.sales_order_id = do.sales_order_id
									...
									LEFT JOIN gettipegudang() tipe_gudang ON tipe_gudang.tipegudang_kode = do.tipe_gudang
									WHERE format(do.delivery_order_draft_tgl_rencana_kirim, 'yyyy-MM-dd') BETWEEN '$tgl1' AND '$tgl2'
	...
	public function getDataSearchUbahKeDraft($tgl1, $tgl2, $area, $principle)
	{
		if ($area == "") {
			$areaSearch = "";
		} else {
		...
		if ($principle == "") {
			...
			$principleSearch = "AND so.principle_id = '$principle'";
		}
		...
		$query = $this->db->query("SELECT
									...
									LEFT JOIN principle ON so.principle_id = principle.principle_id
									WHERE format(do.delivery_order_draft_tgl_rencana_kirim, 'yyyy-MM-dd') BETWEEN '$tgl1' AND '$tgl2'
										AND do.delivery_order_draft_status in ('Approved', 'canceled')
										AND do.depo_id = '" . $this->session->userdata('depo_id') . "'
	...
	public function Get_delivery_order_draft_detail_msg_by_list_do($arr_list_do_msg_error)
	{
		$arr_list_do_msg_error_str = "";

		if (isset($arr_list_do_msg_error) && count($arr_list_do_msg_error) > 0) {
			$arr_list_do_msg_error_str = " AND msg.delivery_order_draft_id IN (" . implode(",", $arr_list_do_msg_error) . ") ";
		}
	...
	public function GetDeliveryOrderDraftPrioritasStokByFilter($tgl, $tipe, $tipe_gudang)
	{
		$wtipe = "";
		...
		if ($tipe_gudang != "") {
			$tipe_gudang = " AND dd.stock_type_kode = '$tipe_gudang' ";
		}
	...
	public function Get_delivery_order_draft_detail_by_sku_id($tgl, $tipe, $sku_id, $flag)
	{
		...
		$query = $this->db->query("SELECT
									...
									so.sales_order_no_po,
									CASE WHEN dodrt.delivery_order_detail_draft_id IS NOT NULL THEN '1' ELSE '0' END is_abaikan,
									CASE WHEN dodrt.delivery_order_detail_draft_id IS NOT NULL THEN '1' ELSE '0' END is_revisi,
									COUNT(DISTINCT ss2.sku_konversi_group) AS jml_sku_stock_group
								FROM delivery_order_draft hdr
									LEFT JOIN delivery_order_detail_draft dtl
									ON dtl.delivery_order_draft_id = hdr.delivery_order_draft_id
									LEFT JOIN (SELECT
									...
									GROUP BY sku_id) ss
									ON dtl.sku_id = ss.sku_id
									LEFT JOIN (select delivery_order_detail_draft_id, sku_id_asal from delivery_order_detail_revisi_temp group by delivery_order_detail_draft_id, sku_id_asal) AS dodrt
									ON dodrt.delivery_order_detail_draft_id = dtl.delivery_order_detail_draft_id
									AND dodrt.sku_id_asal = dtl.sku_id
									LEFT JOIN tipe_delivery_order tp
									ON tp.tipe_delivery_order_id = hdr.tipe_delivery_order_id
									INNER JOIN sku s
									ON s.sku_id = dtl.sku_id
									...
									LEFT JOIN (SELECT 
										...
										ss.sku_id
									FROM sku_stock ss
									INNER JOIN depo_detail dd ON dd.depo_detail_id = ss.depo_detail_id
									...
									GROUP BY sku.sku_konversi_group,
											 sku.sku_nama_produk,
											 ss.sku_id
									HAVING SUM(ISNULL(ss.sku_stock_awal, 0) + ISNULL(ss.sku_stock_masuk, 0) - ISNULL(ss.sku_stock_saldo_alokasi, 0) - ISNULL(ss.sku_stock_keluar, 0)) > 0
									) ss2
									ON ss2.sku_konversi_group = s.sku_konversi_group and ss2.sku_id <> dtl.sku_id
									WHERE hdr.depo_id = '$depo_id'
									AND FORMAT(hdr.delivery_order_draft_tgl_rencana_kirim, 'yyyy-MM-dd') <= '$tgl'
									AND hdr.delivery_order_draft_status = 'Draft'
									AND tp.tipe_delivery_order_alias = '$tipe'
									...
									GROUP BY hdr.delivery_order_draft_id,
											...
											dtl.sku_id,
											so.sales_order_no_po,
											CASE WHEN dodrt.delivery_order_detail_draft_id IS NOT NULL THEN '1' ELSE '0' END
	...
	public function proses_approval_delivery_order_draft($delivery_order_draft_id, $delivery_order_draft_update_tgl, $pengguna_username)
	{
		$query = $this->db->query("exec proses_approval_delivery_order_draft '$delivery_order_draft_id','$delivery_order_draft_update_tgl','$pengguna_username'");
		...
		if ($query->num_rows() == 0) {
			...
			foreach ($query->result_array() as $key => $value) {
				...
				if (isset($value['StatusMsg']) && $value['StatusMsg'] == "400") {

					$this->db->query("INSERT INTO delivery_order_draft_detail_msg (delivery_order_draft_detail_msg_id,depo_id,delivery_order_draft_id,sku_kode,delivery_order_draft_detail_msg,principle_id)
					SELECT NEWID(),'" . $value['depo_id'] . "','" . $value['delivery_order_draft_id'] . "','" . $value['sku_kode'] . "','" . $value['ErrMsg'] . "','" . $value['principle_id'] . "'");
				}
			}
	...
	public function proses_delivery_order_ubah_ke_draft($delivery_order_draft_id, $pengguna_username)
	{
		$query = $this->db->query("exec proses_delivery_order_ubah_ke_draft '$delivery_order_draft_id','$pengguna_username'");
	...
	public function proses_pembatalan_delivery_order_draft($delivery_order_draft_id, $pengguna_username)
	{
		$query = $this->db->query("exec proses_pembatalan_delivery_order_draft '$delivery_order_draft_id','$pengguna_username'");
	...
	public function Get_sku_group_by_sku_konversi_group($delivery_order_detail_draft_id, $sku_konversi_group, $tipe_gudang, $tipe_do, $sku_id)
	{
		$wtipe = "";
		if ($tipe_do == 'DO Reschedule') {
			$wtipe = "AND isnull(dd.depo_detail_flag_jual, 0) = 1
						AND isnull(dd.depo_detail_is_bonus, 0) in (0)
						AND isnull(dd.depo_detail_is_flashout, 0) = 0
						AND isnull(dd.depo_detail_is_kirimulang, 0) = 1
		...
		if ($tipe_do == 'Picking per SKU' || $tipe_do == 'Canvas') {
			$wtipe = "AND isnull(dd.depo_detail_flag_jual, 0) = 1
						AND isnull(dd.depo_detail_is_bonus, 0) in (0,1)
						AND isnull(dd.depo_detail_is_flashout, 0) = 0
						AND isnull(dd.depo_detail_is_kirimulang, 0) = 0
		...
		$query = $this->db->query("SELECT
										...
										ss.sku_kemasan,
										ss.sku_satuan,
										ss.sku_konversi_group,
										ss.sku_konversi_level,
										ss.sku_konversi_faktor,
										ISNULL(ss.sku_stock_qty, 0) AS sku_stock_qty_asli,
										ISNULL(dodr.sku_qty, 0) AS sku_qty,
										ISNULL(ss.sku_stock_qty, 0) - ISNULL(dodrt.sku_qty, 0) AS sku_stock_qty
									FROM (SELECT ss.sku_id,
										sku.sku_kode,
										sku.sku_nama_produk,
										sku.sku_kemasan,
										sku.sku_satuan,
										...
										sku.sku_konversi_faktor,
										SUM(ISNULL(ss.sku_stock_awal, 0) + ISNULL(ss.sku_stock_masuk, 0) - ISNULL(ss.sku_stock_saldo_alokasi, 0) - ISNULL(ss.sku_stock_keluar, 0)) AS sku_stock_qty
									FROM sku_stock ss
									INNER JOIN depo_detail dd ON dd.depo_detail_id = ss.depo_detail_id
									LEFT JOIN sku ON sku.sku_id = ss.sku_id
									WHERE sku.sku_konversi_group = '$sku_konversi_group'
									AND sku.sku_id NOT IN ('$sku_id')
									AND ss.depo_id = '$depo_id'
									" . $tipe_gudang . "
									" . $wtipe . "
									GROUP BY ss.sku_id,
										sku.sku_kode,
										...
										sku.sku_konversi_group,
										sku.sku_konversi_level,
										sku.sku_konversi_faktor) AS ss
									LEFT JOIN delivery_order_detail_revisi_temp dodr 
									ON dodr.sku_id = ss.sku_id
									AND dodr.delivery_order_detail_draft_id = '$delivery_order_detail_draft_id'
									LEFT JOIN (SELECT sku_id, 
											sum(sku_qty) AS sku_qty
									FROM delivery_order_detail_revisi_temp
									GROUP BY sku_id) dodrt ON dodrt.sku_id = ss.sku_id
									WHERE ss.sku_id IS NOT NULL
									AND ISNULL(ss.sku_stock_qty, 0) > 0
									ORDER BY ss.sku_konversi_group ASC, ss.sku_konversi_level DESC");
	...
	public function Get_delivery_order_detail_revisi_temp_by_do_id($delivery_order_detail_draft_id)
	{

		$query = $this->db->query("SELECT 
									delivery_order_detail_revisi_id,
									delivery_order_detail_draft_id,
									delivery_order_draft_id,
									sku_id_asal,
									sku_id,
	...
	public function insert_delivery_order_detail_revisi_temp($dod_revisi_id, $delivery_order_detail_draft_id, $delivery_order_draft_id, $sku_id_asal, $sku_id, $sku_qty)
	{
		$delivery_order_draft_id = $delivery_order_draft_id == "" ? null : $delivery_order_draft_id;
		$sku_id_asal = $sku_id_asal == "" ? null : $sku_id_asal;
		$sku_id = $sku_id == "" ? null : $sku_id;
		$sku_qty = $sku_qty == "" ? 0 : $sku_qty;

		$this->db->set("delivery_order_detail_revisi_id", $dod_revisi_id);
		$this->db->set("delivery_order_detail_draft_id", $delivery_order_detail_draft_id);
		$this->db->set("delivery_order_draft_id", $delivery_order_draft_id);
		$this->db->set("sku_id_asal", $sku_id_asal);
		$this->db->set("sku_id", $sku_id);
		$this->db->set("sku_qty", $sku_qty);
		$this->db->set("pengguna_username", $this->session->userdata('pengguna_username'));
		$this->db->set("update_date", "GETDATE()", FALSE);

		$queryinsert = $this->db->insert("delivery_order_detail_revisi_temp");

		return $queryinsert;
		// return $this->db->last_query();
	}

	public function proses_revisi_delivery_order_detail_draft($delivery_order_detail_draft_id, $delivery_order_draft_id, $sku_id, $sku_qty, $log_tipe, $pengguna_username)
	{

		$query = $this->db->query("exec proses_revisi_delivery_order_detail_draft '$delivery_order_detail_draft_id','$delivery_order_draft_id', '$sku_id', '$sku_qty', '$log_tipe', '$pengguna_username'");

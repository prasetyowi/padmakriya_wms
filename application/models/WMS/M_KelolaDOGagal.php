...
class M_KelolaDOGagal extends CI_Model
{
	...
	public function getPrincipleByPerusahaan($perusahaan_id)
	{
		$query = $this->db->query("SELECT b.principle_id, b.principle_kode
		FROM client_wms_principle a
		LEFT JOIN principle b ON a.principle_id = b.principle_id
		WHERE a.client_wms_id = '$perusahaan_id'");
	...
	public function GetTotalKelolaDOGagalByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $search, $so_eksternal, $is_reschedule, $client_wms_id, $principle_id)
	{
	...
	public function GetKelolaDOGagalByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $start, $end, $search, $so_eksternal, $is_reschedule, $client_wms_id, $principle_id)
	{
		...
		$query = $this->db->query("select 
									...
									do.delivery_order_kirim_area,
									ISNULL(do.delivery_order_nominal_tunai, 0) AS delivery_order_nominal_tunai,
									isnull(p.principle_kode, '') as principle_kode
									FROM delivery_order do
									LEFT JOIN sales_order so
									ON so.sales_order_id = do.sales_order_id
	...
	public function GetKelolaDOGagalNotRescheduleByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $so_eksternal, $client_wms_id, $principle_id)
	{
	...
	public function GetKelolaDOGagalNotRescheduleBuatDraftByFilter($tgl1, $tgl2, $do_no, $customer, $alamat, $tipe_pembayaran, $tipe_layanan, $status, $tipe, $segmen1, $segmen2, $segmen3, $so_eksternal, $client_wms_id, $principle_id)
	{
	...
	public function Generate_do_not_reschedule_detail($delivery_order_draft_id, $delivery_order_id, $tipe_delivery_order_id)
	{
		...
		if ($tipe_delivery_order_id != 'C5BE83E2-01E8-4E24-B766-26BB4158F2CD') {
			$tipe_delivery_order = "AND tipe_delivery_order_id NOT IN ('C5BE83E2-01E8-4E24-B766-26BB4158F2CD')";
		} else {
			$tipe_delivery_order = "";
		}
		...
		right JOIN (
				...
				FROM delivery_order_detail2_gagal a 
			INNER JOIN delivery_order b on a.delivery_order_id = b.delivery_order_id
			WHERE a.delivery_order_id IN ('$delivery_order_id') " . $tipe_delivery_order . "
					UNION
				SELECT DISTINCT
					delivery_order_detail_id,
					a.delivery_order_id,
	...
	public function Generate_do_not_reschedule_detail2($delivery_order_draft_id, $delivery_order_id, $tipe_delivery_order_id)
	{
		// $query = $this->db->query("insert delivery_order_detail_draft (delivery_order_detail_draft_id,
		// 							delivery_order_draft_id,
		// 							sku_id,
		...
		$query = $this->db->query("insert delivery_order_detail2_draft (delivery_order_detail2_draft_id,
		...
		sku_qty_composite)
		select 
		NEWID() AS delivery_order_detail2_draft_id,
		dodd.delivery_order_detail_draft_id AS delivery_order_detail_draft_id,
		'$delivery_order_draft_id' AS delivery_order_draft_id,
		dod2.sku_id,
		dod.sku_stock_id,
		dod.sku_expdate,
		dod2.sku_qty,
		dod.sku_qty_composite
		from delivery_order_detail2 dod
		right JOIN (
				SELECT DISTINCT
					delivery_order_detail2_id,
					a.delivery_order_id,
					sku_id,
					sku_stock_id,
					sku_expdate,
			...
			INNER JOIN delivery_order b on a.delivery_order_id = b.delivery_order_id
			WHERE a.delivery_order_id IN ('$delivery_order_id') AND tipe_delivery_order_id IN ('C5BE83E2-01E8-4E24-B766-26BB4158F2CD')
		) dod2 on dod2.delivery_order_detail2_id = dod.delivery_order_detail2_id
		INNER JOIN sku
			ON dod2.sku_id = sku.sku_id
		INNER JOIN delivery_order_detail_draft dodd
			ON dodd.delivery_order_draft_id = '$delivery_order_draft_id' AND dodd.sku_id = dod2.sku_id
		where dod.delivery_order_id = '$delivery_order_id'");
	...
	public function process_delivery_order_reschedule($delivery_order_id, $tgl_kirim_ulang, $pengguna_username)
	{
		$query = $this->db->query("exec proses_delivery_order_reschedule '$delivery_order_id','$tgl_kirim_ulang','$pengguna_username'");
	...
	public function process_delivery_order_not_reschedule($delivery_order_id, $tgl_kirim_ulang, $pengguna_username)
	{
		$query = $this->db->query("exec proses_delivery_order_not_reschedule '$delivery_order_id','$tgl_kirim_ulang','$pengguna_username'");

...
class M_PersiapanMutasiAntarUnit extends CI_Model
{
	...
	// 			((isnull(sk.sku_stock_awal,0) + isnull(sku_stock_masuk, 0)) - (isnull(sk.sku_stock_saldo_alokasi, 0) - isnull(sk.sku_stock_keluar, 0))) * CAST(s.sku_konversi_faktor AS INT) as sku_qty_pcs,
	// 			CAST(s.sku_konversi_faktor AS INT) as cek
	// 			, row_number() over (
	...
	function getDataSKUByParams()
	{
		...
		$total_data         = $this->count_all_data(null, null, null, null, null, $depo_id, $dataPost['principleId'], $dataPost['depoDetailId']);
		$records_filtered   = $total_data;
		$principle = $dataPost['principleId'] == 'all' ? 'null' : "'" . $dataPost['principleId'] . "'";

		$data = $this->db->query("exec proses_sku_stock_composite_global null,$length,$order_column,'$order_dir', '$depo_id', " . $principle . ", '" . $dataPost['depoDetailId'] . "'")->result_array();
	...
	public function count_all_data($start = null, $length = null, $search_value = null, $order_column = null, $order_dir = null, $depo_id, $principle_id, $depo_detail_id)
	{
		...
		$query = $this->db->query("exec proses_sku_stock_composite_global $start,$length,$order_column,$order_dir, '$depo_id', $principle_id, $depo_detail_id");
		// Ambil hasil query
				...
				if ($dataPost->mutasiAntarDepoDetail) {
					foreach ($dataPost->mutasiAntarDepoDetail as $key => $mutasiAntarDepoDetail) {
						if ($mutasiAntarDepo->counter === $mutasiAntarDepoDetail->counter) {
							...
							$sku_stock_id = $this->db->query("exec proses_bongkar_otomatis '" . $this->session->userdata('depo_id') . "', '" . $mutasiAntarDepo->gudangAsal . "', '" . $mutasiAntarDepoDetail->sku_id . "', '" . $mutasiAntarDepoDetail->ed . "', '" . $mutasiAntarDepoDetail->qtyAmbil . "'")->row();
							...
							$this->db->insert('tr_mutasi_depo_detail_2', [
								...
								'sku_stock_id' => $sku_stock_id->sku_stock_id,
	...
	function getDataMutasiDepo($dataPost): array
	{
		...
		$mutasiDepoDetail2 = $this->db->query("SELECT sk.sku_stock_id, si.sku_induk_nama, s.sku_id, s.sku_kode, s.sku_nama_produk, s.sku_kemasan, s.sku_satuan, s.sku_konversi_group, 
		s.sku_konversi_faktor, p.principle_kode, format(sk.sku_stock_expired_date, 'yyyy-MM-dd') as ed, 
		qtySum.total_qtySistem AS qtySistem,  qtyAmbilSum.total_qtyAmbil AS qtyAmbilPcs,
		tmd.qty_plan as qtyAmbil, tmd.tr_mutasi_depo_detail_id
		FROM tr_mutasi_depo_detail_2 tmd
		LEFT JOIN sku_stock sk ON tmd.sku_stock_id = sk.sku_stock_id
		LEFT JOIN sku s ON tmd.sku_id = s.sku_id
		LEFT JOIN sku_induk si ON sk.sku_induk_id = si.sku_induk_id
		LEFT JOIN principle p ON s.principle_id = p.principle_id
		LEFT JOIN principle_brand pb ON s.principle_brand_id = pb.principle_brand_id
		LEFT JOIN (
			SELECT 
				sk_inner.sku_stock_expired_date, 
				s_inner.sku_konversi_group,
				sk_inner.depo_detail_id, 
				sk_inner.depo_id,
				sk_inner.client_wms_id, 
				SUM((ISNULL(sk_inner.sku_stock_awal, 0) + ISNULL(sk_inner.sku_stock_masuk, 0) - ISNULL(sk_inner.sku_stock_saldo_alokasi, 0) - ISNULL(sk_inner.sku_stock_keluar, 0)) * s_inner.sku_konversi_faktor) AS total_qtySistem
			FROM 
				sku_stock sk_inner
			LEFT JOIN 
				sku s_inner ON sk_inner.sku_id = s_inner.sku_id
			GROUP BY 
				sk_inner.sku_stock_expired_date, 
				s_inner.sku_konversi_group,
				sk_inner.depo_detail_id, 
		...
		ON sk.sku_stock_expired_date = qtySum.sku_stock_expired_date 
		AND s.sku_konversi_group = qtySum.sku_konversi_group
		AND sk.depo_detail_id = qtySum.depo_detail_id
		AND sk.depo_id = qtySum.depo_id
		AND sk.client_wms_id = qtySum.client_wms_id
		LEFT JOIN (
			SELECT 
				sk_inner.sku_stock_expired_date, 
				s_inner.sku_konversi_group,
				...
				sk_inner.depo_id,
				sk_inner.client_wms_id, 
				tmd_inner.tr_mutasi_depo_id,
				SUM(tmd_inner.qty_plan * s_inner.sku_konversi_faktor) AS total_qtyAmbil
			FROM 
				tr_mutasi_depo_detail_2 tmd_inner
			LEFT JOIN 
				sku_stock sk_inner ON tmd_inner.sku_stock_id = sk_inner.sku_stock_id
			LEFT JOIN 
				sku s_inner ON tmd_inner.sku_id = s_inner.sku_id
			GROUP BY 
				sk_inner.sku_stock_expired_date, 
				s_inner.sku_konversi_group,
				sk_inner.depo_detail_id, 
		...
		ON sk.sku_stock_expired_date = qtyAmbilSum.sku_stock_expired_date 
		AND s.sku_konversi_group = qtyAmbilSum.sku_konversi_group
		AND sk.depo_detail_id = qtyAmbilSum.depo_detail_id
		AND sk.depo_id = qtyAmbilSum.depo_id
		AND sk.client_wms_id = qtyAmbilSum.client_wms_id
		AND  tmd.tr_mutasi_depo_id = qtyAmbilSum.tr_mutasi_depo_id
		WHERE tmd.tr_mutasi_depo_id = '$dataPost->trMutasiDepoId'")->result();
	...
	public function getSKUSatuan($sku_konversi_group)
	{
	   ...
	   ,sku_konversi_faktor
	  FROM sku
	  WHERE sku_konversi_group = '$sku_konversi_group'
	  ORDER BY sku_konversi_level ASC");

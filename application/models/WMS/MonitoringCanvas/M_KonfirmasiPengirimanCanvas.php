...
class M_KonfirmasiPengirimanCanvas extends CI_Model
{
	...
	public function getDataByFilter($dataPost)
	{
		...
		$this->db->select("dob.delivery_order_batch_id as id_fdjr,
			...
			->join('canvas', 'do.canvas_id = canvas.canvas_id', 'inner')
			...
			->where("FORMAT(dob.delivery_order_batch_tanggal_kirim, 'yyyy-MM-dd') >=", date('Y-m-d', strtotime(str_replace("/", "-", $exlodeTanggal[0]))))
			->where("FORMAT(dob.delivery_order_batch_tanggal_kirim, 'yyyy-MM-dd') <=", date('Y-m-d', strtotime(str_replace("/", "-", $exlodeTanggal[1]))));
